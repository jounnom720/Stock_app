...
APP_VERSION = "v5.14.9 share final"  # Share 최종: 공개 샘플 고정 / 개인 자동복원 차단 / 세션 상태 중심
...
if 모바일여부():
    ...
    st.caption("Share 버전 · 최초 샘플 제공 · 사용자별 세션 데이터 분리")
else:
    st.title(f"📈 투자 분석 시스템 {APP_VERSION}")
    st.caption("Share 버전 · 최초 샘플 제공 · 사용자별 세션 데이터 분리")
...
def Share공개샘플거래이력표():
    """Share 공개용 기본 샘플 거래이력입니다.
    개인 거래이력, 자동저장 JSON, 복원 메타를 참조하지 않습니다.
    """
    return pd.DataFrame([
        {"종목코드": "069500", "종목명": "KODEX 200", "거래일자": "2026-01-05", "거래구분": "매수", "거래수량": 8, "거래단가": 36000, "운용사": "샘플계좌", "비고": "공개용 샘플"},
        {"종목코드": "005930", "종목명": "삼성전자", "거래일자": "2026-01-07", "거래구분": "매수", "거래수량": 5, "거래단가": 72000, "운용사": "샘플계좌", "비고": "공개용 샘플"},
        {"종목코드": "000660", "종목명": "SK하이닉스", "거래일자": "2026-01-10", "거래구분": "매수", "거래수량": 2, "거래단가": 185000, "운용사": "샘플계좌", "비고": "공개용 샘플"},
        {"종목코드": "487240", "종목명": "KODEX AI전력핵심설비", "거래일자": "2026-02-03", "거래구분": "매수", "거래수량": 4, "거래단가": 61000, "운용사": "샘플계좌", "비고": "공개용 샘플"},
        {"종목코드": "069500", "종목명": "KODEX 200", "거래일자": "2026-03-04", "거래구분": "매도", "거래수량": 2, "거래단가": 38500, "운용사": "샘플계좌", "비고": "공개용 샘플 부분매도"},
    ], columns=거래이력표준열)
...
if "share_session_id_v5146" not in st.session_state:
    st.session_state["share_session_id_v5146"] = uuid.uuid4().hex[:12]
SHARE_SESSION_ID = st.session_state["share_session_id_v5146"]

거래이력자동저장파일 = f"v5146_share_{SHARE_SESSION_ID}_trade_history_autosave.json"
최근업로드거래이력파일 = f"v5146_share_{SHARE_SESSION_ID}_trade_history_latest_uploaded.json"
최근업로드메타파일 = f"v5146_share_{SHARE_SESSION_ID}_trade_history_latest_uploaded_meta.json"
거래이력복원메타파일 = f"v5146_share_{SHARE_SESSION_ID}_trade_history_restore_meta.json"
...
IRP비주식자산저장파일 = f"v5146_share_{SHARE_SESSION_ID}_integrated_non_stock_assets.json"
IRP비주식자산레거시저장파일목록 = []


def 기본IRP비주식자산표():
    """Share 공개용 비주식자산 샘플입니다. 개인 IRP/예수금 정보는 포함하지 않습니다."""
    return pd.DataFrame([
        {"계좌": "샘플 IRP", "자산군": "정기예금", "상품명": "샘플 정기예금", "원금": 3000000, "평가금액": 3030000, "예상연수익률": 3.0, "만기일": "2027-01-31", "반영일자": "2026-05-01", "비고": "공개용 샘플"},
        {"계좌": "샘플 IRP", "자산군": "TDF", "상품명": "샘플 TDF 2035", "원금": 2000000, "평가금액": 2080000, "예상연수익률": 4.0, "만기일": "", "반영일자": "2026-05-01", "비고": "공개용 샘플"},
        {"계좌": "샘플 증권", "자산군": "현금성자산", "상품명": "샘플 예수금", "원금": 1000000, "평가금액": 1000000, "예상연수익률": 0.0, "만기일": "", "반영일자": "2026-05-01", "비고": "공개용 샘플"},
    ])
...
def 자동백업파일저장(df, backup_type="daily", reason="", source="app"):
    """Share 버전: 서버 백업 파일을 만들지 않습니다."""
    st.session_state["share_backup_skipped_v5149"] = 서울현재시각ISO()
    return True, "Share 버전에서는 서버 백업 파일을 생성하지 않습니다."
...
def 자동백업일일실행(df):
    """Share 버전: 일일 자동백업을 수행하지 않습니다."""
    return False, "Share 버전에서는 일일 자동백업을 건너뜁니다."


def 자동백업수정전실행(이전df, 다음df=None, source="editor"):
    """Share 버전: 수정 전 백업을 수행하지 않습니다."""
    return False, "Share 버전에서는 수정 전 백업을 건너뜁니다."
...
def 거래이력복원메타저장(meta):
    """Share 버전: 복원 메타를 파일로 저장하지 않습니다."""
    st.session_state["share_trade_restore_meta_v5149"] = meta if isinstance(meta, dict) else {}
    return True, "Share 세션 메타 반영 완료"


def 거래이력복원메타불러오기():
    """Share 버전: 과거 JSON 복원 메타를 읽지 않습니다."""
    return st.session_state.get("share_trade_restore_meta_v5149")


def 거래이력자동저장실행(df):
    """Share 버전: 거래이력을 서버 JSON에 저장하지 않고 현재 세션에만 보관합니다."""
    try:
        작업 = 거래이력편집용자동보정(df if df is not None else pd.DataFrame())
        st.session_state["trade_history_editor_df_v1"] = 작업.copy()
        st.session_state["trade_history_df_v22"] = 작업.copy()
        st.session_state["trade_history_calc_df_v1"] = 거래이력계산대상추출(작업)
        st.session_state["trade_history_last_saved_signature_v1"] = 거래이력서명생성(작업)
        st.session_state["share_trade_restore_meta_v5149"] = 거래이력저장메타생성(작업, source="share_session")
        return True, "Share 세션에만 반영했습니다."
    except Exception as e:
        return False, f"Share 세션 반영 실패: {e}"


def 최근업로드거래이력저장(df, 파일명=""):
    """Share 버전: 최근 업로드본도 서버 파일이 아니라 현재 세션에만 보관합니다."""
    try:
        작업 = 거래이력편집용자동보정(df if df is not None else pd.DataFrame())
        st.session_state["share_latest_uploaded_trade_df_v5149"] = 작업.copy()
        st.session_state["share_latest_uploaded_meta_v5149"] = {
            "파일명": 파일명 or "",
            "저장시각": 서울현재시각ISO(),
            "건수": int(len(작업)),
            "source": "share_latest_uploaded_session",
            "app_version": APP_VERSION,
        }
        return True, "Share 세션에 최근 업로드본을 반영했습니다."
    except Exception as e:
        return False, f"Share 세션 업로드 반영 실패: {e}"


def 최근업로드거래이력불러오기():
    """Share 버전: 현재 세션의 최근 업로드본만 반환합니다."""
    df = st.session_state.get("share_latest_uploaded_trade_df_v5149")
    if df is None:
        return None
    try:
        return 거래이력편집용자동보정(df.copy())
    except Exception:
        return None


def 최근업로드메타불러오기():
    """Share 버전: 서버 메타 파일을 읽지 않고 현재 세션 메타만 반환합니다."""
    return st.session_state.get("share_latest_uploaded_meta_v5149")
...
def 거래이력세션반영(df, 저장강제=False, 자동저장허용=True):
    """Share 버전: 기존 호출부와 동일하게 4개 값을 반환하되 서버 파일 저장은 하지 않습니다."""
    편집df = 거래이력편집용자동보정(df)
...
def 거래이력복원후보목록생성():
    """Share 버전: 서버에 남은 v5146/v5145 JSON 자동복원 후보를 모두 무시합니다."""
    return []


def 거래이력자동복원상태문구():
    출처 = st.session_state.get("trade_history_source_v1", "public_sample")
    건수 = len(st.session_state.get("trade_history_df_v22", pd.DataFrame()))
    if 출처 in ["latest_uploaded", "share_upload", "share_session", "editor"]:
        return f"Share 세션 데이터: 현재 {건수}건 · 서버 자동복원 없음"
    return f"Share 공개 샘플: 현재 {건수}건 · 서버 자동복원 없음"


def 현재거래이력가져오기():
    """Share 최종 버전 거래이력 시작 규칙.
    1) 현재 Streamlit 세션에 거래이력이 있으면 그것을 사용합니다.
    2) 세션이 없으면 서버 JSON/복원 메타/개인 기본값을 보지 않고 공개 샘플로 시작합니다.
    3) 따라서 재실행·새 접속 때 개인 거래이력이 기본값으로 되살아나지 않습니다.
    """
    if "trade_history_df_v22" in st.session_state:
        현재df = st.session_state.get("trade_history_df_v22", pd.DataFrame())
        if isinstance(현재df, pd.DataFrame) and not 현재df.empty:
            try:
                동적종목매핑갱신(현재df)
            except Exception:
                pass
    ...
    st.session_state["trade_history_source_v1"] = "public_sample"
...
with st.sidebar.expander("거래이력 관리", expanded=False):
    ...
    st.caption("Share 세션 방식: 서버 자동복원/개인 JSON 저장을 사용하지 않습니다.")
