...
APP_VERSION = "v5.14.6 share"  # Share용 사용자별 세션 데이터 분리 / 최초 샘플 제공 / 서버 저장 충돌 방지
...
def 현재거래이력가져오기():
    ...
    if "trade_history_df_v22" not in st.session_state:
        ...
        if 후보목록:
            ...
            # legacy 파일에서 복원된 경우 즉시 v5.14.6 저장파일로 이관합니다.
            if 출처 == "legacy_migrated":
                try:
                    거래이력자동저장실행(초기df)
                except Exception:
                    pass
            ...
            초기df = 기본포트폴리오.copy()
            st.session_state["trade_history_source_v1"] = "empty_default"
        ...
        if not 편집df.empty:
            거래이력복원메타저장(거래이력저장메타생성(편집df, source=st.session_state.get("trade_history_source_v1", "restore")))
...
with st.sidebar.expander("거래이력 관리", expanded=False):
    ...
    st.caption(f"Share 세션 ID: {SHARE_SESSION_ID} · 다른 사용자와 저장파일이 분리됩니다.")
