...
if not PYKRX_AVAILABLE:
    st.error("pykrx가 설치되어 있지 않습니다. 터미널에서 'pip install pykrx setuptools' 후 다시 실행해 주세요.")
    st.stop()
...
def 전체종목매핑가져오기():
    ...
    try:
        ...
        for 시장 in ["KOSPI", "KOSDAQ", "ETF"]:
            try:
                코드목록 = stock.get_market_ticker_list(오늘, market=시장)
            except TypeError:
                코드목록 = stock.get_market_ticker_list(market=시장)
            for 코드 in 코드목록:
                try:
                    이름 = stock.get_market_ticker_name(코드)
                    if 이름:
                        결과[str(코드).zfill(6)] = str(이름).strip()
                except Exception:
                    continue
    except Exception:
        return 결과
...
def 종목명자동보정(종목코드, 종목명=""):
    코드 = str(종목코드).strip().zfill(6) if 종목코드 is not None else ""
    이름 = "" if pd.isna(종목명) else str(종목명).strip()
    if 코드 in 코드명매핑:
        return 코드명매핑[코드]
    if PYKRX_AVAILABLE and len(코드) == 6 and 코드.isdigit():
        try:
            조회이름 = stock.get_market_ticker_name(코드)
            if 조회이름:
                return str(조회이름).strip()
        except Exception:
            pass
    return 이름 if 이름 else 코드
...
def 종목코드자동보정(종목명, 종목코드=""):
    ...
    if 이름 in 이름코드매핑:
        return 이름코드매핑[이름]
    전체매핑 = 전체종목매핑가져오기()
    for 조회코드, 조회이름 in 전체매핑.items():
        if 조회이름 == 이름:
            return 조회코드
    return 숫자코드.zfill(6) if 숫자코드 else ""
...
def 거래이력자동보정(df):
    ...
    보정["종목코드"] = 보정["종목코드"].apply(lambda 값: "" if pd.isna(값) else str(값))
    보정["종목명"] = 보정["종목명"].apply(lambda 값: "" if pd.isna(값) else str(값).strip())
    보정["비고"] = 보정["비고"].apply(lambda 값: "" if pd.isna(값) else str(값).strip())

    보정["종목코드"] = 보정.apply(
        lambda 행: 종목코드자동보정(행.get("종목명", ""), 행.get("종목코드", "")),
        axis=1,
    )
    보정["종목명"] = 보정.apply(
    ...
    보정["종목코드"] = 보정["종목코드"].apply(
        lambda 값: 값 if (isinstance(값, str) and len(값) == 6 and 값.isdigit()) else ""
    )

    보정["거래일자"] = pd.to_datetime(보정["거래일자"], errors="coerce").dt.date
    보정["거래구분"] = 보정["거래구분"].astype(str).str.strip().replace({"buy": "매수", "BUY": "매수", "sell": "매도", "SELL": "매도"})
    보정.loc[~보정["거래구분"].isin(["매수", "매도"]), "거래구분"] = "매수"
    보정["거래수량"] = pd.to_numeric(보정["거래수량"], errors="coerce").fillna(0).clip(lower=0)
    보정["거래단가"] = 보정["거래단가"].apply(통화문자정리)
    보정["거래단가"] = pd.to_numeric(보정["거래단가"], errors="coerce").fillna(0).clip(lower=0)

    보정 = 보정[["종목코드", "종목명", "거래일자", "거래구분", "거래수량", "거래단가", "비고"]].copy()
    완전빈행 = (
        보정["종목코드"].eq("")
        & 보정["종목명"].eq("")
        & 보정["거래일자"].isna()
        & 보정["거래수량"].fillna(0).eq(0)
        & 보정["거래단가"].fillna(0).eq(0)
        & 보정["비고"].eq("")
    )
    보정 = 보정.loc[~완전빈행].reset_index(drop=True)
    return 보정
...
def 거래이력검증표생성(df):
    ...
    점검결과 = []
    작업 = df.reset_index(drop=True).copy()
    for idx, 행 in 작업.iterrows():
        ...
        if pd.isna(pd.to_datetime(거래일자, errors="coerce")):
...
기본포트폴리오 = pd.DataFrame([
    {"종목코드": "069500", "종목명": "KODEX 200", "거래일자": "2026-02-20", "거래구분": "매수", "거래수량": 11, "거래단가": 86239, "비고": ""},
    {"종목코드": "069500", "종목명": "KODEX 200", "거래일자": "2026-03-04", "거래구분": "매도", "거래수량": 11, "거래단가": 78475, "비고": "미국의 이란 침공으로 주식 폭락, -10% 기준 손절"},
...
def 거래이력표준열맞추기(df):
    표준열 = ["종목코드", "종목명", "거래일자", "거래구분", "거래수량", "거래단가", "비고"]
    결과 = df.copy()
    for 열 in 표준열:
...
def 업로드파일에서거래이력읽기(업로드파일):
    ...
    for 인코딩 in ["utf-8-sig", "utf-8", "cp949", "euc-kr"]:
        try:
            읽기df = pd.read_csv(io.BytesIO(원본바이트), encoding=인코딩)
            return 거래이력표준열맞추기(읽기df)
...
def 포트폴리오계산(원본포트폴리오):
    ...
    계산표["데이터상태"] = 계산표["현재가"].apply(lambda 값: "정상" if pd.notna(값) and 값 > 0 else "현재가 조회 실패")
    계산표["평가금액"] = 계산표["현재가"].fillna(0) * 계산표["보유수량"]
...
st.caption("주요 지수와 대표 종목의 현재가·전일대비·등락률은 pykrx 최근 거래일 종가 기준으로 계산합니다.")
...
st.caption(
    "업로드한 거래이력 파일 내용을 기본 거래내역에 반영했습니다. 거래일자·거래구분·거래수량·거래단가를 입력하면 아래 포트폴리오 현황이 자동 집계됩니다. "
...
if "trade_history_df_v22" not in st.session_state:
    st.session_state["trade_history_df_v22"] = 거래이력자동보정(기본포트폴리오.copy())
...
불러오기칸1, 불러오기칸2 = st.columns([1, 1])
with 불러오기칸1:
    if st.button("업로드 파일 반영", use_container_width=True, disabled=업로드파일 is None):
        try:
            불러온df = 업로드파일에서거래이력읽기(업로드파일)
            st.session_state["trade_history_df_v22"] = 거래이력자동보정(불러온df.copy())
            st.success("거래이력을 불러왔습니다.")
            st.rerun()
        except Exception as e:
            st.error(f"불러오기 중 오류가 발생했습니다: {e}")
with 불러오기칸2:
    if st.button("기본 거래내역으로 초기화", use_container_width=True):
        st.session_state["trade_history_df_v22"] = 거래이력자동보정(기본포트폴리오.copy())
        st.success("기본 거래내역으로 초기화했습니다.")
        st.rerun()

st.info("거래이력 파일은 CSV, JSON, Excel(xlsx/xls) 형식을 불러올 수 있습니다. 거래구분은 매수/매도만 사용해 주세요.")

수정포트폴리오 = st.data_editor(
    st.session_state["trade_history_df_v22"].copy(),
    num_rows="dynamic",
    use_container_width=True,
    hide_index=True,
    disabled=[],
    column_order=["종목코드", "종목명", "거래일자", "거래구분", "거래수량", "거래단가", "비고"],
    column_config={
...
st.write("※ 주요 지수와 종목 차트는 pykrx 기반 과거 시세를 사용하고, 현재값은 네이버 금융 우선·pykrx 보조 방식으로 조회하도록 구성했습니다.")
