...
st.title("📈 투자 분석 시스템 v4.23 Final")
st.caption("시장 점검, 종목 분석, 거래 이력 기반 포트폴리오 집계, 저장/불러오기, 리밸런싱과 위험도 분석을 한 화면에서 확인하는 최종 정리본입니다.")
...
def 거래이력자동보정(df):
    ...
    보정["거래구분"] = 보정["거래구분"].astype(str).str.strip()
...
st.caption(
    ...
    "종목코드 또는 종목명 중 하나를 입력하면 다른 값은 가능한 범위에서 자동 보정됩니다. 거래단가는 숫자로 입력해 주세요."
)
