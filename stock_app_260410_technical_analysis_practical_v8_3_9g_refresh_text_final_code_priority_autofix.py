...
def 거래이력자동보정(df):
    ...
    def _행보정(행):
        ...
        코드 = "" if pd.isna(코드) else re.sub(r"[^0-9]", "", str(코드)).zfill(6) if re.sub(r"[^0-9]", "", str(코드)) else ""
        이름 = 종목명이름정리(이름)
...
def 실시간포함시세요약가져오기(구분, 코드, lookback_days=15):
    요약 = 최근시세요약가져오기(구분, 코드, lookback_days=lookback_days).copy()
    실시간값 = 실시간현재가가져오기(구분, 코드)
    if 실시간값 is not None and 실시간값 > 0:
        전일가 = 요약.get("전일가")
        전일대비 = None if 전일가 in [None, 0] else 실시간값 - 전일가
        등락률 = None if 전일가 in [None, 0] else (전일대비 / 전일가) * 100
        요약["현재가"] = float(실시간값)
        요약["전일대비"] = 전일대비
        요약["등락률"] = 등락률
        요약["상태"] = "실시간 현재가 반영"
        요약["기준일"] = datetime.today().date()
    return 요약
...
def 자산현재가정보(자산명, 자산정보):
    구분 = 자산정보["구분"]
    코드 = 자산정보["코드"]
    정보 = 실시간포함시세요약가져오기(구분, 코드, lookback_days=15)
    정보["자산명"] = 자산명
    return 정보
...
@st.cache_data(ttl=10)
def 종목현재가가져오기(종목코드):
    return 실시간포함시세요약가져오기("stock", 종목코드, lookback_days=15).get("현재가")


@st.cache_data(ttl=10)
def ETF현재가가져오기(종목코드):
    return 실시간포함시세요약가져오기("etf", 종목코드, lookback_days=15).get("현재가")


@st.cache_data(ttl=10)
def 인덱스현재가가져오기(지수코드):
    return 실시간포함시세요약가져오기("index", 지수코드, lookback_days=15).get("현재가")
...
def 대시보드스타일적용():
    ...
    .simple-market-title {{font-size: 0.90rem; font-weight: 800; color: {제목색}; margin-bottom: 6px; line-height: 1.2; min-height: 2.1em; word-break: keep-all;}}
    .simple-market-price {{font-size: 1.20rem; font-weight: 800; color: {제목색}; line-height: 1.1; letter-spacing: -0.02em;}}
    .simple-market-delta {{font-size: 0.80rem; font-weight: 700; margin-top: 6px; min-height: 1.25em;}}
    .simple-market-delta.up {{color: #dc2626;}}
    .simple-market-delta.down {{color: #2563eb;}}
    .simple-market-delta.flat {{color: {메타색};}}
    .simple-market-meta {{font-size: 0.74rem; color: {메타색}; margin-top: auto; padding-top: 7px;}}
...
모니터헤더칸1, 모니터헤더칸2 = st.columns([1.15, 8.85], gap="small")
with 모니터헤더칸1:
...
with 모니터헤더칸2:
    ...
    st.markdown(
        f"<div style='padding:12px 4px 0 14px; color:#94a3b8; font-size:0.98rem; font-weight:600; line-height:1.2;'>{조회일시문자}</div>",
        unsafe_allow_html=True,
    )
...
수정포트폴리오 = st.data_editor(
    ...
    column_config={
        "종목코드": st.column_config.TextColumn("종목코드", help="6자리 종목코드를 입력하면 종목명이 자동 보정되며, 코드·이름 불일치 시 종목코드 기준으로 자동 수정됩니다."),
