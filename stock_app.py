...
if not PYKRX_AVAILABLE:
    안내문구 = "pykrx 모듈을 현재 환경에서 사용할 수 없어 일부 국내 시세 기능은 제한됩니다."
    if PYKRX_IMPORT_ERROR:
        안내문구 += f" (상세: {PYKRX_IMPORT_ERROR})"
    st.warning(안내문구)
    ...
    for col, item in zip(cols, 요약데이터[row_start:row_start + 3]):
        with col:
            ...
            if not item["차트데이터"].empty:
                st.plotly_chart(미니차트그래프(item["차트데이터"], item["자산명"]), use_container_width=True)
...
if 선택데이터.empty:
    ...
    with 차트탭1:
        ...
        st.plotly_chart(가격그래프(선택데이터, f"{차트_선택자산} 6개월 라인 차트"), use_container_width=True, config={"displaylogo": False, "responsive": True})
        st.markdown("##### 신호 체크표")
    ...
    with 차트탭2:
        ...
        for 기간라벨, 기간탭 in zip(["일", "주", "월", "년"], 기간탭들):
            with 기간탭:
                ...
                st.plotly_chart(캔들그림, use_container_width=True, config={"displaylogo": False, "responsive": True})
                ...
                st.dataframe(index_1부터(캔들분석["체크표"]), use_container_width=True)
