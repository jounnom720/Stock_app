...
def 오늘의요약생성(계산포트폴리오, 리밸런싱표, 추가배분표, 총수익률, 추가투자금):
    요약문 = []

    if 총수익률 > 0:
        요약문.append(f"포트폴리오 전체 수익률은 현재 {총수익률:.2f}%로 수익 상태입니다.")
    elif 총수익률 < 0:
        요약문.append(f"포트폴리오 전체 수익률은 현재 {총수익률:.2f}%로 손실 상태입니다.")
    else:
        요약문.append("포트폴리오 전체 수익률은 현재 보합 수준입니다.")

    과대비중표 = 리밸런싱표[리밸런싱표["비중차이"] > 0].sort_values("비중차이", ascending=False)
    부족비중표 = 리밸런싱표[리밸런싱표["비중차이"] < 0].sort_values("비중차이")

    if not 과대비중표.empty:
        과대종목 = 과대비중표.iloc[0]
        요약문.append(
            f"{과대종목['종목명']} 비중이 목표보다 {과대종목['비중차이']:.2f}%p 높아 상대적으로 비중이 큰 상태입니다."
        )

    if not 부족비중표.empty:
        부족종목 = 부족비중표.iloc[0]
        요약문.append(
            f"{부족종목['종목명']} 비중은 목표보다 {abs(부족종목['비중차이']):.2f}%p 낮아 보완 우선순위가 높습니다."
        )

    if 추가투자금 > 0:
        추천표 = 추가배분표[추가배분표["추천매수수량"] > 0].sort_values("추천배정금액", ascending=False)
        if not 추천표.empty:
            상위종목 = 추천표.iloc[0]
            요약문.append(
                f"추가 투자금은 우선적으로 {상위종목['종목명']} 쪽에 배분하는 것이 목표 비중에 더 가까워지는 방향입니다."
            )
        else:
            요약문.append("현재 추가 투자금으로 1주 이상 매수 추천되는 종목은 뚜렷하지 않습니다.")
