...
APP_VERSION = "v5.8.2-share"  # 공유용 / 자동백업 + xlsx 백업 + 입력보정 핫픽스 반영본
...
기본포트폴리오 = pd.DataFrame(columns=[
    "종목코드", "종목명", "거래일자", "거래구분", "거래수량", "거래단가", "운용사", "비고"
])
...
def 보유종목상세해설생성(종목명, 가격데이터, 포트폴리오행=None):
    ...
    if 포트폴리오행 is not None and len(포트폴리오행) > 0:
        행 = 포트폴리오행.iloc[0]
        문장목록.append(
            f"사용자님 계좌 기준 현재 **보유수량 {숫자표시(행.get('보유수량'))}주**, 매입 평균단가는 **{금액표시(행.get('매입평균단가'))}**, 평가손익은 **{손익문자열(행.get('평가손익'))}원**입니다."
        )
