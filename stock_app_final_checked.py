...
MONEY_COLUMNS = [
    "거래단가", "평균매입가", "현재가", "평가금액", "미실현손익", "실현손익",
    "총매수금액", "총매도금액", "현재평가금액", "목표평가금액", "차이금액"
]

DEDUP_KEY_COLUMNS = ["거래일자", "거래구분", "종목코드", "종목명", "거래수량", "거래단가", "운용사"]
...
def deduplicate_transactions(df: pd.DataFrame):
    if df.empty:
        return df.copy(), 0

    work = clean_transactions(df.copy())
    before = len(work)

    key_df = work[DEDUP_KEY_COLUMNS].copy()
    key_df["거래일자"] = pd.to_datetime(key_df["거래일자"], errors="coerce").dt.strftime("%Y-%m-%d")
    key_df["거래수량"] = pd.to_numeric(key_df["거래수량"], errors="coerce").fillna(0).round(8)
    key_df["거래단가"] = pd.to_numeric(key_df["거래단가"], errors="coerce").fillna(0).round(8)

    keep_mask = ~key_df.duplicated(keep="first")
    deduped = work.loc[keep_mask].copy().reset_index(drop=True)
    deduped["입력순서"] = range(1, len(deduped) + 1)
    removed = before - len(deduped)
    return deduped, removed


def merge_transactions(existing_df: pd.DataFrame, new_df: pd.DataFrame):
    existing_clean, _ = deduplicate_transactions(existing_df if existing_df is not None else pd.DataFrame())
    new_clean, _ = deduplicate_transactions(new_df if new_df is not None else pd.DataFrame())

    if existing_clean.empty:
        result = new_clean.copy()
        result["입력순서"] = range(1, len(result) + 1)
        return result, 0

    merged = pd.concat([existing_clean, new_clean], ignore_index=True)
    deduped, removed = deduplicate_transactions(merged)
    return deduped, removed
...
def load_uploaded_file(uploaded_file) -> pd.DataFrame:
    ...
    deduped, _ = deduplicate_transactions(cleaned)
    return deduped
...
def save_transactions_json(df: pd.DataFrame, path: str = AUTO_SAVE_PATH):
    if df.empty:
        return
    save_df, _ = deduplicate_transactions(df)
...
def load_transactions_json(path: str = AUTO_SAVE_PATH) -> pd.DataFrame:
    ...
    try:
        ...
        deduped, _ = deduplicate_transactions(df)
        return deduped
    except Exception:
        return empty_df
...
def build_column_config(df: pd.DataFrame, pct_cols=None):
    ...
    for col in df.columns:
        ...
        elif col in MONEY_COLUMNS and pd.api.types.is_numeric_dtype(df[col]):
...
def show_table(df: pd.DataFrame, height: int | None = None, pct_cols=None):
    pct_cols = pct_cols or []
    effective_height = height if isinstance(height, int) and height > 0 else "content"
    st.dataframe(
        df,
...
if "transactions" not in st.session_state:
    loaded = load_transactions_json()
    st.session_state.transactions = loaded
    st.session_state.last_removed_duplicates = 0
...
with st.sidebar:
    ...
    if st.button("현재 거래내역 저장", use_container_width=True):
        st.session_state.transactions, removed_now = deduplicate_transactions(st.session_state.transactions)
        save_transactions_json(st.session_state.transactions)
        if removed_now > 0:
            st.success(f"저장 완료 · 중복 {removed_now:,}건 정리")
        else:
            st.success("저장되었습니다.")

    if st.button("거래이력 중복 정리", use_container_width=True):
        st.session_state.transactions, removed_now = deduplicate_transactions(st.session_state.transactions)
        save_transactions_json(st.session_state.transactions)
        if removed_now > 0:
            st.success(f"중복 {removed_now:,}건을 정리했습니다.")
        else:
            st.info("중복 거래가 없습니다.")

    if st.button("거래이력 전체 초기화", use_container_width=True):
        st.session_state.transactions = pd.DataFrame(
            columns=["거래일자", "거래구분", "종목코드", "종목명", "거래수량", "거래단가", "운용사", "입력순서"]
        )
        st.session_state.last_removed_duplicates = 0
        if os.path.exists(AUTO_SAVE_PATH):
            try:
                ...
                pass
        st.success("현재 거래이력과 자동저장 파일을 초기화했습니다.")

    if not st.session_state.transactions.empty:
        export_json_df = st.session_state.transactions.copy()
        export_json_df["거래일자"] = pd.to_datetime(export_json_df["거래일자"]).dt.strftime("%Y-%m-%d")
        st.download_button(
            "거래이력 JSON 백업",
...
st.markdown(
    """
    <h1 style='margin-bottom:0.15rem;'>📊 개인 투자 포트폴리오</h1>
    <p style='margin-top:0; margin-bottom:0.9rem; font-size:1rem; color:#ff6b6b; font-weight:700;'>
        개발자 : 조현웅
        <a href='mailto:hwcho@me.com' style='color:#33c3ff; text-decoration:none; font-weight:800; margin-left:6px;'>
            hwcho@me.com
        </a>
    </p>
    """,
...
with st.container(border=True):
    ...
    with top2:
        upload_mode = st.radio(
            "업로드 방식",
            ["기존 데이터 교체", "기존 데이터에 추가"],
            horizontal=False,
            label_visibility="collapsed",
        )
        st.caption("교체: 현재 거래이력을 새 파일 기준으로 덮어쓰기 / 추가: 정확히 같은 거래는 제외")
        if st.button("업로드 파일 반영", use_container_width=True, type="primary"):
            if uploaded_file is None:
                ...
                try:
                    ...
                    if upload_mode == "기존 데이터 교체":
                        st.session_state.transactions, removed_now = deduplicate_transactions(upload_df)
                    else:
                        st.session_state.transactions, removed_now = merge_transactions(
                            st.session_state.transactions, upload_df
                        )
                    st.session_state.last_removed_duplicates = removed_now
                    save_transactions_json(st.session_state.transactions)

                    if removed_now > 0:
                        st.success(f"{len(upload_df):,}건 처리 완료 · 중복 {removed_now:,}건 자동 제외")
                    else:
        ...
        with c8:
            ...
            if st.button("직접 입력 추가", use_container_width=True):
                if input_qty <= 0 or input_price < 0 or (not input_code and not input_name):
                    ...
                    save_transactions_json(st.session_state.transactions)
                    if removed_now > 0:
                        st.warning("동일한 거래가 이미 있어 중복 추가를 막았습니다.")
                    else:
...
st.markdown("---")
st.subheader("🧾 거래 이력")
st.caption("반복 업로드로 생긴 정확히 같은 거래는 자동으로 한 번만 남기도록 정리합니다.")
if st.session_state.transactions.empty:
    st.info("거래 파일을 업로드하거나 직접 입력해 주세요.")
    st.stop()

# 화면 표시 직전에도 한 번 더 정리
st.session_state.transactions, removed_refresh = deduplicate_transactions(st.session_state.transactions)
if removed_refresh > 0:
    save_transactions_json(st.session_state.transactions)
    st.info(f"중복 거래 {removed_refresh:,}건을 자동 정리했습니다.")

view_tx = st.session_state.transactions.copy()
view_tx["거래일자"] = pd.to_datetime(view_tx["거래일자"]).dt.strftime("%Y-%m-%d")
if not show_all_history:
    view_tx = view_tx.tail(20)

view_tx_display = make_display_df(view_tx, int_like_cols=["거래수량", "거래단가", "입력순서"])
...
selected_trades["거래일자"] = pd.to_datetime(selected_trades["거래일자"]).dt.strftime("%Y-%m-%d")
...
selected_trades_display = make_display_df(selected_trades, int_like_cols=["거래수량", "거래단가", "입력순서"])
