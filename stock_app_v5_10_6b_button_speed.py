...
APP_VERSION = "v5.10.6b"  # 버튼형 화면 선택 + 사이드바/백업 지연 로딩
...
def 시세스냅샷캐시(거래이력json문자열, refresh_token=0):
    ...
    결과 = {}
    for 구분, 코드, 이름 in 고유자산:
        try:
            정보 = 실시간포함시세요약가져오기(구분, 코드, lookback_days=15, refresh_token=refresh_token).copy()
            정보["자산명"] = 이름
            결과[f"{구분}:{str(코드)}"] = 정보
        except Exception:
            결과[f"{구분}:{str(코드)}"] = {
...
def 종목별누적성적표UI(거래df, 계산포트폴리오):
    ...
    with st.expander("v5.10 종목별 누적 성적표", expanded=True):
        ...
        손실1위 = 성적표.iloc[성적표["총손익"].idxmin()] if not 성적표.empty else None
        ...
        카드2.metric("손실 1위", 손실1위["종목명"] if 손실1위 is not None else "-", 손익문자열(손실1위["총손익"]) if 손실1위 is not None else "")
        ...
        st.caption("총손익은 실현손익과 현재 보유분 평가손익을 합산한 값입니다. 세금·수수료·배당금은 반영하지 않습니다.")
...
def 모니터카드하단메모생성(정보):
    if 정보 is None:
        return ""
    줄목록 = []
    기준시각 = 정보.get("기준시각") or 정보.get("조회시각")
    출처 = 정보.get("출처", "-")
    상태 = 정보.get("상태", "")
    첫줄조각 = []
    if 출처 not in [None, "", "-"]:
        첫줄조각.append(f"출처 {출처}")
    if 기준시각 not in [None, "", "-"]:
        첫줄조각.append(f"기준 {카드기준시각문자열(기준시각)}")
    if 첫줄조각:
        줄목록.append(" · ".join(첫줄조각))
    if 상태 and 상태 not in ["최근 종가 반영", "장중 실시간호가 반영(Yahoo)", "국내 현재가 반영(Naver)", "준실시간 1분봉 반영"]:
        줄목록.append(f"상태 {상태}")
    return "\n".join([x for x in 줄목록 if x])
...
def 시장지표카드하단메모생성(행):
    if 행 is None:
        return ""
    출처 = 행.get("출처", "-")
    기준시각 = 행.get("기준시각") or 행.get("조회시각")
    상태 = 행.get("상태", "")
    구분 = 행.get("지표구분", "준실시간")
    줄1 = f"출처 {출처} · 기준 {카드기준시각문자열(기준시각)}"
    줄2 = f"구분 {구분}"
    if 상태:
        return f"{줄1}\n{줄2}\n상태 {상태}"
    return f"{줄1}\n{줄2}"
...
def 대시보드보유정보사전(거래df):
    try:
        계산결과 = 포트폴리오계산캐시(거래이력서명생성(거래df), refresh_token=st.session_state.get("price_refresh_token_v51", 0))
...
def 주요모니터자산구성(거래df):
    ...
    선호순서 = ["069500", "471990", "005930", "000660", "229200", "102110"]
    우선순위맵 = {코드: idx for idx, 코드 in enumerate(선호순서)}

    고유보유코드 = []
    seen = set()
    for 코드 in 보유코드목록:
        코드 = str(코드).zfill(6)
        if 코드 and 코드 not in seen:
            고유보유코드.append(코드)
            seen.add(코드)

    정렬보유코드 = sorted(
        고유보유코드,
        key=lambda x: (우선순위맵.get(x, 999), 고유보유코드.index(x))
    )
...
st.markdown("---")
st.markdown("### 화면 구성")
st.caption("속도 개선을 위해 선택한 화면만 계산합니다. 다른 섹터는 선택할 때 로딩됩니다.")

섹터목록 = ["① 주요 모니터링", "② 포트폴리오 현황", "③ 분석 / 인사이트"]
if "main_section_selector_v5106b" not in st.session_state:
    st.session_state["main_section_selector_v5106b"] = "① 주요 모니터링"

선택섹터 = st.session_state["main_section_selector_v5106b"]
...
for idx, 섹터명 in enumerate(섹터목록):
    with 버튼칸[idx]:
        if st.button(
            섹터명,
            key=f"main_section_btn_v5106b_{idx}",
            use_container_width=True,
            type="primary" if 선택섹터 == 섹터명 else "secondary",
        ):
            st.session_state["main_section_selector_v5106b"] = 섹터명
            st.rerun()

선택섹터 = st.session_state["main_section_selector_v5106b"]
st.caption(f"현재 화면: {선택섹터}")
...
if 선택섹터 == "① 주요 모니터링":
    ...
    st.markdown(
        '<div class="top-monitor-sub">코스피·코스닥과 현재 보유 종목을 카드형으로 정리했습니다. 왼쪽 실선과 수치 색상은 상승은 빨간색, 하락은 파란색으로 표시합니다.</div>',
        unsafe_allow_html=True,
    )
    ...
    with 모니터헤더칸2:
        조회일시문자 = 서울조회문자열(st.session_state["manual_price_refresh_ts_v1"], 포맷="조회 %Y-%m-%d %H:%M · 비교 전일 종가 기준")
        st.markdown(
    ...
    if 모바일여부():
            ...
            for col, (자산명, 자산정보, 구분라벨) in zip(cols, 현재행):
                with col:
                    if 자산명 == "__ADD__":
                        ...
                        정보 = 자산현재가정보(자산명, 자산정보, refresh_token=st.session_state.get("price_refresh_token_v51", 0))
        ...
        첫째줄우선순서 = ["코스피", "코스닥", "KODEX 200", "KODEX AI반도체핵심장비"]
        둘째줄우선순서 = ["삼성전자", "SK하이닉스"]

        자산맵 = {자산명: (자산명, 자산정보, 구분라벨) for 자산명, 자산정보, 구분라벨 in 모니터자산목록}
        첫째줄 = [자산맵[이름] for 이름 in 첫째줄우선순서 if 이름 in 자산맵]
        사용이름 = {x[0] for x in 첫째줄}
        둘째줄 = [자산맵[이름] for 이름 in 둘째줄우선순서 if 이름 in 자산맵 and 이름 not in 사용이름]
        사용이름 |= {x[0] for x in 둘째줄}
        나머지 = [항목 for 항목 in 모니터자산목록 if 항목[0] not in 사용이름]
        ...
        for 행인덱스, 현재행 in enumerate(행목록):
            ...
            for idx in range(4):
                with cols[idx]:
                    ...
                    elif 행인덱스 == 1 and idx == len(현재행):
                        if 모니터추가카드버튼(key="monitor_add_card_desktop_v53"):
                            새상태 = not st.session_state.get("show_monitor_add_form_v53", False)
                            st.session_state["show_monitor_add_form_v53"] = 새상태
                            if 새상태:
                                st.session_state["monitor_add_select_v53"] = ""
                                st.session_state["monitor_add_code_v53"] = ""
                            st.rerun()
    ...
    if 보유코드목록:
        ...
        관심문자 = ", ".join([종목코드기준종목명(c) or 코드명매핑.get(c) or c for c in 세션모니터관심종목가져오기()])
        if 관심문자:
            st.caption(f"자동 연결된 보유 종목: {보유종목명문자} · 추가 관심종목: {관심문자}")
        else:
            st.caption(f"자동 연결된 보유 종목: {보유종목명문자}")
    else:
        관심문자 = ", ".join([종목코드기준종목명(c) or 코드명매핑.get(c) or c for c in 세션모니터관심종목가져오기()])
        if 관심문자:
            st.caption(f"현재 보유 종목이 없어 코스피와 코스닥 및 관심종목만 표시합니다. 추가 관심종목: {관심문자}")
        else:
    ...
    if 모니터실패건수 > 0:
        st.warning(f"상단 모니터에서 시세를 불러오지 못한 자산이 {모니터실패건수}개 있습니다. 카드 하단의 기준일·조회 실패 문구를 확인해 주세요.")
    else:
        st.caption("상단 모니터의 변화값은 전일 종가 대비이며, 카드 하단에 출처·기준시각·비교기준을 함께 표시합니다.")
    ...
    st.markdown("---")
    st.subheader("주요 지표")
    st.caption("환율·금·에너지·금리·변동성 지표를 카드형으로 정리했습니다. USD/KRW·국제 금·WTI·브렌트유·미국 10년물 금리·VIX를 함께 확인해 주세요.")
...
with st.sidebar.expander("거래이력 관리", expanded=False):
    ...
    st.caption("업로드·저장·백업만 이곳에서 관리합니다. 본문 화면은 모니터와 분석 결과 중심으로 유지합니다.")
