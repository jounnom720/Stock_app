...
APP_VERSION = "v5.13.6-share"  # 공개용 Share 버전 / 샘플 엑셀 강제 초기 연결 / 중복 코멘트 개선 UI
SHARE_MODE = True
...
if 모바일여부():
    st.title(f"📈 투자 분석 시스템 (공개용) {APP_VERSION}")
    st.caption("모바일 조회용 간소화 화면 · 샘플 통합자산관리 데이터 내장")
else:
    st.title(f"📈 투자 분석 시스템 (공개용) {APP_VERSION}")
    st.caption("공개용 Share 버전 · 개인정보 제거 · 샘플 거래이력 및 비주식자산 데이터 내장")
...
def IRP비주식자산불러오기():
    ...
    df = 공개샘플비주식자산불러오기() if SHARE_MODE else 기본IRP비주식자산표()
    st.session_state["irp_non_stock_assets_df_v512"] = df
    return df
...
with st.sidebar.expander("거래이력 관리", expanded=False):
    ...
    st.caption("공개용 기본 데이터는 내장된 샘플 통합자산관리 엑셀입니다. 필요 시 별도 파일도 업로드할 수 있습니다.")
...
st.markdown("---")
st.markdown('공개용 Share 버전 · 샘플 데이터 기반 · 개발자 조현웅 <a href="mailto:hwcho@me.com">hwcho@me.com</a>', unsafe_allow_html=True)
