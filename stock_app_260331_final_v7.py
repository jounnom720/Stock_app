...
if 모바일여부():
    ...
    with 목표행2_1:
        목표_AI반도체 = st.number_input("KODEX AI반도체핵심장비 목표 비중(%)", 0.0, 100.0, float(저장된목표비중["471990"]), 1.0)
    with 목표행2_2:
        목표_삼성전자 = st.number_input("삼성전자 목표 비중(%)", 0.0, 100.0, float(저장된목표비중["005930"]), 1.0)

    목표행3_1, 목표행3_2 = st.columns(2)
    with 목표행3_1:
        목표_SK하이닉스 = st.number_input("SK하이닉스 목표 비중(%)", 0.0, 100.0, float(저장된목표비중["000660"]), 1.0)
    with 목표행3_2:
        st.caption("현재 보유/관심 종목에 맞춰 목표비중을 설정합니다.")
else:
    목표칸1, 목표칸2, 목표칸3, 목표칸4, 목표칸5 = st.columns(5)
...
목표비중합계 = 목표_KODEX200 + 목표_KODEX코스닥150 + 목표_AI반도체 + 목표_삼성전자 + 목표_SK하이닉스
목표비중사전 = {
    "069500": 목표_KODEX200,
...
if abs(목표비중합계 - 100.0) > 0.001:
    ...
    if 모바일여부():
        ...
        if "현재비중" in 리밸런싱표시.columns:
            모바일형식사전["현재비중"] = lambda x: 안전소수포맷(x, 2)
        if "목표비중" in 리밸런싱표시.columns:
            모바일형식사전["목표비중"] = lambda x: 안전소수포맷(x, 2)
        ...
        st.dataframe(
            리밸런싱표시.style.format({
                "현재가": 안전정수포맷,
                "현재비중": lambda x: 안전소수포맷(x, 2),
                "목표비중": lambda x: 안전소수포맷(x, 2),
                "비중차이": 수익률문자열,
                "평가금액": 안전정수포맷,
                "목표평가금액": 안전정수포맷,
                "리밸런싱금액": 손익문자열,
                "정확계산수량": lambda x: 안전소수포맷(x, 2),
                "주문참고수량": 안전정수포맷,
    ...
    if not 보유종목옵션:
        ...
        if 가격데이터.empty:
            ...
            with 보유분석탭1:
                ...
                st.dataframe(index_1부터(개요체크표), width="stretch")
