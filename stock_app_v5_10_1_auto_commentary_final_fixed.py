...
APP_VERSION = "v5.10.1"  # 종목별 누적 성적표 추가
...
def 종목별자동코멘트생성(성적표):
    ...
    for _, 행 in 성적표.iterrows():
        ...
        문장 = []
        if 등급 in ["우수", "양호"]:
            문장.append("누적 성과가 비교적 양호한 편입니다")
        elif 등급 == "중립":
            문장.append("현재까지는 뚜렷한 우위나 위험 신호가 크지 않은 중립 구간입니다")
        elif 등급 == "주의":
            문장.append("손익 흐름이 약해 신중한 관찰이 필요합니다")
        else:
            문장.append("반복 손실 또는 성과 부진 가능성이 있어 매매 원인 점검이 필요합니다")

        if 구분 == "ETF":
            문장.append("ETF 특성상 단기 판단보다 분할 접근과 비중 관리 중심으로 보는 것이 적절합니다")
        else:
            문장.append("개별주는 업황·실적·대외 변수 영향을 크게 받을 수 있어 비중 관리가 중요합니다")

        if 매도거래수 >= 3:
            if 승률 >= 60:
                문장.append(f"매도 기준 승률은 {승률:.0f}%로 양호합니다")
            elif 승률 < 40:
                문장.append(f"매도 기준 승률은 {승률:.0f}%로 낮아 진입·청산 기준을 다시 확인할 필요가 있습니다")
        else:
            문장.append("매도 사례가 많지 않아 승률 평가는 참고 수준으로 보는 것이 좋습니다")

        if 보유수량 > 0:
            if 평가손익 > 0:
                문장.append("현재 보유분은 평가이익 상태입니다")
            elif 평가손익 < 0:
                문장.append("현재 보유분은 평가손실 상태이므로 추가 매수보다 손실 원인 확인이 우선입니다")

        if 실현손익 > 0 and 평가손익 < 0:
            문장.append("확정 수익은 있으나 보유분 손실이 있어 최근 흐름 점검이 필요합니다")
        elif 실현손익 < 0 and 평가손익 > 0:
            문장.append("과거 확정 손실 이후 보유분은 회복 흐름을 보이고 있습니다")
        ...
        결과.append({
            ...
            "총수익률": 총수익률,
            "승률": 승률,
            "자동 코멘트": " ".join(문장) + " 단, 이는 매수·매도 지시가 아니라 거래기록 기반 참고 의견입니다.",
        })
...
def 포트폴리오자동진단문장(성적표):
    ...
    작업 = 성적표.copy()
    for 열 in ["총매수금액", "총손익", "총수익률", "보유수량"]:
        if 열 not in 작업.columns:
            작업[열] = 0
        작업[열] = pd.to_numeric(작업[열], errors="coerce").fillna(0)

    작업["구분"] = 작업.apply(lambda 행: 종목분류라벨(행.get("종목명", ""), 행.get("종목코드", "")), axis=1)
    총손익 = 작업["총손익"].sum()
    총매수 = 작업["총매수금액"].sum()
    전체수익률 = (총손익 / 총매수 * 100) if 총매수 > 0 else 0
    보유종목수 = int((작업["보유수량"] > 0).sum())
    etf개수 = int((작업["구분"] == "ETF").sum())
    개별주개수 = int((작업["구분"] == "개별주").sum())

    문장 = []
    문장.append(f"전체 누적 기준 총손익은 {손익문자열(총손익)}, 총수익률은 {수익률문자열(전체수익률)}입니다.")

    if etf개수 >= 개별주개수:
        문장.append("현재 거래 구성은 ETF 비중이 비교적 높아 분산성과 안정성 측면의 장점이 있습니다.")
    else:
        문장.append("현재 거래 구성은 개별주 비중이 상대적으로 높아 업종·종목별 변동성 관리가 중요합니다.")

    if 보유종목수 >= 5:
        문장.append(f"현재 보유 종목은 {보유종목수}개로 분산 효과는 있으나, 관리 종목 수가 늘어날 수 있습니다.")
    elif 보유종목수 > 0:
        문장.append(f"현재 보유 종목은 {보유종목수}개입니다. 특정 종목 집중도가 높지 않은지 함께 확인하는 것이 좋습니다.")
    else:
        문장.append("현재 보유 종목이 없거나 보유수량 정보가 부족합니다.")

    손실종목 = 작업[작업["총손익"] < 0]
    if not 손실종목.empty:
        문장.append(f"총손익이 음수인 종목이 {len(손실종목)}개 있습니다. 추가 매수 전 손실 원인과 최초 매수 근거를 다시 확인하는 방식이 안전합니다.")
    else:
        문장.append("현재 집계상 총손익 음수 종목은 크지 않아 보입니다.")

    문장.append("이 진단은 거래기록과 현재 평가자료 기반의 보수적 참고 의견이며, 세계정세·금리·환율·기업 실적 변화는 별도 확인이 필요합니다.")
    return 문장
...
def 자동분석코멘트UI(성적표):
    ...
    with st.expander("v5.10.1 자동 분석 코멘트", expanded=True):
        st.caption("매수·매도 지시가 아니라, 거래기록을 바탕으로 실수를 줄이기 위한 보수적 참고 의견입니다.")

        진단문장 = 포트폴리오자동진단문장(성적표)
        for 문장 in 진단문장:
            st.write(f"- {문장}")
        ...
        보기열 = ["종목명", "구분", "점수", "판단등급", "총손익", "총수익률", "승률", "자동 코멘트"]
        표시 = 코멘트표[보기열].copy()
        표데이터프레임(
        ...
        if st.button("업로드 파일 반영", disabled=업로드파일 is None, key="apply_upload_btn_v26", use_container_width=True):
            try:
                ...
                if 변경됨:
                    ...
                    st.info("업로드 내용이 현재 거래이력과 동일하여 다시 계산하지 않았습니다.")
                if not 저장성공:
