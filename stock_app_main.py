...
# [2026-08-21] '거래이력' 컬럼 순서를 Jone 개인 시트(asset_management)의 실제 순서에 맞춤.
# 기존 순서(거래일자, 운용사, 종목코드, 종목명, ...)로 만들어진 신규 유저 시트와 관리자 본인 시트의
# 컬럼 순서가 서로 달라 혼동이 있었음 — 앱은 컬럼을 이름 기준으로 읽으므로 기능상 영향은 없었지만,
# 앞으로 생성되는 신규 유저 시트를 관리자 시트와 동일한 모양으로 통일한다.
