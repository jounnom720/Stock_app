...
ASSET_MASTER_V518 = {
    ...
    "005380": {"name": "현대차", "kind": "주식", "industry": "자동차", "aliases": ["005380", "현대차", "현대자동차", "HYUNDAI MOTOR"]},
...
ASSET_MASTER_V51715 = {
    ...
    "005380": {
        ...
        "주산업": "자동차",
        "보조태그": ["자동차", "수출", "국내대형주"],
        "aliases": ["005380", "현대차", "현대자동차", "HYUNDAI MOTOR"],
    },
    "278470": {
        "표시명": "에이피알",
        "정규명": "에이피알",
...
APP_VERSION = "v5.21.0-asset-change-engine"
...
# v5.21.0 자산변화 해석 엔진
# 목적:
# - 예수금 감소를 무조건 외부 인출로 보지 않는다.
# - 같은 시점의 주식/ETF 매수·매도 거래를 함께 확인해 내부 자산 이동으로 해석한다.
# - 자산변화로그 P~S열(원금변화사유/확인금액/설명/자동분석)을 기존 구조 그대로 활용한다.
# ============================================================
def _v521_금액숫자(value, default=0.0):
    try:
        if value is None:
            return default
...
def _v521_자산종류문구(종목명="", 종목코드=""):
    이름 = str(종목명 or "").upper().replace(" ", "")
    코드 = str(종목코드 or "").upper().replace(" ", "")
    if any(k in 이름 for k in ["ETF", "KODEX", "TIGER", "ACE", "KBSTAR", "SOL", "HANARO", "ARIRANG", "PLUS"]):
        return "ETF"
    if 코드 in ASSET_MASTER_V518:
        kind = ASSET_MASTER_V518.get(코드, {}).get("kind", "")
        if kind:
            return kind
    return "주식"


def _v521_현금출처문구(계좌="", 종목명="", 거래구분=""):
    계좌문자 = str(계좌 or "")
    종목문자 = str(종목명 or "")
    if "IRP" in 계좌문자.upper() or "현금성 대기" in 계좌문자 or "대기자산" in 계좌문자:
        return "현금성 대기자산"
    if "CMA" in 계좌문자.upper():
        return "CMA"
    return "예수금"


def _v521_최근거래후보(logdf=None, max_rows=5):
    """Google Sheets 거래이력에서 최근 매수/매도 거래 후보를 찾는다."""
    try:
        try:
            거래원본 = 구글시트데이터프레임읽기(GOOGLE_SHEETS_TRADE_SHEET)
        except Exception:
            거래원본 = st.session_state.get("trade_history_edit_df_v1", pd.DataFrame())
        거래 = 거래이력표준열맞추기(거래원본) if "거래이력표준열맞추기" in globals() else pd.DataFrame(거래원본).copy()
        if 거래 is None or 거래.empty:
            return []
        거래 = 거래.copy()
        거래["거래일자_dt_v521"] = 거래["거래일자"].apply(_v521_거래일자값) if "거래일자" in 거래.columns else pd.NaT
        거래["거래구분_v521"] = 거래.get("거래구분", "").astype(str).str.strip()
        거래 = 거래[거래["거래구분_v521"].str.contains("매수|매도", na=False)].copy()
        if 거래.empty:
            return []
        거래["거래금액_v521"] = 거래.apply(lambda r: abs(_v521_금액숫자(r.get("거래수량", 0)) * _v521_금액숫자(r.get("거래단가", 0))), axis=1)
        거래 = 거래[거래["거래금액_v521"] > 0].copy()
        if 거래.empty:
            return []

        # 직전 자산변화로그 기준일 이후 또는 최근 7일 이내 거래를 우선 후보로 사용한다.
        기준일 = None
        try:
            로그 = 자산변화로그표준화(logdf)
            if not 로그.empty and "기준일" in 로그.columns:
                기준일 = pd.to_datetime(로그.iloc[-1].get("기준일", ""), errors="coerce")
        except Exception:
            기준일 = None
        if 기준일 is not None and not pd.isna(기준일):
            후보 = 거래[거래["거래일자_dt_v521"] >= 기준일].copy()
        else:
            후보 = pd.DataFrame()
        if 후보.empty:
            최신일 = 거래["거래일자_dt_v521"].max()
            if 최신일 is not None and not pd.isna(최신일):
                후보 = 거래[거래["거래일자_dt_v521"] == 최신일].copy()
            else:
                후보 = 거래.tail(max_rows).copy()
        후보 = 후보.sort_values(["거래일자_dt_v521", "거래금액_v521"], ascending=[False, False]).head(max_rows)

        결과 = []
        for _, r in 후보.iterrows():
            구분 = str(r.get("거래구분", "")).strip()
            종목명 = asset_name_v518(r.get("종목코드", ""), r.get("종목명", "")) if "asset_name_v518" in globals() else str(r.get("종목명", ""))
            종목코드 = normalize_asset_code_v518(r.get("종목코드", ""), 종목명) if "normalize_asset_code_v518" in globals() else str(r.get("종목코드", ""))
            자산종류 = _v521_자산종류문구(종목명, 종목코드)
            계좌 = str(r.get("운용사", "") or r.get("계좌", "") or "").strip()
            현금문구 = _v521_현금출처문구(계좌, 종목명, 구분)
            금액 = float(r.get("거래금액_v521", 0) or 0)
            if "매수" in 구분:
                설명 = f"{현금문구} → {종목명} {자산종류} 매수"
                유형 = "자산 이동"
            elif "매도" in 구분:
                설명 = f"{종목명} {자산종류} 매도 → {현금문구}"
                유형 = "자산 이동"
            else:
                continue
            결과.append({
        ...
        logging.warning("v5.21 최근 거래 후보 추정 오류: %s", e, exc_info=True)
        return []


def 자산변화자산이동자동추정_v521(미리보기행=None, 로그df=None):
    """최근 거래이력을 바탕으로 자산변화로그에 저장할 자산이동 설명을 만든다."""
    try:
        후보 = _v521_최근거래후보(logdf=로그df, max_rows=5)
        if not 후보:
            return None
        # 여러 건이면 금액 큰 순서로 대표 후보를 만들고, 나머지는 보조 설명에 포함한다.
        후보 = sorted(후보, key=lambda x: abs(float(x.get("금액", 0) or 0)), reverse=True)
        대표 = 후보[0]
        총금액 = sum(float(x.get("금액", 0) or 0) for x in 후보)
        if len(후보) == 1:
            설명 = 대표["설명"]
        else:
            세부 = "; ".join([f"{x['설명']} {원화정수포맷(x.get('금액', 0))}" for x in 후보[:3]])
            설명 = f"최근 거래 {len(후보)}건 자산 이동: {세부}"
        return {
            "원금변화사유": "자산 이동",
            "원금변화확인금액": round(총금액 if len(후보) > 1 else 대표.get("금액", 0)),
            "원금변화설명": 설명,
            "후보건수": len(후보),
            "대표": 대표,
        }
    except Exception as e:
        logging.warning("v5.21 자산이동 자동 추정 오류: %s", e, exc_info=True)
        return None


def 자산변화로그사유자동보정_v521(새행, 기존로그=None, 사용자사유="", 사용자금액=0, 사용자설명=""):
    """저장 직전 P~S열을 보정한다. 최근 매수/매도 거래가 있으면 내부 자산 이동을 우선 반영한다."""
    행 = dict(새행 or {})
    사용자사유 = str(사용자사유 or "").strip()
    사용자설명 = str(사용자설명 or "").strip()
    사용자금액 = _v521_금액숫자(사용자금액, 0)
    자동 = 자산변화자산이동자동추정_v521(행, 기존로그)

    # 사용자가 명시적으로 원금증가/원금감소를 선택하고 설명도 쓴 경우는 존중한다.
    명시사용자입력 = bool(사용자설명 and 사용자사유 in ["외부 입금", "원금증가", "외부 인출", "원금감소"])
    if 자동 and not 명시사용자입력:
        행["원금변화사유"] = 자동.get("원금변화사유", "자산 이동")
        행["원금변화확인금액"] = round(float(자동.get("원금변화확인금액", 0) or 0))
        행["원금변화설명"] = 자동.get("원금변화설명", "")
        원금변화 = float(행.get("원금변화", 0) or 0)
        if abs(원금변화) <= 1000:
            행["변화유형"] = "자산이동"
            행["자동분석"] = (
                f"최근 거래이력을 기준으로 {행['원금변화설명']} 내역이 확인됩니다. "
                "이는 외부 입금이나 생활비 인출이 아니라 현금성자산이 주식/ETF로 바뀐 내부 자산 이동입니다. "
                "통합원금 변화는 없습니다."
            )
        else:
            행["변화유형"] = "자산이동+잔액조정 확인"
            행["자동분석"] = (
                f"최근 거래이력을 기준으로 {행['원금변화설명']} 내역이 확인됩니다. "
                f"다만 직전 저장 대비 총원금 변화가 {손익원화문자열(원금변화)} 남아 있으므로, "
                "현금성자산 잔액 수정·비주식자산 중복 제거·과거 스냅샷 기준 차이를 함께 확인해 주세요."
            )
        return 행

    # 자동 추정이 없으면 사용자 입력을 그대로 반영한다.
    행["원금변화사유"] = 사용자사유
    행["원금변화확인금액"] = round(float(사용자금액 or 0))
    행["원금변화설명"] = 사용자설명
    return 행
...
def 자산변화상위요인문장(계좌요약=None, 자산군요약=None, 미리보기행=None):
    ...
    try:
        ...
        if abs(원금변화) >= 1:
            ...
            문장.append(f"직전 저장 대비 원금은 {손익원화문자열(원금변화)} {방향}했습니다. 단, 최근 매수·매도 거래가 있으면 외부 입출금이 아니라 현금성자산과 주식/ETF 사이의 자산 이동일 수 있습니다.")
        elif abs(평가액변화) >= 1:
...
def 자산변화원금사유자동추정(미리보기행=None, 로그df=None, 현재스냅샷=None, 계좌요약=None, 자산군요약=None):
    ...
    try:
        ...
        평가손익변화 = float(미리보기행.get("평가손익변화", 0) or 0)
        if abs(원금변화) < 1:
            자동이동 = 자산변화자산이동자동추정_v521(미리보기행, 로그df) if "자산변화자산이동자동추정_v521" in globals() else None
            if 자동이동:
                return {
                    "추천사유": "자산 이동",
                    "신뢰도": "높음",
                    "핵심해석": f"{자동이동.get('원금변화설명', '')} 내역이 확인됩니다. 통합원금 변화 없이 현금성자산이 주식/ETF로 이동한 거래입니다.",
                    "확인사항": "거래이력의 매수금액과 비주식자산의 예수금 감소액이 같은지 확인하면 됩니다.",
                }
            return {
                "추천사유": "원금 변화 없음",
                "신뢰도": "높음",
...
def 자산변화사유입력UI(미리보기행):
    ...
    기본사유 = "자산 이동" if "자산 이동" in 추천사유 else "자산 재분류"
    if "입금" in 추천사유 or 원금변화 > 0:
        기본사유 = "외부 입금"
    elif "인출" in 추천사유 or 원금변화 < 0:
        기본사유 = "외부 인출"
    elif "내부" in 추천사유 or "자산 이동" in 추천사유:
        기본사유 = "자산 이동"
    ...
    기본설명 = ""
    if 원금변화사유 == "자산 이동":
        자동이동 = 자산변화자산이동자동추정_v521(미리보기행, 자산변화로그읽기()) if "자산변화자산이동자동추정_v521" in globals() else None
        기본설명 = 자동이동.get("원금변화설명", "현금성자산 → 주식/ETF 매수") if 자동이동 else "현금성자산 → 주식/ETF 매수"
    elif 원금변화사유 == "현금→ETF/주식 매수":
