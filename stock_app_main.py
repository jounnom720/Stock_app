...
#
# 수정 내용
# - 계좌별 비주식·현금성 자산 확인표를 Editor 반환값 기준으로 고정
# - Streamlit data_editor key를 기존 안정 key로 복구하여 오늘 수정값 회귀 방지
# - 자산 변동 추이 UI 자체는 보존하고 내부 중복 Render Path 호출만 삭제
# - 최근 자산변화/자산 변동 추이의 기존 증권사 앱 스타일 UI는 건드리지 않음
# - 거래건수 계산·거래이력·회계 엔진은 수정하지 않음
#
# 영향 범위
# - UI / Render Path / Editor 표시 기준
#
# 회계 엔진 영향
# - 없음
# ======================================================
...
def IRP비주식자산편집UI():
    ...
    주의:
    - 회계 엔진, 원금 계산, 실현손익 계산에는 관여하지 않습니다.
    - data_editor key는 기존 안정 key를 유지하여 오늘 입력 중인 값이 과거값으로 회귀하지 않도록 합니다.
    """
    st.markdown("### 계좌별 비주식·현금성 자산 관리")
    st.caption("TDF, 정기예금, 현금성 자산은 실시간 시세 조회 대신 원금과 평가금액을 직접 입력해 통합 자산에 반영합니다.")

    try:
        세션df = st.session_state.get("irp_non_stock_assets_df_v512")
    except Exception:
        세션df = None

    if 세션df is not None:
        try:
            현재df = IRP비주식자산표준열맞추기(세션df)
        except Exception:
    ...
    with st.expander("계좌별 비주식·현금성 자산 입력/수정", expanded=False):
        ...
        # 아래 확인표와 저장 버튼은 반드시 data_editor가 반환한 편집df만 사용합니다.
        표시저장기준df = IRP비주식자산표준열맞추기(편집df)
        try:
            st.session_state["irp_non_stock_assets_df_v512"] = 표시저장기준df.copy()
            st.session_state["irp_non_stock_assets_editor_truth_v5316"] = 표시저장기준df.copy()
        except Exception:
            pass

        st.caption("아래 확인표는 현재 Editor 반환값을 즉시 표준화한 결과입니다. 수정 전 DataFrame을 다시 표시하지 않습니다.")
...
def 최근자산변화_진단패널_v52812(df):
    try:
        d = _v52812_safe_df(df)
        if d.empty:
            return
        if "행유형" in d.columns:
            trade = d[d["행유형"].astype(str) == "실거래"].copy()
            explain = d[d["행유형"].astype(str) == "설명행"].copy()
        else:
            trade = d.copy()
            explain = pd.DataFrame()
        recent_realized = int(round(pd.to_numeric(trade.get("실현손익", 0), errors="coerce").fillna(0).sum())) if not trade.empty else 0
        ledger_realized = int(st.session_state.get("v5289_ledger_realized_total", st.session_state.get("v5269_ledger_realized_total", 0)))
        ledger_rows = int(st.session_state.get("v5289_ledger_trade_rows", len(trade)))
        diff = ledger_realized - recent_realized
        with st.expander("최근자산변화 진단 v5.28.12", expanded=False):
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("전체 표시행", f"{len(d):,}건")
            c2.metric("실거래", f"{len(trade):,}건")
            c3.metric("설명행", f"{len(explain):,}건")
            c4.metric("원장 거래행", f"{ledger_rows:,}건")
            d1, d2, d3 = st.columns(3)
            d1.metric("거래형 실현손익", _v5289_money_fmt(recent_realized, signed=True))
            d2.metric("원장 기준 전체 실현손익", _v5289_money_fmt(ledger_realized, signed=True))
            d3.metric("차이", _v5289_money_fmt(diff, signed=True))
            if abs(diff) == 3690927:
                st.info("차이는 TDF2035 실현손익 3,690,927원 포함 여부 때문입니다.")
            elif abs(diff) > 1:
        ...
        try:
            st.caption(f"최근자산변화 진단 표시 오류 v5.28.12: {type(e).__name__}: {e}")
        except Exception:
            pass


def 최근자산변화_표시_v52812(이동df, 최대표시=80):
    try:
        df = _v52812_safe_df(이동df)
        st.markdown("### 🔎 최근 자산변화")
        if df.empty:
            st.caption("최근 자산변화 표시 대상이 없습니다.")
            return df

        최근자산변화_진단패널_v52812(df)
        trade_df = df[df["행유형"].astype(str) == "실거래"].copy() if "행유형" in df.columns else df.copy()
        explain_df = df[df["행유형"].astype(str) == "설명행"].copy() if "행유형" in df.columns else pd.DataFrame()
        recent_realized = int(round(pd.to_numeric(trade_df.get("실현손익", 0), errors="coerce").fillna(0).sum())) if not trade_df.empty else 0
        ledger_realized = int(st.session_state.get("v5289_ledger_realized_total", st.session_state.get("v5269_ledger_realized_total", 0)))

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("실거래", f"{len(trade_df):,}건")
        c2.metric("설명행", f"{len(explain_df):,}건")
        c3.metric("거래형 실현손익", _v5289_money_fmt(recent_realized, signed=True))
        c4.metric("원장 기준 전체 실현손익", _v5289_money_fmt(ledger_realized, signed=True))
        st.caption(f"전체 표시행 {len(df):,}건 = 실거래 {len(trade_df):,}건 + 설명행 {len(explain_df):,}건")

        표시 = _v5289_display_df(df.head(max(최대표시, 80)))
        try:
            sty = 표시.style.map(lambda v: _v5289_profit_css(v), subset=["실현손익"])
        except Exception:
            try:
        ...
        try:
            st.caption(f"최근자산변화 v5.28.12 표시 오류: {type(e).__name__}: {e}")
        except Exception:
            pass
        return _v52812_safe_df(이동df)
...
def _v529_money(value, signed=False):
    try:
        return _v5289_money_fmt(value, signed=signed)
    except Exception:
        try:
            v = float(value or 0)
            prefix = "+" if signed and v > 0 else ""
            return f"{prefix}{int(round(v)):,}원"
        except Exception:
            return "0원"
...
def _v529_qty(value):
    try:
        return _v5289_qty_fmt(value)
    except Exception:
        try:
            v = float(value or 0)
            return f"{int(round(v)):,}주" if abs(v) >= 1 else "-"
        except Exception:
            return "-"


def _v529_price(value):
    try:
        return _v5289_price_fmt(value)
    except Exception:
        try:
            v = float(value or 0)
            return f"{int(round(v)):,}원" if abs(v) >= 1 else "-"
        except Exception:
            return "-"


def _v529_text(value):
    try:
...
def _v529_profit_css(value):
    try:
        return _v5289_profit_css(value)
    except Exception:
        s = str(value)
        if s.startswith("+"):
...
def _v529_display_df(df):
    ...
    d = d[cols].copy()
    d["수량"] = d["수량"].apply(_v529_qty)
    d["단가"] = d["단가"].apply(_v529_price)
    d["금액"] = d["금액"].apply(_v529_money)
    d["실현손익"] = d["실현손익"].apply(lambda x: _v529_money(x, signed=True))
    for c in ["날짜", "유형", "구분", "종목명", "계좌", "자동분석"]:
        d[c] = d[c].apply(_v529_text)
    return d


def _v529_recent_summary_numbers(df):
    d = _v529_safe_df(df)
    if d.empty:
        return 0, 0, 0, 0, 0, 0
    if "행유형" in d.columns:
        ...
        explain = pd.DataFrame()
    trade_realized = 0
    if not trade.empty and "실현손익" in trade.columns:
        trade_realized = int(round(pd.to_numeric(trade["실현손익"], errors="coerce").fillna(0).sum()))
    ledger_realized = int(st.session_state.get("v5289_ledger_realized_total", st.session_state.get("v5269_ledger_realized_total", 0)))
    diff = ledger_realized - trade_realized
    return len(d), len(trade), len(explain), trade_realized, ledger_realized, diff


def 최근자산변화_진단패널_v529(df):
    try:
        total, trade_count, explain_count, trade_realized, ledger_realized, diff = _v529_recent_summary_numbers(df)

        with st.expander("최근자산변화 진단 v5.29", expanded=False):
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("실거래", f"{trade_count:,}건")
            c2.metric("설명행", f"{explain_count:,}건")
            c3.metric("거래형 실현손익", _v529_money(trade_realized, signed=True))
            c4.metric("원장 기준 전체 실현손익", _v529_money(ledger_realized, signed=True))

            if explain_count:
                st.caption(f"전체 표시행 {total:,}건 = 실거래 {trade_count:,}건 + 설명행 {explain_count:,}건")
            if abs(diff) == 3690927:
                st.info("거래형 실현손익과 원장 기준 전체 실현손익의 차이는 TDF2035 실현손익 3,690,927원 포함 여부입니다.")
            elif abs(diff) > 1:
                st.warning(f"거래형 실현손익과 원장 기준 전체 실현손익 차이: {_v529_money(diff, signed=True)}")
    except Exception as e:
        try:
            st.caption(f"최근자산변화 진단 표시 오류 v5.29: {type(e).__name__}: {e}")
        except Exception:
            pass


def 최근자산변화_표시_v529(이동df, 최대표시=80):
    try:
        df = _v529_safe_df(이동df)
        st.markdown("### 🔎 최근 자산변화")
        if df.empty:
            st.caption("최근 자산변화 표시 대상이 없습니다.")
            return df
        ...
        if abs(diff) == 3690927:
            st.caption("거래형 실현손익은 주식·ETF 매도 기준이며, 원장 기준 전체 실현손익은 TDF2035 실현손익 3,690,927원을 포함합니다.")
        elif abs(diff) > 1:
            st.warning(f"거래형 실현손익과 원장 기준 전체 실현손익 차이: {_v529_money(diff, signed=True)}")
        else:
            st.caption(f"전체 표시행 {total:,}건 = 실거래 {trade_count:,}건 + 설명행 {explain_count:,}건")
        ...
        show_explain = False
        if explain_count:
            show_explain = st.checkbox("설명행 포함해서 보기", value=False, key="v529_recent_show_explain")
        if "행유형" in df.columns and not show_explain:
            view = df[df["행유형"].astype(str) == "실거래"].copy()
        else:
            view = df.copy()

        표시 = _v529_display_df(view.head(max(최대표시, 80)))
        try:
            sty = 표시.style.map(lambda v: _v529_profit_css(v), subset=["실현손익"])
        except Exception:
            try:
                sty = 표시.style.applymap(lambda v: _v529_profit_css(v), subset=["실현손익"])
            except Exception:
                sty = 표시
        try:
            if "표데이터프레임" in globals():
        ...
        if explain_count and not show_explain:
            with st.expander(f"설명행 {explain_count:,}건 보기", expanded=False):
                explain_df = df[df["행유형"].astype(str) == "설명행"].copy() if "행유형" in df.columns else pd.DataFrame()
                if not explain_df.empty:
                    ex_display = _v529_display_df(explain_df)
                    if "표데이터프레임" in globals():
                        표데이터프레임(ex_display, width="stretch", hide_index=True)
                    else:
                        st.dataframe(ex_display, use_container_width=True, hide_index=True)
        return df
    except Exception as e:
        try:
            st.caption(f"최근자산변화 표시 오류 v5.29: {type(e).__name__}: {e}")
        except Exception:
            pass
        return _v529_safe_df(이동df)
...
def _v5251_apply_realized_to_cash_rows(통합, realized):
    ...
    과거 v5.25.1 로직은 휴머노이드 ETF 매도 손실(-28,602원)을
    현금성 대기자산 원금에 더해 90,138원이 118,740원으로 되살아나는
    오류를 만들었습니다.

    원칙:
    - 실현손익은 최근자산변화/거래검증에서만 관리합니다.
    - 예수금·현금성 대기자산은 투자상품이 아니라 현재 잔액입니다.
    - 따라서 현금성자산은 항상 원금 = 평가금액, 평가손익 = 0, 수익률 = 0입니다.
    """
    try:
        t = pd.DataFrame(통합).copy() if 통합 is not None else pd.DataFrame()
        if t.empty:
            return t

        # 과거 임시 매도대금 보정행은 제거합니다.
        if "상품명" in t.columns:
            old = t["상품명"].astype(str).str.contains("매도대금 임시반영", na=False)
            if "비고" in t.columns:
                old = old | t["비고"].astype(str).str.contains("매도대금.*임시", regex=True, na=False)
            t = t[~old].copy()

        for c in ["원금", "평가금액", "평가손익", "수익률"]:
            if c not in t.columns:
                t[c] = 0
        for c in ["원금", "평가금액", "평가손익", "수익률"]:
            t[c] = pd.to_numeric(t[c], errors="coerce").fillna(0)

        텍스트 = pd.Series("", index=t.index, dtype="object")
        for c in ["계좌", "자산군", "상품명", "비고", "유형", "메모"]:
            if c in t.columns:
                텍스트 = 텍스트 + " " + t[c].astype(str)

        현금마스크 = 텍스트.str.contains("현금|예수금|대기자산|CMA|MMDA|MMF|입출금", case=False, regex=True, na=False)
        if 현금마스크.any():
            원금값 = pd.to_numeric(t.loc[현금마스크, "원금"], errors="coerce").fillna(0)
            평가값 = pd.to_numeric(t.loc[현금마스크, "평가금액"], errors="coerce").fillna(0)
            현재잔액 = 평가값.where(평가값 > 0, 원금값)
            t.loc[현금마스크, "원금"] = 현재잔액
            t.loc[현금마스크, "평가금액"] = 현재잔액
            t.loc[현금마스크, "평가손익"] = 0
            t.loc[현금마스크, "수익률"] = 0
            if "비고" in t.columns:
                t.loc[현금마스크, "비고"] = t.loc[현금마스크, "비고"].astype(str).str.replace(
                    r"\s*·?\s*실현손익 반영:.*$", "", regex=True
                )

        # 비현금성 자산은 기존 원금/평가금액 기준으로 손익을 재계산합니다.
        t["평가손익"] = pd.to_numeric(t["평가금액"], errors="coerce").fillna(0) - pd.to_numeric(t["원금"], errors="coerce").fillna(0)
        if 현금마스크.any():
            t.loc[현금마스크, "평가손익"] = 0
            t.loc[현금마스크, "수익률"] = 0
        t["수익률"] = np.where(pd.to_numeric(t["원금"], errors="coerce").fillna(0) != 0, t["평가손익"] / pd.to_numeric(t["원금"], errors="coerce").fillna(0) * 100, 0)
        if 현금마스크.any():
            t.loc[현금마스크, "수익률"] = 0
        ...
        logging.warning("v5292 cash principal final normalization failed: %s", e, exc_info=True)
        return 통합
...
def _v5268_date(value):
    try:
        if '_v5267_date' in globals():
            return _v5267_date(value)
    except Exception:
        pass
    try:
        ts = pd.to_datetime(value, errors='coerce')
...
def _v5268_ledger_detail(거래df):
    """회계검증 엔진의 원장 기준 실현손익 상세를 가져옵니다."""
    try:
        if 'v5260_거래원장실현손익계산' not in globals():
            return pd.DataFrame(), 0
        detail, _summary, total = v5260_거래원장실현손익계산(거래df, include_manual_tdf=True)
        d = pd.DataFrame(detail).copy() if detail is not None else pd.DataFrame()
        if d.empty:
            return d, int(round(_v5268_num(total, 0)))
        d['실현손익'] = pd.to_numeric(d.get('실현손익', 0), errors='coerce').fillna(0)
        d = d[d['실현손익'].round().astype(int) != 0].copy()
        return d, int(round(_v5268_num(total, d['실현손익'].sum())))
    except Exception as e:
        try:
            logging.warning('v5268 ledger detail failed: %s', e, exc_info=True)
        except Exception:
            pass
        return pd.DataFrame(), 0


def _v5268_existing_realized_keys(movements):
    """최근자산변화 이동목록 안에 이미 존재하는 실현손익 거래 키를 수집합니다."""
    keys = set()
    try:
        df = pd.DataFrame(movements).copy()
        if df.empty:
            return keys
        for _, r in df.iterrows():
            pnl = int(round(_v5268_num(r.get('수익손실부분', r.get('실현손익', 0)), 0)))
            if pnl == 0:
                continue
            date = _v5268_date(r.get('날짜', r.get('거래일자', '')))
            code = str(r.get('종목코드', '') or '').strip()
            name_text = _v5268_text(r, ['종목명', '상세설명', '자동분석', '시스템해석'])
            amount = int(round(abs(_v5268_num(r.get('금액', r.get('매도금액', 0)), 0))))
            keys.add((date, pnl))
            keys.add((date, amount, pnl))
            if code:
                keys.add((date, code, pnl))
                keys.add((date, code, amount, pnl))
            # 종목명이 길거나 약칭으로 표시되는 경우를 대비한 토큰 키
            for token in ['TDF2035', 'SK하이닉스', 'AI반도체핵심장비', 'AI반도체', '코스닥150', '휴머노이드', 'AI전력', 'HD현대마린엔진']:
                if token in name_text:
                    keys.add((date, token, pnl))
                    keys.add((date, token, amount, pnl))
    except Exception:
        pass
    return keys
...
def _v5268_detail_to_movement(r):
    code = str(r.get('종목코드', '') or '').strip()
    name = str(r.get('종목명', '') or code or '').strip()
    date = _v5268_date(r.get('거래일자', r.get('날짜', '')))
    sell_amt = int(round(_v5268_num(r.get('매도금액', 0), 0)))
    cost = int(round(_v5268_num(r.get('매수원금', 0), 0)))
    pnl = int(round(_v5268_num(r.get('실현손익', 0), 0)))
    qty = r.get('매도수량', r.get('수량', 0))
    price = r.get('매도단가', r.get('단가', 0))
    account = str(r.get('운용사', '') or r.get('계좌', '') or '')
    is_tdf = ('TDF' in name.upper()) or ('TDF' in code.upper())
    pnl_label = '수익실현' if pnl > 0 else '손실실현'
    return {
        '날짜': date,
        '계좌': account,
        '구분': '수익실현' if is_tdf else '매도',
        '종목코드': code,
        '종목명': name,
        '자산유형': 'TDF' if is_tdf else '주식형자산',
        '수량': qty,
        ...
        '원금부분': cost,
        '수익손실부분': pnl,
        '변화유형': '수익실현' if pnl > 0 else '손실실현',
        '상세설명': f'{name} 매도 → 현금성 대기자산',
        '자동분석': f'거래원장 기준 보강: 매도금액 {sell_amt:,}원, 원금 {cost:,}원, {pnl_label} {abs(pnl):,}원으로 반영합니다.',
        '출처': 'v5.26.8 거래원장실현손익보강',
    }


def _v5268_missing_realized_rows(거래df, movements):
    detail, ledger_total = _v5268_ledger_detail(거래df)
    try:
        st.session_state['v5268_ledger_realized_sum'] = int(ledger_total)
    except Exception:
        pass
    if detail.empty:
        return pd.DataFrame()
    existing = _v5268_existing_realized_keys(movements)
    rows = []
    for _, r in detail.iterrows():
        pnl = int(round(_v5268_num(r.get('실현손익', 0), 0)))
        if pnl == 0:
        ...
        code = str(r.get('종목코드', '') or '').strip()
        name = str(r.get('종목명', '') or '')
        amount = int(round(abs(_v5268_num(r.get('매도금액', 0), 0))))
        token = ''
        for t in ['TDF2035', 'SK하이닉스', 'AI반도체핵심장비', 'AI반도체', '코스닥150', '휴머노이드', 'AI전력', 'HD현대마린엔진']:
            if t in name:
                token = t
                break
        if ((date, pnl) in existing or
            (date, amount, pnl) in existing or
            (code and (date, code, pnl) in existing) or
            (code and (date, code, amount, pnl) in existing) or
            (token and (date, token, pnl) in existing) or
            (token and (date, token, amount, pnl) in existing)):
            continue
        rows.append(_v5268_detail_to_movement(r))
    return pd.DataFrame(rows)


try:
    _자산이동목록통합_v5268_base = 자산이동목록통합_v5225
except Exception:
...
def _v5269_num(value, default=0.0):
    try:
        if '_v5268_num' in globals():
            return _v5268_num(value, default)
    except Exception:
        pass
    try:
        if value is None or pd.isna(value):
...
def _v5269_ledger_detail_total(거래df):
    try:
        if 'v5260_거래원장실현손익계산' not in globals():
        ...
        if not d.empty and '실현손익' in d.columns:
            d['실현손익'] = pd.to_numeric(d['실현손익'], errors='coerce').fillna(0)
        total_int = int(round(_v5269_num(total, d['실현손익'].sum() if not d.empty and '실현손익' in d.columns else 0)))
        return d, total_int
    except Exception as e:
        try:
            logging.warning('v5269 ledger total failed: %s', e, exc_info=True)
        except Exception:
            pass
        return pd.DataFrame(), None


def _v5269_existing_exact_realized_keys(movements):
    keys = set()
    try:
        df = pd.DataFrame(movements).copy()
        if df.empty:
            return keys
        for _, r in df.iterrows():
            date = _v5269_date(r.get('날짜', r.get('거래일자', '')))
            amount = int(round(abs(_v5269_num(r.get('금액', r.get('매도금액', r.get('이동금액', 0))), 0))))
            pnl = int(round(_v5269_num(r.get('수익손실부분', r.get('실현손익', 0)), 0)))
            code = str(r.get('종목코드', '') or '').strip()
            name_blob = ' '.join(str(r.get(c, '') or '') for c in ['종목명', '상세설명', '자동분석', '시스템해석'])
            if pnl == 0:
                continue
            # 금액+손익이 모두 같은 경우만 같은 거래로 봅니다.
            keys.add((date, amount, pnl))
            if code:
                keys.add((date, code, amount, pnl))
            for token in ['TDF2035', 'SK하이닉스', 'AI반도체핵심장비', 'AI전력핵심설비', '코스닥150', '휴머노이드', 'HD현대마린엔진']:
...
def _v5269_detail_to_movement(r):
    code = str(r.get('종목코드', '') or '').strip()
    name = str(r.get('종목명', '') or code or '').strip()
    date = _v5269_date(r.get('거래일자', r.get('날짜', '')))
    sell_amt = int(round(_v5269_num(r.get('매도금액', 0), 0)))
    cost = int(round(_v5269_num(r.get('매수원금', 0), 0)))
    pnl = int(round(_v5269_num(r.get('실현손익', 0), 0)))
    account = str(r.get('운용사', '') or r.get('계좌', '') or '')
    pnl_word = '수익실현' if pnl > 0 else '손실실현'
    return {
        '날짜': date,
        '계좌': account,
        '구분': '매도',
        ...
        '수익손실부분': pnl,
        '상세설명': f'{name} 매도 → 현금성 대기자산',
        '자동분석': f'거래원장 기준 보강: 매도금액 {sell_amt:,}원, 원금 {cost:,}원, {pnl_word} {abs(pnl):,}원으로 반영합니다.',
        '출처': 'v5.26.9 거래원장실현손익보강',
    }


def _v5269_missing_realized_rows(거래df, movements):
    detail, ledger_total = _v5269_ledger_detail_total(거래df)
    try:
    ...
    if detail.empty:
        return pd.DataFrame()
    existing = _v5269_existing_exact_realized_keys(movements)
    rows = []
    for _, r in detail.iterrows():
        pnl = int(round(_v5269_num(r.get('실현손익', 0), 0)))
        if pnl == 0:
        ...
        code = str(r.get('종목코드', '') or '').strip()
        name = str(r.get('종목명', '') or '')
        amount = int(round(abs(_v5269_num(r.get('매도금액', 0), 0))))
        token = ''
        for t in ['TDF2035', 'SK하이닉스', 'AI반도체핵심장비', 'AI전력핵심설비', '코스닥150', '휴머노이드', 'HD현대마린엔진']:
            if t in name:
                token = t
                break
        if ((date, amount, pnl) in existing or
            ...
            (token and (date, token, amount, pnl) in existing)):
            continue
        rows.append(_v5269_detail_to_movement(r))
    try:
        st.session_state['v5269_missing_realized_rows'] = int(len(rows))
    except Exception:
        pass
    return pd.DataFrame(rows)
...
try:
    _자산이동목록통합_v5269_base = 자산이동목록통합_v5225
except Exception:
...
def _v5269_display_recent_movements(이동df, 최대표시=80):
    try:
        ...
        for c in ['금액', '이동금액', '원금부분', '수익손실부분']:
            if c in 이동df.columns:
                이동df[c] = pd.to_numeric(이동df[c], errors='coerce').fillna(0)
        if '금액' not in 이동df.columns:
            이동df['금액'] = 0
        if '이동금액' not in 이동df.columns:
            이동df['이동금액'] = 이동df['금액'].abs()
        if '원금부분' not in 이동df.columns:
            이동df['원금부분'] = 이동df['이동금액'].abs()
        if '수익손실부분' not in 이동df.columns:
            이동df['수익손실부분'] = 0
        ...
        표시행손익 = int(round(pd.to_numeric(이동df['수익손실부분'], errors='coerce').fillna(0).sum()))
        원장손익 = st.session_state.get('v5269_ledger_realized_total', None)
        총손익 = int(원장손익) if 원장손익 is not None else 표시행손익
        ...
        손익표시 = 원화정수포맷(총손익) if abs(총손익) >= 1 else '0원'
        신규표시 = f'{len(신규편입):,}종목' if 신규편입 else '-'
        손익주석 = 'Google Sheets 원장 기준'
        if 원장손익 is not None and int(원장손익) != 표시행손익:
            손익주석 = f'원장 기준 / 표시행 합계 {원화정수포맷(표시행손익)}'
        ...
        kpi_html = (
            ...
            f'<div class="asset-kpi"><div class="asset-kpi-label">이동금액</div><div class="asset-kpi-value">{원화정수포맷(총금액)}</div><div class="asset-kpi-note">조회기간 {조회기간표시}</div></div>'
            f'<div class="asset-kpi"><div class="asset-kpi-label">신규편입</div><div class="asset-kpi-value">{_html_escape_v5224(신규표시)}</div><div class="asset-kpi-note">최근 매수 기준</div></div>'
            f'<div class="asset-kpi"><div class="asset-kpi-label">실현손익</div><div class="asset-kpi-value">{손익표시}</div><div class="asset-kpi-note">{_html_escape_v5224(손익주석)}</div></div>'
            '</div>'
        )
        st.markdown(kpi_html, unsafe_allow_html=True)
        ...
        for _, row in 이동df.head(최대표시).iterrows():
            ...
            자동 = str(row.get('자동분석', '')).strip()
            계좌 = _계좌짧게_v5226(row.get('계좌', ''))
            이동금액 = abs(_v5269_num(row.get('이동금액', row.get('금액', 0))))
            원금부분 = abs(_v5269_num(row.get('원금부분', 이동금액)))
            손익부분 = _v5269_num(row.get('수익손실부분', 0))
            손익배지 = ''
            if 손익부분 > 0:
                손익배지 = f'<span class="profit-pill-pos">수익실현 {원화정수포맷(손익부분)}</span>'
            elif 손익부분 < 0:
        ...
        table_html = (
            ...
            f'<tbody>{"".join(rows_html)}</tbody>'
            '</table>'
            '<div class="asset-change-foot">ⓘ 실현손익 KPI는 Google Sheets 거래원장 기준으로 표시합니다. 표시행에는 거래·자금이체·현금대기 설명행이 함께 포함될 수 있습니다.</div>'
            '</div>'
        )
        st.markdown(table_html, unsafe_allow_html=True)
        if len(이동df) > 최대표시:
            with st.expander(f'전체 자산 변화 목록 보기 · {len(이동df):,}건', expanded=False):
                표시열 = [c for c in ['날짜', '구분', '상세설명', '금액', '원금부분', '수익손실부분', '계좌', '자동분석', '출처'] if c in 이동df.columns]
                표시 = 이동df[표시열].copy()
                try:
                    숫자포맷 = {c: 원화정수포맷 for c in ['금액', '원금부분', '수익손실부분'] if c in 표시.columns}
                    표데이터프레임(표시.style.format(숫자포맷), width='stretch', hide_index=True)
        ...
        st.caption(f'최근 자산변화 v5.26.9 표시 오류: {type(e).__name__}: {e}')
        try:
            return 이동df
        except Exception:
            return pd.DataFrame()
...
_V5271_ASSET_ALIASES = {
    '487240': 'KODEXAI전력핵심설비',
    'KODEXAI전력핵심설비': 'KODEXAI전력핵심설비',
    'AI전력핵심설비': 'KODEXAI전력핵심설비',
    '471990': 'KODEXAI반도체핵심장비',
    'KODEXAI반도체핵심장비': 'KODEXAI반도체핵심장비',
    'AI반도체핵심장비': 'KODEXAI반도체핵심장비',
    '000660': 'SK하이닉스',
    'SK하이닉스': 'SK하이닉스',
    '071970': 'HD현대마린엔진',
    'HD현대마린엔진': 'HD현대마린엔진',
    '0148J0': 'TIGER코리아휴머노이드로봇산업',
    'TIGER코리아휴머노이드로봇산업': 'TIGER코리아휴머노이드로봇산업',
    '휴머노이드': 'TIGER코리아휴머노이드로봇산업',
    '229200': 'KODEX코스닥150',
    'KODEX코스닥150': 'KODEX코스닥150',
    '005930': '삼성전자',
    '삼성전자': '삼성전자',
    '005380': '현대차',
    '현대차': '현대차',
...
def _v5271_asset_key(row):
    try:
        pieces = []
        for c in ['종목코드', '종목명', '상세설명', '자동분석', '구분']:
            pieces.append(str(row.get(c, '') or ''))
        text = _v5271_compact(' '.join(pieces))
        # 코드/명칭을 동일한 대표키로 정규화합니다.
        for alias, canonical in sorted(_V5271_ASSET_ALIASES.items(), key=lambda x: len(x[0]), reverse=True):
            if _v5271_compact(alias) and _v5271_compact(alias) in text:
                return canonical
        # 알려진 자산이 아니면 종목코드 우선, 없으면 종목명/상세설명 일부 사용
        code = _v5271_compact(row.get('종목코드', ''))
        if code:
            return code
        name = _v5271_compact(row.get('종목명', ''))
        if name:
            return name
        return text[:40]
    except Exception:
...
def _v5271_account_key(value):
    s = _v5271_compact(value)
    if '신한' in s and 'IRP' in s:
        return '신한IRP'
    if '미래에셋' in s:
        return '미래에셋'
    return s
...
def _v5271_row_text(row, cols=None):
    try:
        if cols is None:
            cols = ['구분', '상세설명', '자동분석', '변화유형', '출처', '계좌', '종목명']
        return ' '.join(str(row.get(c, '') or '') for c in cols)
    except Exception:
        return ''


def _v5271_is_sell_like(row):
    text = _v5271_row_text(row)
    return ('매도' in text) or ('수익실현' in text) or ('손실실현' in text)


def _v5271_pnl_value(row):
    try:
        for c in ['수익손실부분', '실현손익', '손익', '처분손익']:
            if c in row:
                n = _v5271_num(row.get(c, 0), 0)
                if abs(n) > 0:
                    return int(round(n))
        return 0
    except Exception:
...
def _v5271_amount_value(row):
    try:
        for c in ['금액', '이동금액', '매도금액']:
            if c in row:
                n = abs(_v5271_num(row.get(c, 0), 0))
                if n > 0:
                    return int(round(n))
        return 0
    except Exception:
...
def _v5271_is_explain_row(row):
    """실제 거래가 아니라 매도대금/예수금/현금대기 흐름 설명용 행인지 판단합니다."""
    try:
        if _v5271_pnl_value(row) != 0:
            return False
        if not _v5271_is_sell_like(row):
            return False
        text = _v5271_row_text(row)
        markers = [
            '원금변화 없음',
...
def _v5271_clean_recent_display_rows(df):
    try:
        out = pd.DataFrame(df).copy()
        if out.empty:
            return out
        for c in ['날짜', '계좌', '구분', '종목코드', '종목명', '상세설명', '금액', '이동금액', '원금부분', '수익손실부분', '자동분석', '출처']:
            if c not in out.columns:
                out[c] = 0 if c in ['금액', '이동금액', '원금부분', '수익손실부분'] else ''

        out['_date_v5271'] = out['날짜'].apply(_v5271_date)
        out['_acct_v5271'] = out['계좌'].apply(_v5271_account_key)
        out['_asset_v5271'] = out.apply(_v5271_asset_key, axis=1)
        out['_amount_v5271'] = out.apply(_v5271_amount_value, axis=1)
        out['_pnl_v5271'] = out.apply(_v5271_pnl_value, axis=1)
        out['_sell_v5271'] = out.apply(_v5271_is_sell_like, axis=1)
        out['_explain_v5271'] = out.apply(_v5271_is_explain_row, axis=1)

        real_keys = set()
        for _, r in out.iterrows():
            if bool(r.get('_sell_v5271')) and int(r.get('_pnl_v5271', 0)) != 0 and int(r.get('_amount_v5271', 0)) > 0:
                real_keys.add((
                    str(r.get('_date_v5271', '')),
                    str(r.get('_acct_v5271', '')),
                    str(r.get('_asset_v5271', '')),
                    int(r.get('_amount_v5271', 0)),
                ))
        ...
        for idx, r in out.iterrows():
            ...
            if bool(r.get('_explain_v5271')) and key in real_keys:
                drop_idx.append(idx)

        cleaned = out.drop(index=drop_idx) if drop_idx else out
        try:
            st.session_state['v5271_removed_explain_rows'] = int(len(drop_idx))
            st.session_state['v5271_recent_rows_before'] = int(len(out))
            st.session_state['v5271_recent_rows_after'] = int(len(cleaned))
            st.session_state['v5271_recent_realized_sum_after'] = int(round(pd.to_numeric(cleaned.get('수익손실부분', 0), errors='coerce').fillna(0).sum()))
        except Exception:
            pass
        return cleaned.drop(columns=['_date_v5271', '_acct_v5271', '_asset_v5271', '_amount_v5271', '_pnl_v5271', '_sell_v5271', '_explain_v5271'], errors='ignore').reset_index(drop=True)
    except Exception as e:
        try:
            logging.warning('v5271 clean recent display rows failed: %s', e, exc_info=True)
        except Exception:
            pass
        return df
...
def _v5282_num(value, default=0.0):
    try:
        if value is None:
            return default
        if isinstance(value, str):
...
def _v5282_date(value):
    try:
        ...
        pass
    return _v5282_text(value)


def _v5282_pick_col(df, candidates):
    try:
        for c in candidates:
            if c in df.columns:
                return c
...
def _v5282_trade_rows_from_ledger(거래df):
    """거래원장 원본 행을 최근자산변화 표시용 행으로 변환합니다.
    같은 날짜·같은 종목이어도 수량/단가/금액이 다르면 별도 거래로 남깁니다.
    """
    try:
        src = pd.DataFrame(거래df).copy()
    except Exception:
        return pd.DataFrame()
    if src.empty:
        return pd.DataFrame()

    date_col = _v5282_pick_col(src, ["거래일자", "날짜", "일자", "매매일자"])
    code_col = _v5282_pick_col(src, ["종목코드", "코드", "ticker", "symbol"])
    name_col = _v5282_pick_col(src, ["종목명", "상품명", "자산명", "보유종목"])
    type_col = _v5282_pick_col(src, ["거래구분", "구분", "매매구분", "유형"])
    qty_col = _v5282_pick_col(src, ["수량", "매수수량", "매도수량", "보유수량"])
    price_col = _v5282_pick_col(src, ["단가", "매수단가", "매도단가", "평균매입단가", "거래단가"])
    amount_col = _v5282_pick_col(src, ["금액", "거래금액", "매수금액", "매도금액", "매입금액"])
    account_col = _v5282_pick_col(src, ["계좌", "운용사", "증권사"])
    memo_col = _v5282_pick_col(src, ["비고", "메모", "투자메모", "자동분석"])

    rows = []
    for _, r in src.iterrows():
        구분 = _v5282_text(r.get(type_col, "")) if type_col else ""
        # 최근자산변화는 실제 거래성 행만 원장 기준으로 보강합니다.
        if 구분 and not any(k in 구분 for k in ["매수", "매도", "배당", "입금", "출금"]):
            continue
        날짜 = _v5282_date(r.get(date_col, "")) if date_col else ""
        코드 = _v5282_text(r.get(code_col, "")) if code_col else ""
        이름 = _v5282_text(r.get(name_col, "")) if name_col else ""
        계좌 = _v5282_text(r.get(account_col, "")) if account_col else ""
        수량 = abs(_v5282_num(r.get(qty_col, 0), 0)) if qty_col else 0
        단가 = abs(_v5282_num(r.get(price_col, 0), 0)) if price_col else 0
        금액 = abs(_v5282_num(r.get(amount_col, 0), 0)) if amount_col else 0
        if 금액 <= 0 and 수량 > 0 and 단가 > 0:
            금액 = 수량 * 단가
        메모 = _v5282_text(r.get(memo_col, "")) if memo_col else ""
        if not 날짜 and 금액 <= 0:
            continue

        상세 = f"{계좌 + ' ' if 계좌 else ''}{이름 or 코드} {구분}".strip()
        자동 = "거래원장 기준 실제 거래행 반영"
        if "매수" in 구분 and 계좌:
            자동 = f"{계좌} 예수금에서 {이름 or 코드} 매수로 이동"
        elif "매도" in 구분:
            자동 = "거래원장 기준 매도 거래 반영"
        ...
        rows.append({
            ...
            "단가": 단가,
            "금액": 금액,
            "원금부분": 금액 if "매수" in 구분 else 0,
            "수익손실부분": 0,
            "상세설명": 상세,
            "자동분석": 자동 if not 메모 else f"{자동} · {메모}",
            "출처": "v5282_거래원장",
        })
    out = pd.DataFrame(rows)
    return out
...
def _v5282_trade_exact_key(row):
    try:
        return "|".join([
            _v5282_text(row.get("날짜", ""))[:10],
            _v5282_text(row.get("계좌", "")),
            _v5282_text(row.get("구분", "")),
            _v5282_text(row.get("종목코드", "")),
            _v5282_text(row.get("종목명", "")),
            str(int(round(abs(_v5282_num(row.get("수량", 0), 0))))),
            str(int(round(abs(_v5282_num(row.get("단가", 0), 0))))),
            str(int(round(abs(_v5282_num(row.get("금액", 0), 0))))),
        ])
    except Exception:
        return ""


def _v5282_trade_loose_key(row):
    try:
        return "|".join([
            _v5282_text(row.get("날짜", ""))[:10],
            _v5282_text(row.get("계좌", "")),
...
def 최근자산변화_행유형판정_v5282(row):
    try:
        구분 = _v5282_text(row.get("구분", ""))
        text = " ".join([
            구분,
            _v5282_text(row.get("상세설명", "")),
            _v5282_text(row.get("자동분석", "")),
            _v5282_text(row.get("출처", "")),
            _v5282_text(row.get("자산유형", "")),
        ])
        if "v5282_거래원장" in text:
            return "실거래"
        if any(k in 구분 for k in ["매수", "매도", "배당", "입금", "출금"]):
            return "실거래"
        if any(k in text for k in ["잔액", "보유", "설명행", "현재잔액"]):
            return "설명행"
        if any(k in text for k in ["자산이동", "이체", "대기자산", "현금성", "예수금", "매도대금"]):
            return "자산이동"
        return "실거래" if abs(_v5282_num(row.get("금액", 0), 0)) > 0 else "설명행"
    except Exception:
        return "미분류"


def 최근자산변화_정리_v5282(이동df):
    try:
        df = pd.DataFrame(이동df).copy()
    except Exception:
        return pd.DataFrame()
    ...
    df["날짜"] = df["날짜"].apply(_v5282_date)
    df["원금"] = df["원금부분"]
    df["실현손익"] = df["수익손실부분"]
    df["행유형"] = df.apply(최근자산변화_행유형판정_v5282, axis=1)

    try:
        df["_dt_v5282"] = pd.to_datetime(df["날짜"], errors="coerce")
        df["_exact_v5282"] = df.apply(_v5282_trade_exact_key, axis=1)
        df = df.sort_values(["_dt_v5282", "금액"], ascending=[False, False], kind="mergesort")
        # 완전 동일 행만 제거합니다. 삼성전자 3회 매수처럼 금액/수량/단가가 다르면 보존됩니다.
        df = df.drop_duplicates("_exact_v5282", keep="first")
        df = df.drop(columns=["_dt_v5282", "_exact_v5282"], errors="ignore").reset_index(drop=True)
    except Exception:
        pass

    try:
        st.session_state["v5282_recent_total_rows"] = int(len(df))
        st.session_state["v5282_recent_type_counts"] = df["행유형"].value_counts().to_dict()
        st.session_state["v5282_recent_realized_sum"] = int(round(pd.to_numeric(df["수익손실부분"], errors="coerce").fillna(0).sum()))
    except Exception:
        pass
    return df


def _v5282_merge_legacy_and_ledger(base, 거래df):
    try:
        base_df = pd.DataFrame(base).copy()
    except Exception:
        base_df = pd.DataFrame()
    ledger_rows = _v5282_trade_rows_from_ledger(거래df)
    if ledger_rows.empty:
        return 최근자산변화_정리_v5282(base_df)

    try:
        ledger_loose = set(ledger_rows.apply(_v5282_trade_loose_key, axis=1).tolist())
        if not base_df.empty:
            base_df["_loose_v5282"] = base_df.apply(_v5282_trade_loose_key, axis=1)
            trade_mask = base_df.get("구분", pd.Series([""] * len(base_df))).astype(str).str.contains("매수|매도|배당|입금|출금", na=False)
            base_df = base_df.loc[~(base_df["_loose_v5282"].isin(ledger_loose) & trade_mask)].drop(columns=["_loose_v5282"], errors="ignore")
    except Exception:
        base_df = base_df.drop(columns=["_loose_v5282"], errors="ignore") if not base_df.empty else base_df

    try:
        merged = pd.concat([base_df, ledger_rows], ignore_index=True, sort=False)
    except Exception:
        merged = ledger_rows if base_df.empty else base_df
    return 최근자산변화_정리_v5282(merged)


def 최근자산변화_생성_v5282(거래df=None, 비주식자산df=None, 최근일수=3650):
    try:
        if _자산이동목록통합_v5282_legacy_base:
            base = _자산이동목록통합_v5282_legacy_base(거래df, 비주식자산df, 최근일수=최근일수)
        else:
            base = pd.DataFrame()
    except Exception as e:
        try:
            logging.warning("v5282 legacy movement failed: %s", e, exc_info=True)
        except Exception:
            pass
        base = pd.DataFrame()
    out = _v5282_merge_legacy_and_ledger(base, 거래df)
    try:
        ledger_rows = _v5282_trade_rows_from_ledger(거래df)
        st.session_state["v5282_ledger_trade_rows"] = int(len(ledger_rows))
        st.session_state["v5282_recent_rows_after_merge"] = int(len(out))
        ...
        if not recent_ledger.empty:
            ...
            out_keys = set(out.apply(_v5282_trade_exact_key, axis=1).tolist()) if not out.empty else set()
            missing = recent_ledger[~recent_ledger.apply(_v5282_trade_exact_key, axis=1).isin(out_keys)]
            st.session_state["v5282_recent_missing_ledger_rows"] = int(len(missing))
        else:
            st.session_state["v5282_recent_missing_ledger_rows"] = 0
    except Exception:
        pass
    return out


def 최근자산변화_진단패널_v5282(df=None):
    try:
        if df is not None:
            dfx = 최근자산변화_정리_v5282(df)
            total = len(dfx)
            counts = dfx["행유형"].value_counts().to_dict() if "행유형" in dfx.columns else {}
            realized = int(round(pd.to_numeric(dfx.get("수익손실부분", 0), errors="coerce").fillna(0).sum()))
        else:
            total = st.session_state.get("v5282_recent_total_rows", 0)
            counts = st.session_state.get("v5282_recent_type_counts", {})
            realized = st.session_state.get("v5282_recent_realized_sum", 0)
        with st.expander("최근자산변화 진단 v5.28.2", expanded=False):
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("전체", f"{int(total):,}건")
            c2.metric("실거래", f"{int(counts.get('실거래', 0)):,}건")
            c3.metric("자산이동", f"{int(counts.get('자산이동', 0)):,}건")
            c4.metric("설명행", f"{int(counts.get('설명행', 0)):,}건")
            d1, d2, d3 = st.columns(3)
            d1.metric("원장 거래행", f"{int(st.session_state.get('v5282_ledger_trade_rows', 0)):,}건")
            d2.metric("최근자산변화", f"{int(st.session_state.get('v5282_recent_rows_after_merge', 0)):,}건")
            d3.metric("최근 원장 누락", f"{int(st.session_state.get('v5282_recent_missing_ledger_rows', 0)):,}건")
            st.caption(f"표시목록 기준 실현손익 합계: {int(realized):,}원")
    except Exception:
        pass


def 최근자산변화_표시_v5282(이동df, 최대표시=80):
    try:
        df = 최근자산변화_정리_v5282(이동df)
        st.markdown("### 🔎 최근 자산변화")
        최근자산변화_진단패널_v5282(df)
        if df.empty:
        ...
        try:
            ...
            st.dataframe(표시, use_container_width=True, hide_index=True)
        if len(df) > max(최대표시, 80):
            with st.expander(f"전체 최근자산변화 보기 · {len(df):,}건", expanded=False):
                st.dataframe(df, use_container_width=True, hide_index=True)
        return df
    except Exception as e:
        try:
            st.caption(f"최근자산변화 v5.28.2 표시 오류: {type(e).__name__}: {e}")
        except Exception:
            pass
        try:
...
def _v5283_num(value, default=0.0):
    try:
        if value is None:
            return default
        if isinstance(value, str):
...
def _v5283_date(value):
    try:
        ...
        pass
    return _v5283_text(value)


def _v5283_pick_col(df, candidates):
    try:
        ...
        for c in candidates:
            ...
            if key in norm:
                return norm[key]
        for c in df.columns:
            sc = str(c).strip().replace(" ", "")
            if any(str(k).replace(" ", "") in sc for k in candidates):
                return c
    except Exception:
        pass
    return None
...
def _v5283_col_after(df, base_col, offset):
    try:
        cols = list(df.columns)
        if base_col in cols:
            idx = cols.index(base_col) + offset
            if 0 <= idx < len(cols):
                return cols[idx]
    except Exception:
        pass
...
def _v5283_value(row, col, default=""):
    try:
...
def _v5283_trade_rows_from_ledger(거래df):
    try:
        src = pd.DataFrame(거래df).copy()
    except Exception:
        return pd.DataFrame()
    ...
    date_col = _v5283_pick_col(src, ["거래일자", "거래일", "날짜", "일자", "매매일자"])
    code_col = _v5283_pick_col(src, ["종목코드", "코드", "ticker", "symbol"])
    name_col = _v5283_pick_col(src, ["종목명", "상품명", "자산명", "보유종목"])
    type_col = _v5283_pick_col(src, ["거래구분", "구분", "매매구분", "유형"])
    qty_col = _v5283_pick_col(src, ["수량", "매수수량", "매도수량", "거래수량", "체결수량", "주수"])
    price_col = _v5283_pick_col(src, ["단가", "매수단가", "매도단가", "거래단가", "체결단가", "평균매입단가", "가격"])
    amount_col = _v5283_pick_col(src, ["금액", "거래금액", "매수금액", "매도금액", "매입금액", "체결금액", "매매금액", "거래대금", "매수원금"])
    account_col = _v5283_pick_col(src, ["계좌", "운용사", "증권사"])
    memo_col = _v5283_pick_col(src, ["비고", "메모", "투자메모", "자동분석"])

    # 위치 기반 보조: 원장 기본 배열은 날짜/구분/수량/단가/금액 순서인 경우가 많습니다.
    if qty_col is None and type_col is not None:
        qty_col = _v5283_col_after(src, type_col, 1)
    if price_col is None and type_col is not None:
        price_col = _v5283_col_after(src, type_col, 2)
    if amount_col is None and type_col is not None:
        amount_col = _v5283_col_after(src, type_col, 3)
    if account_col is None and amount_col is not None:
        account_col = _v5283_col_after(src, amount_col, 1)
    if memo_col is None and account_col is not None:
        memo_col = _v5283_col_after(src, account_col, 1)

    rows = []
    for idx, r in src.iterrows():
        구분 = _v5283_text(_v5283_value(r, type_col, ""))
        if 구분 and not any(k in 구분 for k in ["매수", "매도", "배당", "입금", "출금"]):
            continue
        날짜 = _v5283_date(_v5283_value(r, date_col, ""))
        코드 = _v5283_text(_v5283_value(r, code_col, ""))
        이름 = _v5283_text(_v5283_value(r, name_col, ""))
        계좌 = _v5283_text(_v5283_value(r, account_col, ""))
        수량 = abs(_v5283_num(_v5283_value(r, qty_col, 0), 0))
        단가 = abs(_v5283_num(_v5283_value(r, price_col, 0), 0))
        금액 = abs(_v5283_num(_v5283_value(r, amount_col, 0), 0))

        # 그래도 금액/수량이 0이면 행 안의 숫자 배열에서 보조 추정합니다.
        try:
            vals = [_v5283_num(v, None) for v in list(r.values)]
            nums = [v for v in vals if v is not None and abs(v) > 0]
            if 수량 <= 0 and 단가 > 0:
                smalls = [v for v in nums if 0 < abs(v) <= 100000 and abs(v) != 단가 and abs(v) != 금액]
                if smalls:
                    수량 = float(smalls[0])
            if 금액 <= 0 and 수량 > 0 and 단가 > 0:
                금액 = 수량 * 단가
            if 금액 <= 0:
                large = [v for v in nums if abs(v) >= 10000 and abs(v) != 단가]
                if large:
                    금액 = float(max(large, key=lambda x: abs(x)))
        except Exception:
            pass

        메모 = _v5283_text(_v5283_value(r, memo_col, ""))
        if not 날짜 or (not 이름 and not 코드 and 금액 <= 0):
            continue
        상세 = f"{계좌 + ' ' if 계좌 else ''}{이름 or 코드} {구분 or '거래'}".strip()
        if "매수" in 구분 and 계좌:
            자동 = f"{계좌} 예수금에서 {이름 or 코드} 매수로 이동"
        elif "매도" in 구분:
            자동 = "거래원장 기준 매도 거래 반영"
...
def _v5283_trade_exact_key(row):
    try:
        return "|".join([
            _v5283_text(row.get("날짜", ""))[:10],
            _v5283_text(row.get("계좌", "")),
            _v5283_text(row.get("구분", "")),
            _v5283_text(row.get("종목코드", "")),
            _v5283_text(row.get("종목명", "")),
            str(int(round(abs(_v5283_num(row.get("수량", 0), 0))))),
            str(int(round(abs(_v5283_num(row.get("단가", 0), 0))))),
            str(int(round(abs(_v5283_num(row.get("금액", 0), 0))))),
            str(int(round(abs(_v5283_num(row.get("원장행번호", 0), 0))))),
        ])
    except Exception:
        return ""


def 최근자산변화_행유형판정_v5283(row):
    try:
        구분 = _v5283_text(row.get("구분", ""))
        text = " ".join([구분, _v5283_text(row.get("상세설명", "")), _v5283_text(row.get("자동분석", "")), _v5283_text(row.get("출처", "")), _v5283_text(row.get("자산유형", ""))])
        if "v5283_거래원장" in text:
            return "실거래"
        if any(k in 구분 for k in ["매수", "매도", "배당", "입금", "출금"]):
            return "실거래"
        if any(k in text for k in ["잔액", "보유", "설명행", "현재잔액"]):
        ...
        if any(k in text for k in ["자산이동", "이체", "대기자산", "현금성", "예수금", "매도대금"]):
            return "자산이동"
        return "실거래" if abs(_v5283_num(row.get("금액", 0), 0)) > 0 else "설명행"
    except Exception:
        return "미분류"


def 최근자산변화_정리_v5283(이동df):
    try:
        df = pd.DataFrame(이동df).copy()
    except Exception:
        return pd.DataFrame()
    ...
    for c in ["금액", "원금부분", "수익손실부분", "수량", "단가", "원장행번호"]:
        if c not in df.columns:
            df[c] = 0
        df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0)
    df["날짜"] = df["날짜"].apply(_v5283_date)
    df["원금"] = df["원금부분"]
    df["실현손익"] = df["수익손실부분"]
    df["행유형"] = df.apply(최근자산변화_행유형판정_v5283, axis=1)
    try:
        df["_dt_v5283"] = pd.to_datetime(df["날짜"], errors="coerce")
        df["_exact_v5283"] = df.apply(_v5283_trade_exact_key, axis=1)
        df = df.sort_values(["_dt_v5283", "원장행번호", "금액"], ascending=[False, False, False], kind="mergesort")
        df = df.drop_duplicates("_exact_v5283", keep="first")
        df = df.drop(columns=["_dt_v5283", "_exact_v5283"], errors="ignore").reset_index(drop=True)
    except Exception:
        pass
    try:
        st.session_state["v5283_recent_total_rows"] = int(len(df))
        st.session_state["v5283_recent_type_counts"] = df["행유형"].value_counts().to_dict()
        st.session_state["v5283_recent_realized_sum"] = int(round(pd.to_numeric(df["수익손실부분"], errors="coerce").fillna(0).sum()))
    except Exception:
        pass
    return df


def _v5283_merge_legacy_and_ledger(base, 거래df):
    try:
        base_df = pd.DataFrame(base).copy()
    except Exception:
        base_df = pd.DataFrame()
    ledger_rows = _v5283_trade_rows_from_ledger(거래df)

    # 핵심 수정: 원장 거래행을 신뢰 기준으로 삼고, 기존 래퍼의 거래성 행은 전부 제거합니다.
    # 그래야 50건 거래원장 + 설명행만 남고, v5.28.2처럼 99건 실거래 중복이 생기지 않습니다.
    try:
        if not base_df.empty:
            구분시리즈 = base_df.get("구분", pd.Series([""] * len(base_df))).astype(str)
            출처시리즈 = base_df.get("출처", pd.Series([""] * len(base_df))).astype(str)
            상세시리즈 = base_df.get("상세설명", pd.Series([""] * len(base_df))).astype(str)
            trade_mask = 구분시리즈.str.contains("매수|매도|배당|입금|출금", na=False)
            ledger_like_mask = 출처시리즈.str.contains("거래원장", na=False) | 상세시리즈.str.contains("매수|매도", na=False)
            base_df = base_df.loc[~(trade_mask | ledger_like_mask)].copy()
    except Exception:
        pass

    try:
        merged = pd.concat([base_df, ledger_rows], ignore_index=True, sort=False) if not ledger_rows.empty else base_df
    except Exception:
        merged = ledger_rows if base_df.empty else base_df
    return 최근자산변화_정리_v5283(merged)


def 최근자산변화_생성_v5283(거래df=None, 비주식자산df=None, 최근일수=3650):
    try:
        if _자산이동목록통합_v5283_legacy_base:
            base = _자산이동목록통합_v5283_legacy_base(거래df, 비주식자산df, 최근일수=최근일수)
        else:
            base = pd.DataFrame()
    except Exception as e:
        try:
            logging.warning("v5283 legacy movement failed: %s", e, exc_info=True)
        except Exception:
            pass
        base = pd.DataFrame()
    out = _v5283_merge_legacy_and_ledger(base, 거래df)
    try:
        ledger_rows = _v5283_trade_rows_from_ledger(거래df)
        st.session_state["v5283_ledger_trade_rows"] = int(len(ledger_rows))
        st.session_state["v5283_recent_rows_after_merge"] = int(len(out))
        out_keys = set(out.apply(_v5283_trade_exact_key, axis=1).tolist()) if not out.empty else set()
        missing = ledger_rows[~ledger_rows.apply(_v5283_trade_exact_key, axis=1).isin(out_keys)] if not ledger_rows.empty else pd.DataFrame()
        st.session_state["v5283_recent_missing_ledger_rows"] = int(len(missing))
    except Exception:
        pass
    return out


def 최근자산변화_진단패널_v5283(df=None):
    try:
        dfx = 최근자산변화_정리_v5283(df) if df is not None else pd.DataFrame()
        total = len(dfx) if not dfx.empty else int(st.session_state.get("v5283_recent_total_rows", 0))
        counts = dfx["행유형"].value_counts().to_dict() if not dfx.empty and "행유형" in dfx.columns else st.session_state.get("v5283_recent_type_counts", {})
        realized = int(round(pd.to_numeric(dfx.get("수익손실부분", 0), errors="coerce").fillna(0).sum())) if not dfx.empty else int(st.session_state.get("v5283_recent_realized_sum", 0))
        with st.expander("최근자산변화 진단 v5.28.3", expanded=False):
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("전체", f"{int(total):,}건")
            c2.metric("실거래", f"{int(counts.get('실거래', 0)):,}건")
            c3.metric("자산이동", f"{int(counts.get('자산이동', 0)):,}건")
            c4.metric("설명행", f"{int(counts.get('설명행', 0)):,}건")
            d1, d2, d3 = st.columns(3)
            d1.metric("원장 거래행", f"{int(st.session_state.get('v5283_ledger_trade_rows', 0)):,}건")
            d2.metric("최근자산변화", f"{int(st.session_state.get('v5283_recent_rows_after_merge', 0)):,}건")
            d3.metric("원장 누락", f"{int(st.session_state.get('v5283_recent_missing_ledger_rows', 0)):,}건")
            st.caption(f"표시목록 기준 실현손익 합계: {int(realized):,}원")
    except Exception:
        pass


def 최근자산변화_표시_v5283(이동df, 최대표시=80):
    try:
        df = 최근자산변화_정리_v5283(이동df)
        st.markdown("### 🔎 최근 자산변화")
        최근자산변화_진단패널_v5283(df)
        if df.empty:
            st.caption("최근 자산변화로 표시할 내역이 없습니다.")
            return df
        총건수 = len(df)
        ...
        c2.metric("이동금액", 원화정수포맷(총금액) if "원화정수포맷" in globals() else f"{총금액:,}원")
        c3.metric("실현손익", 원화정수포맷(총손익) if "원화정수포맷" in globals() else f"{총손익:,}원")
        표시열 = [c for c in ["날짜", "행유형", "구분", "상세설명", "금액", "원금", "실현손익", "계좌", "종목코드", "종목명", "수량", "단가", "원장행번호", "자동분석", "출처"] if c in df.columns]
        표시 = df[표시열].head(max(최대표시, 80)).copy()
        for c in ["금액", "원금", "실현손익", "수량", "단가"]:
            if c in 표시.columns:
                표시[c] = pd.to_numeric(표시[c], errors="coerce").fillna(0)
        ...
        try:
            st.caption(f"최근자산변화 v5.28.3 표시 오류: {type(e).__name__}: {e}")
        except Exception:
            pass
        return pd.DataFrame()
...
def 최근자산변화_표시_v52811(이동df, 최대표시=80):
    try:
        df = _v52811_safe_df(이동df)
        st.markdown("### 🔎 최근 자산변화")
        if df.empty:
            st.caption("최근 자산변화로 표시할 내역이 없습니다.")
            return df

        최근자산변화_진단패널_v52811(df)
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("실거래", f"{trade_rows:,}건")
        c2.metric("설명행", f"{explain_rows:,}건")
        c3.metric("거래형 실현손익", _v5288_money_fmt(realized_sum, signed=True))
        c4.metric("원장 기준 전체 실현손익", _v5288_money_fmt(ledger_realized, signed=True))
        st.caption(
        ...
        try:
            sty = 표시.style.map(lambda v: _v5288_signed_money_style(v), subset=["실현손익"])
        except Exception:
            try:
        ...
        try:
            st.caption(f"최근자산변화 v5.28.11 표시 오류: {type(e).__name__}: {e}")
        except Exception:
            pass
...
# ------------------------------------------------------------
# 목적:
# - 포트폴리오 화면의 자산변동추이UI 호출 직전에 최근자산변화/현금성이동 UI 경로를 단일화합니다.
# - 과거 v5.22~v5.29 계열 UI 함수가 서로 덮어쓰는 문제를 차단합니다.
...
_v5311_original_recent_generator = globals().get("최근자산변화_생성_v5289")
_v5311_original_recent_display = (
    globals().get("최근자산변화_표시_v5297")
    or globals().get("최근자산변화_표시_v5296")
    or globals().get("최근자산변화_표시_v529")
    or globals().get("최근자산변화_표시_v52811")
    or globals().get("최근자산변화_표시_v5289")
)
...
_v5289_recent_cash_flow_card = v5311_현금성이동UI단일진입점
_v52811_recent_cash_flow_card = v5311_현금성이동UI단일진입점
_v52812_recent_cash_flow_card = v5311_현금성이동UI단일진입점
_v529_latest_cash_flow_card = v5311_현금성이동UI단일진입점
...
def v5311_최근자산변화표시단일진입점(이동df, 최대표시=100, *args, **kwargs):
    ...
    try:
        ...
        표시건수 = 100
    try:
        if callable(_v5311_original_recent_display):
            return _v5311_original_recent_display(이동df, 최대표시=표시건수)
    except Exception as e:
        try:
            st.caption(f"최근 자산변화 표시 오류 v5.31.1: {type(e).__name__}: {e}")
        except Exception:
            pass
    try:
        return pd.DataFrame(이동df)
...
def v5311_최근자산변화생성단일진입점(거래df=None, 비주식자산df=None, 최근일수=3650, *args, **kwargs):
    """최근자산변화 생성 경로 단일 진입점.

    생성 로직은 기존 검증 함수에 위임합니다. 이 함수 자체는 계산 공식을 바꾸지 않습니다.
    """
    try:
        if callable(_v5311_original_recent_generator):
            return _v5311_original_recent_generator(거래df, 비주식자산df, 최근일수=최근일수)
    except Exception as e:
        try:
            st.caption(f"최근 자산변화 생성 오류 v5.31.1: {type(e).__name__}: {e}")
        except Exception:
            pass
    return pd.DataFrame()
...
def v5311_UI중복정리상태():
    ...
    try:
        rows = [
            ...
            {"함수": "_v52811_recent_cash_flow_card", "연결": getattr(_v52811_recent_cash_flow_card, "__name__", str(_v52811_recent_cash_flow_card))},
            {"함수": "_v52812_recent_cash_flow_card", "연결": getattr(_v52812_recent_cash_flow_card, "__name__", str(_v52812_recent_cash_flow_card))},
            {"함수": "_v529_latest_cash_flow_card", "연결": getattr(_v529_latest_cash_flow_card, "__name__", str(_v529_latest_cash_flow_card))},
            {"함수": "최근자산변화카드표시", "연결": getattr(최근자산변화카드표시, "__name__", str(최근자산변화카드표시))},
            {"함수": "최근자산변화표시_v5224", "연결": getattr(최근자산변화표시_v5224, "__name__", str(최근자산변화표시_v5224))},
            {"함수": "자산이동목록통합_v5225", "연결": getattr(자산이동목록통합_v5225, "__name__", str(자산이동목록통합_v5225))},
        ...
        return pd.DataFrame([{"함수": "v5311_UI중복정리상태", "연결": f"점검 오류: {type(e).__name__}: {e}"}])

# ============================================================
# end v5.31.1 UI DUPLICATE CONSOLIDATION / RENDER PATH GUARD
# ============================================================
...
if 선택섹터 == "포트폴리오 현황":
    ...
    # v5.28.3: 포트폴리오 화면에서도 시세 새로고침을 직접 실행할 수 있게 합니다.
    try:
        pf_c1, pf_c2 = st.columns([1.4, 8.6], gap="small")
        with pf_c1:
            if st.button("시세 새로고침", key="refresh_portfolio_btn_v5283", width="stretch"):
                try:
                    시세관련캐시초기화()
                except Exception:
                    pass
                st.session_state["monitor_realtime_mode_v1"] = True
                st.session_state["manual_price_refresh_ts_v1"] = 서울현재시각ISO() if "서울현재시각ISO" in globals() else str(datetime.now())
                st.session_state["price_refresh_token_v51"] = st.session_state.get("price_refresh_token_v51", 0) + 1
                st.session_state["price_snapshot_map_v1"] = {}
                st.session_state["price_snapshot_token_v1"] = st.session_state["price_refresh_token_v51"]
                for _k in ["portfolio_cache_key_v1", "portfolio_cache_df_v1", "portfolio_holding_cache_df_v1"]:
                    try:
        ...
        with pf_c2:
            try:
                st.caption(f"현재 시세 토큰: {st.session_state.get('price_refresh_token_v51', 0)} · 버튼 클릭 시 포트폴리오 평가금액을 다시 계산합니다.")
            except Exception:
                pass
    except Exception:
        pass
    ...
    if 계산포트폴리오.empty:
        ...
        # ④ 통합자산 (IRP 포함) — 먼저 계산
        # v5.29.1: 현금성자산은 투자상품이 아니라 현재 잔액입니다.
        # 따라서 통합자산 계산 직전에 예수금·현금성 대기자산의 원금과 평가금액을 현재잔액으로 재정규화합니다.
        def _v5291_cash_balance_normalize(df):
            try:
                작업 = pd.DataFrame() if df is None else pd.DataFrame(df).copy()
                if 작업.empty:
                    return 작업
                if not {"원금", "평가금액"}.issubset(set(작업.columns)):
                    return 작업
                텍스트 = pd.Series("", index=작업.index, dtype="object")
                for 열 in ["자산군", "상품명", "유형", "계좌", "비고", "메모"]:
                    if 열 in 작업.columns:
                        텍스트 = 텍스트 + " " + 작업[열].astype(str)
                현금마스크 = 텍스트.str.contains("현금|예수금|대기자산|CMA|MMDA|MMF|입출금", case=False, na=False)
                if 현금마스크.any():
                    원금값 = pd.to_numeric(작업.loc[현금마스크, "원금"], errors="coerce").fillna(0)
                    평가값 = pd.to_numeric(작업.loc[현금마스크, "평가금액"], errors="coerce").fillna(0)
                    현재잔액 = 평가값.where(평가값 > 0, 원금값)
                    작업.loc[현금마스크, "원금"] = 현재잔액
                    작업.loc[현금마스크, "평가금액"] = 현재잔액
                    if "평가손익" in 작업.columns:
                        작업.loc[현금마스크, "평가손익"] = 0
                    if "수익률" in 작업.columns:
                        작업.loc[현금마스크, "수익률"] = 0
                return 작업
            except Exception as e:
        ...
        통합자산표 = _v5291_cash_balance_normalize(통합자산표)
...
def _v5284_num(value, default=0.0):
    try:
        if value is None:
            return default
        if isinstance(value, str):
...
def _v5284_date(value):
    try:
        ...
        pass
    return _v5284_text(value)


def _v5284_pick_col(df, candidates):
    try:
        norm = {str(c).strip().replace(" ", ""): c for c in df.columns}
        for c in candidates:
            key = str(c).strip().replace(" ", "")
...
def _v5284_col_after(df, base_col, offset):
    try:
        cols = list(df.columns)
        if base_col in cols:
            idx = cols.index(base_col) + offset
...
def _v5284_value(row, col, default=""):
    try:
        if col is not None and col in row:
            return row.get(col, default)
    except Exception:
...
def _v5284_realized_detail_map(거래df):
    """회계검증 상세 실현손익을 최근자산변화 매도행에 붙이기 위한 보조 맵."""
    m = {}
    total = None
    try:
        if "_v5269_ledger_detail_total" in globals():
            detail, total = _v5269_ledger_detail_total(거래df)
        elif "v5260_거래원장실현손익계산" in globals():
            detail, _summary, total = v5260_거래원장실현손익계산(거래df, include_manual_tdf=True)
        else:
            detail = pd.DataFrame()
        d = pd.DataFrame(detail).copy()
        if total is not None:
            try:
                st.session_state["v5284_ledger_realized_total"] = int(round(_v5284_num(total, 0)))
                st.session_state["v5269_ledger_realized_total"] = int(round(_v5284_num(total, 0)))
            except Exception:
                pass
        if d.empty:
            return m
        for _, r in d.iterrows():
            date = _v5284_date(r.get("거래일자", r.get("날짜", "")))
            code = _v5284_text(r.get("종목코드", ""))
            name = _v5284_text(r.get("종목명", ""))
            sell_amt = int(round(abs(_v5284_num(r.get("매도금액", 0), 0))))
            cost = int(round(abs(_v5284_num(r.get("매수원금", 0), 0))))
            pnl = int(round(_v5284_num(r.get("실현손익", 0), 0)))
            key1 = (date, code, sell_amt)
            key2 = (date, name, sell_amt)
            val = {"원금부분": cost, "수익손실부분": pnl, "매도금액": sell_amt}
            if sell_amt:
                m[key1] = val
                m[key2] = val
    except Exception as e:
        try:
            logging.warning("v5284 realized detail map failed: %s", e, exc_info=True)
        except Exception:
            pass
    return m


def _v5284_trade_rows_from_ledger(거래df):
    try:
        src = pd.DataFrame(거래df).copy()
    except Exception:
        return pd.DataFrame()
    ...
    date_col = _v5284_pick_col(src, ["거래일자", "거래일", "날짜", "일자", "매매일자"])
    code_col = _v5284_pick_col(src, ["종목코드", "코드", "ticker", "symbol"])
    name_col = _v5284_pick_col(src, ["종목명", "상품명", "자산명", "보유종목"])
    type_col = _v5284_pick_col(src, ["거래구분", "구분", "매매구분", "유형"])
    qty_col = _v5284_pick_col(src, ["수량", "매수수량", "매도수량", "거래수량", "체결수량", "주수"])
    price_col = _v5284_pick_col(src, ["단가", "매수단가", "매도단가", "거래단가", "체결단가", "평균매입단가", "가격"])
    amount_col = _v5284_pick_col(src, ["금액", "거래금액", "매수금액", "매도금액", "매입금액", "체결금액", "매매금액", "거래대금", "매수원금"])
    account_col = _v5284_pick_col(src, ["계좌", "운용사", "증권사"])
    memo_col = _v5284_pick_col(src, ["비고", "메모", "투자메모", "자동분석"])

    # 기본 거래원장 배열 보조 추출: 종목코드/종목명/날짜/구분/수량/단가/금액/계좌/비고
    if qty_col is None and type_col is not None:
        qty_col = _v5284_col_after(src, type_col, 1)
    if price_col is None and type_col is not None:
        price_col = _v5284_col_after(src, type_col, 2)
    if amount_col is None and type_col is not None:
        amount_col = _v5284_col_after(src, type_col, 3)
    if account_col is None and amount_col is not None:
        account_col = _v5284_col_after(src, amount_col, 1)
    if memo_col is None and account_col is not None:
        memo_col = _v5284_col_after(src, account_col, 1)

    realized_map = _v5284_realized_detail_map(거래df)
    rows = []
    for idx, r in src.iterrows():
        구분 = _v5284_text(_v5284_value(r, type_col, ""))
        if 구분 and not any(k in 구분 for k in ["매수", "매도", "배당", "입금", "출금"]):
            continue
        날짜 = _v5284_date(_v5284_value(r, date_col, ""))
        코드 = _v5284_text(_v5284_value(r, code_col, ""))
        이름 = _v5284_text(_v5284_value(r, name_col, ""))
        계좌 = _v5284_text(_v5284_value(r, account_col, ""))
        수량 = abs(_v5284_num(_v5284_value(r, qty_col, 0), 0))
        단가 = abs(_v5284_num(_v5284_value(r, price_col, 0), 0))
        금액 = abs(_v5284_num(_v5284_value(r, amount_col, 0), 0))

        if 금액 <= 0 and 수량 > 0 and 단가 > 0:
            금액 = 수량 * 단가

        메모 = _v5284_text(_v5284_value(r, memo_col, ""))
        if not 날짜 or (not 이름 and not 코드 and 금액 <= 0):
            continue
        ...
        손익부분 = 0
        if "매도" in 구분:
            key1 = (날짜, 코드, int(round(금액)))
            key2 = (날짜, 이름, int(round(금액)))
            realized = realized_map.get(key1) or realized_map.get(key2)
            if realized:
                원금부분 = realized.get("원금부분", 0)
                손익부분 = realized.get("수익손실부분", 0)
...
def _v5284_explain_rows_from_nonstock(비주식자산df):
    """비주식/현금성 자산 상태 설명행. 거래행이 아니므로 거래건수에는 섞지 않습니다."""
    try:
        src = pd.DataFrame(비주식자산df).copy()
    except Exception:
        return pd.DataFrame()
    if src.empty:
        return pd.DataFrame()
    account_col = _v5284_pick_col(src, ["계좌"])
    group_col = _v5284_pick_col(src, ["자산군"])
    name_col = _v5284_pick_col(src, ["상품명", "자산명", "종목명"])
    principal_col = _v5284_pick_col(src, ["원금", "투자원금"])
    value_col = _v5284_pick_col(src, ["평가금액", "평가액"])
    date_col = _v5284_pick_col(src, ["반영일자", "날짜", "기준일"])
    memo_col = _v5284_pick_col(src, ["비고", "메모"])
    rows = []
    for _, r in src.iterrows():
        자산군 = _v5284_text(_v5284_value(r, group_col, ""))
        상품명 = _v5284_text(_v5284_value(r, name_col, ""))
        if not any(k in f"{자산군} {상품명}" for k in ["현금", "예수", "대기"]):
            continue
        계좌 = _v5284_text(_v5284_value(r, account_col, ""))
        원금 = _v5284_num(_v5284_value(r, principal_col, 0), 0)
        평가 = _v5284_num(_v5284_value(r, value_col, 원금), 원금)
        날짜 = _v5284_date(_v5284_value(r, date_col, ""))
        메모 = _v5284_text(_v5284_value(r, memo_col, ""))
        금액 = 평가 if 평가 else 원금
        if 금액 <= 0:
        ...
        rows.append({
            ...
            "원금부분": 원금,
            "수익손실부분": 0,
            "상세설명": f"{상품명 or 자산군} 보유",
            "자동분석": 메모 or "현금성 자산 상태 설명행입니다. 실제 거래건수에는 포함하지 않습니다.",
            "출처": "v5284_비주식자산",
            "원장행번호": 0,
        })
    return pd.DataFrame(rows)
...
def _v5284_exact_key(row):
    try:
        return "|".join([
            _v5284_text(row.get("날짜", ""))[:10],
            _v5284_text(row.get("계좌", "")),
            _v5284_text(row.get("구분", "")),
            _v5284_text(row.get("종목코드", "")),
            _v5284_text(row.get("종목명", "")),
            str(int(round(abs(_v5284_num(row.get("수량", 0), 0))))),
            str(int(round(abs(_v5284_num(row.get("단가", 0), 0))))),
            str(int(round(abs(_v5284_num(row.get("금액", 0), 0))))),
            str(int(round(abs(_v5284_num(row.get("원장행번호", 0), 0))))),
        ])
    except Exception:
        return ""


def 최근자산변화_행유형판정_v5284(row):
    try:
        출처 = _v5284_text(row.get("출처", ""))
        구분 = _v5284_text(row.get("구분", ""))
        text = " ".join([출처, 구분, _v5284_text(row.get("상세설명", "")), _v5284_text(row.get("자동분석", ""))])
        if "v5284_거래원장" in 출처:
            return "실거래"
        if "v5284_비주식자산" in 출처:
            return "설명행"
        if any(k in 구분 for k in ["매수", "매도", "배당", "입금", "출금"]):
            return "실거래"
        if any(k in text for k in ["잔액", "보유", "설명행", "현재잔액"]):
...
def 최근자산변화_정리_v5284(이동df):
    ...
    if df.empty:
        return df
    for c in ["날짜", "계좌", "구분", "종목코드", "종목명", "상세설명", "자동분석", "출처"]:
        if c not in df.columns:
            df[c] = ""
    for c in ["금액", "원금부분", "수익손실부분", "수량", "단가", "원장행번호"]:
        if c not in df.columns:
            df[c] = 0
        df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0)
    df["날짜"] = df["날짜"].apply(_v5284_date)
    df["원금"] = df["원금부분"]
    df["실현손익"] = df["수익손실부분"]
    df["행유형"] = df.apply(최근자산변화_행유형판정_v5284, axis=1)
    try:
        df["_dt_v5284"] = pd.to_datetime(df["날짜"], errors="coerce")
        df["_exact_v5284"] = df.apply(_v5284_exact_key, axis=1)
        df = df.sort_values(["_dt_v5284", "원장행번호", "금액"], ascending=[False, False, False], kind="mergesort")
        df = df.drop_duplicates("_exact_v5284", keep="first")
        df = df.drop(columns=["_dt_v5284", "_exact_v5284"], errors="ignore").reset_index(drop=True)
    except Exception:
        pass
    try:
        st.session_state["v5284_recent_total_rows"] = int(len(df))
        st.session_state["v5284_recent_type_counts"] = df["행유형"].value_counts().to_dict()
        st.session_state["v5284_recent_realized_sum"] = int(round(pd.to_numeric(df["수익손실부분"], errors="coerce").fillna(0).sum()))
    except Exception:
        pass
    return df


def 최근자산변화_생성_v5284(거래df=None, 비주식자산df=None, 최근일수=3650):
    ledger_rows = _v5284_trade_rows_from_ledger(거래df)
    explain_rows = _v5284_explain_rows_from_nonstock(비주식자산df)
    try:
        out = pd.concat([ledger_rows, explain_rows], ignore_index=True, sort=False)
    except Exception:
        out = ledger_rows if explain_rows.empty else explain_rows
    out = 최근자산변화_정리_v5284(out)
    try:
        st.session_state["v5284_ledger_trade_rows"] = int(len(ledger_rows))
        st.session_state["v5284_recent_rows_after_merge"] = int(len(out))
        out_keys = set(out.apply(_v5284_exact_key, axis=1).tolist()) if not out.empty else set()
        missing = ledger_rows[~ledger_rows.apply(_v5284_exact_key, axis=1).isin(out_keys)] if not ledger_rows.empty else pd.DataFrame()
        st.session_state["v5284_recent_missing_ledger_rows"] = int(len(missing))
    except Exception:
        pass
    return out


def _v5284_money_fmt(x):
    try:
        return f"{int(round(_v5284_num(x, 0))):,}원"
    except Exception:
        return _v5284_text(x)


def _v5284_int_fmt(x):
    try:
        return f"{int(round(_v5284_num(x, 0))):,}"
    except Exception:
        return _v5284_text(x)


def _v5284_number_display_df(df):
    out = pd.DataFrame(df).copy()
    for c in ["금액", "원금", "실현손익"]:
        if c in out.columns:
            out[c] = out[c].apply(_v5284_money_fmt)
    for c in ["수량", "단가", "원장행번호"]:
        if c in out.columns:
            out[c] = out[c].apply(_v5284_int_fmt)
    for c in ["종목코드", "종목명", "계좌", "출처"]:
        if c in out.columns:
            out[c] = out[c].apply(_v5284_text)
    return out


def 최근자산변화_진단패널_v5284(df=None):
    try:
        dfx = 최근자산변화_정리_v5284(df) if df is not None else pd.DataFrame()
        total = len(dfx) if not dfx.empty else int(st.session_state.get("v5284_recent_total_rows", 0))
        counts = dfx["행유형"].value_counts().to_dict() if not dfx.empty and "행유형" in dfx.columns else st.session_state.get("v5284_recent_type_counts", {})
        realized = int(st.session_state.get("v5269_ledger_realized_total", st.session_state.get("v5284_ledger_realized_total", st.session_state.get("v5284_recent_realized_sum", 0))))
        with st.expander("최근자산변화 진단 v5.28.4", expanded=False):
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("전체", f"{int(total):,}건")
            c2.metric("실거래", f"{int(counts.get('실거래', 0)):,}건")
            c3.metric("자산이동", f"{int(counts.get('자산이동', 0)):,}건")
            c4.metric("설명행", f"{int(counts.get('설명행', 0)):,}건")
            d1, d2, d3 = st.columns(3)
            d1.metric("원장 거래행", f"{int(st.session_state.get('v5284_ledger_trade_rows', 0)):,}건")
            d2.metric("최근자산변화", f"{int(st.session_state.get('v5284_recent_rows_after_merge', 0)):,}건")
            d3.metric("원장 누락", f"{int(st.session_state.get('v5284_recent_missing_ledger_rows', 0)):,}건")
            st.caption(f"원장 기준 실현손익: {int(realized):,}원")
    except Exception:
        pass


def 최근자산변화_표시_v5284(이동df, 최대표시=80):
    try:
        df = 최근자산변화_정리_v5284(이동df)
        st.markdown("### 🔎 최근 자산변화")
        최근자산변화_진단패널_v5284(df)
        if df.empty:
            st.caption("최근 자산변화로 표시할 내역이 없습니다.")
            return df
        총건수 = len(df)
        총금액 = int(round(pd.to_numeric(df.get("금액", 0), errors="coerce").fillna(0).abs().sum()))
        원장실현손익 = int(st.session_state.get("v5269_ledger_realized_total", st.session_state.get("v5284_ledger_realized_total", st.session_state.get("v5284_recent_realized_sum", 0))))
        c1, c2, c3 = st.columns(3)
        c1.metric("자산변화", f"{총건수:,}건")
        c2.metric("이동금액", _v5284_money_fmt(총금액))
        c3.metric("원장 실현손익", _v5284_money_fmt(원장실현손익))
        표시열 = [c for c in ["날짜", "행유형", "구분", "상세설명", "금액", "원금", "실현손익", "계좌", "종목코드", "종목명", "수량", "단가", "원장행번호", "자동분석", "출처"] if c in df.columns]
        표시 = _v5284_number_display_df(df[표시열].head(max(최대표시, 80)).copy())
        try:
            if "표데이터프레임" in globals():
                표데이터프레임(표시, width="stretch", hide_index=True)
            else:
        ...
        try:
            st.caption(f"최근자산변화 v5.28.4 표시 오류: {type(e).__name__}: {e}")
        except Exception:
            pass
        return pd.DataFrame()
...
def _v5285_qty_fmt(x):
    try:
        n = float(_v5284_num(x, 0)) if "_v5284_num" in globals() else float(x or 0)
        if abs(n) < 1e-9:
            return "-"
        if abs(n - round(n)) < 1e-9:
            return f"{int(round(n)):,}주"
...
def _v5285_clean_display_rows(df):
    """최근 자산변화 화면 표시용 컬럼과 숫자 표기를 정리합니다."""
    try:
        out = pd.DataFrame(df).copy()
    except Exception:
        return pd.DataFrame()
    ...
    if "수량" not in out.columns:
        out["수량"] = 0
    if "단가" not in out.columns:
        out["단가"] = 0
    if "금액" not in out.columns:
        out["금액"] = 0
    if "실현손익" not in out.columns:
        if "수익손실부분" in out.columns:
            out["실현손익"] = out["수익손실부분"]
        else:
            out["실현손익"] = 0
    if "자동분석" not in out.columns:
        out["자동분석"] = ""
    if "계좌" not in out.columns:
        out["계좌"] = ""

    # 사용자 요청 순서: 날짜 - 유형 - 구분 - 종목명 - 수량 - 단가 - 금액 - 실현손익 - 계좌 - 자동분석
    표시열 = [c for c in ["날짜", "행유형", "구분", "종목명", "수량", "단가", "금액", "실현손익", "계좌", "자동분석"] if c in out.columns]
    out = out[표시열].copy()
    ...
    for c in ["날짜", "유형", "구분", "종목명", "계좌", "자동분석"]:
        if c in out.columns:
            out[c] = out[c].apply(_v5285_text)

    if "수량" in out.columns:
        out["수량"] = out["수량"].apply(_v5285_qty_fmt)
    if "단가" in out.columns:
        out["단가"] = out["단가"].apply(_v5285_price_fmt)
    if "금액" in out.columns:
        out["금액"] = out["금액"].apply(_v5285_money_fmt)
    if "실현손익" in out.columns:
        out["실현손익"] = out["실현손익"].apply(lambda v: _v5285_money_fmt(v, dash_zero=False))

    return out


def 최근자산변화_표시_v5285(이동df, 최대표시=80):
    """v5.28.5 최근 자산변화 최종 표시 함수."""
    try:
        if "최근자산변화_정리_v5284" in globals():
            df = 최근자산변화_정리_v5284(이동df)
        else:
            df = pd.DataFrame(이동df).copy()
        ...
        try:
            if "최근자산변화_진단패널_v5284" in globals():
                최근자산변화_진단패널_v5284(df)
        except Exception:
            pass
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("자산변화", f"{총건수:,}건")
        c2.metric("이동금액", _v5285_money_fmt(총금액))
        c3.metric("원장 기준 실현손익", _v5285_money_fmt(원장실현손익))

        표시 = _v5285_clean_display_rows(df.head(max(최대표시, 80)).copy())
        ...
        try:
            st.caption(f"최근자산변화 v5.28.5 표시 오류: {type(e).__name__}: {e}")
        except Exception:
            pass
        try:
            return pd.DataFrame(이동df)
...
def _v5285_latest_grouped_cash_candidate(거래df=None):
    """최근 현금성 이동 해석용: 동일일자·동일계좌·동일종목·동일구분 거래를 합산합니다."""
    try:
        if 거래df is None:
            if "현재거래이력가져오기" in globals():
                거래df = 현재거래이력가져오기()
                ...
                return {}
        if "_v5284_trade_rows_from_ledger" not in globals():
            return {}

        rows = _v5284_trade_rows_from_ledger(거래df)
        if rows is None or rows.empty:
            return {}

        rows = rows.copy()
        rows["날짜_dt"] = pd.to_datetime(rows.get("날짜", ""), errors="coerce")
        rows = rows.dropna(subset=["날짜_dt"])
        rows = rows[rows["구분"].astype(str).str.contains("매수|매도", na=False)].copy()
        if rows.empty:
            return {}

        latest_dt = rows["날짜_dt"].max()
        latest = rows[rows["날짜_dt"] == latest_dt].copy()
        if latest.empty:
            return {}

        # 같은 날짜 안에서 가장 최근 원장행 종목/계좌/구분 그룹을 선택합니다.
        latest["원장행번호_num"] = pd.to_numeric(latest.get("원장행번호", 0), errors="coerce").fillna(0)
        last_row = latest.sort_values("원장행번호_num", ascending=False).iloc[0]
        기준계좌 = _v5285_text(last_row.get("계좌", ""))
        기준종목 = _v5285_text(last_row.get("종목명", "")) or _v5285_text(last_row.get("종목코드", ""))
        기준코드 = _v5285_text(last_row.get("종목코드", ""))
        기준구분 = _v5285_text(last_row.get("구분", ""))

        mask = (
            (latest.get("계좌", "").astype(str) == 기준계좌)
            & (latest.get("구분", "").astype(str) == 기준구분)
        )
        if 기준코드:
            mask = mask & (latest.get("종목코드", "").astype(str) == 기준코드)
        else:
            mask = mask & (latest.get("종목명", "").astype(str) == 기준종목)

        group = latest[mask].copy()
        if group.empty:
            group = pd.DataFrame([last_row])

        총금액 = int(round(pd.to_numeric(group.get("금액", 0), errors="coerce").fillna(0).sum()))
        총수량 = float(pd.to_numeric(group.get("수량", 0), errors="coerce").fillna(0).sum())
        건수 = int(len(group))
        평균단가 = int(round(총금액 / 총수량)) if 총수량 else int(round(_v5284_num(last_row.get("단가", 0), 0)))

        날짜 = pd.to_datetime(latest_dt).strftime("%Y-%m-%d")
        방향 = "매수" if "매수" in 기준구분 else "매도" if "매도" in 기준구분 else 기준구분

        if 방향 == "매수":
            설명 = f"{기준계좌} 예수금 → {기준종목} 매수"
            if 건수 > 1:
                설명 += f" {건수}건 합계"
            자동 = f"동일일자 {기준종목} 매수 {건수}건을 합산했습니다. 총수량 {_v5285_qty_fmt(총수량)}, 평균단가 {_v5285_price_fmt(평균단가)}."
        elif 방향 == "매도":
            설명 = f"{기준종목} 매도 → {기준계좌} 예수금"
            if 건수 > 1:
                설명 += f" {건수}건 합계"
            자동 = f"동일일자 {기준종목} 매도 {건수}건을 합산했습니다. 총수량 {_v5285_qty_fmt(총수량)}, 평균단가 {_v5285_price_fmt(평균단가)}."
        else:
            설명 = f"{기준종목} {기준구분}"
            자동 = "최근 거래원장 기준 현금성 자산 흐름입니다."
        ...
        try:
            logging.warning("v5285 latest grouped cash candidate failed: %s", e, exc_info=True)
        except Exception:
            pass
        return {}
...
try:
    _자산변화통합최신이동후보_v5285_base = 자산변화통합최신이동후보_v52212
except Exception:
...
def _v5286_num(x, default=0.0):
    try:
        if "_v5284_num" in globals():
            return _v5284_num(x, default)
        if x is None:
            return default
        if isinstance(x, str):
            x = x.replace(',', '').replace('원', '').replace('%', '').strip()
...
def _v5286_money_fmt(x, dash_zero=False):
    try:
        n = int(round(_v5286_num(x, 0)))
        if dash_zero and n == 0:
            return "-"
        return f"{n:,}원"
    except Exception:
        return _v5286_text(x)


def _v5286_qty_fmt(x):
    try:
        n = float(_v5286_num(x, 0))
        if abs(n) < 1e-9:
            return "-"
        if abs(n - round(n)) < 1e-9:
            return f"{int(round(n)):,}주"
...
def _v5286_trade_cash_item(row):
    구분 = _v5286_text(row.get('구분', ''))
    계좌 = _v5286_text(row.get('계좌', ''))
    종목명 = _v5286_text(row.get('종목명', '')) or _v5286_text(row.get('종목코드', ''))
    금액 = int(round(_v5286_num(row.get('금액', 0), 0)))
    수량 = _v5286_num(row.get('수량', 0), 0)
    단가 = _v5286_num(row.get('단가', 0), 0)
    날짜 = _v5286_text(row.get('날짜', ''))[:10]
    if '매수' in 구분:
        설명 = f"{계좌} 예수금 → {종목명} 매수"
        방향 = '매수'
        자동 = f"거래원장 기준 개별 매수 1건입니다. 수량 {_v5286_qty_fmt(수량)}, 단가 {_v5286_price_fmt(단가)}, 금액 {_v5286_money_fmt(금액)}."
    elif '매도' in 구분:
        설명 = f"{종목명} 매도 → {계좌} 예수금"
        방향 = '매도'
        자동 = f"거래원장 기준 개별 매도 1건입니다. 수량 {_v5286_qty_fmt(수량)}, 단가 {_v5286_price_fmt(단가)}, 금액 {_v5286_money_fmt(금액)}."
    else:
        설명 = f"{계좌} {종목명} {구분}"
        방향 = 구분 or '거래'
        자동 = f"거래원장 기준 개별 거래 1건입니다. 금액 {_v5286_money_fmt(금액)}."
    return {
        '추천사유': 방향,
        '확인금액': 금액,
        '설명': 설명,
        ...
        '거래일자': 날짜,
        '종목명': 종목명,
        '종목코드': _v5286_text(row.get('종목코드', '')),
        '거래구분': 구분,
        '방향': 방향,
        '표시구분': 방향,
        '수량표시': _v5286_qty_fmt(수량),
        '단가표시': _v5286_price_fmt(단가),
        '원장행번호': _v5286_text(row.get('원장행번호', '')),
    }


def _v5286_latest_individual_cash_candidates(거래df=None, max_items=5):
    """최근 현금성 이동 해석용: 최신 날짜 거래를 합산하지 않고 원장 행 단위로 반환합니다."""
    try:
        if 거래df is None:
            if '현재거래이력가져오기' in globals():
                거래df = 현재거래이력가져오기()
        ...
        rows['날짜_dt'] = pd.to_datetime(rows.get('날짜', ''), errors='coerce')
        rows = rows.dropna(subset=['날짜_dt'])
        rows = rows[rows.get('구분', '').astype(str).str.contains('매수|매도', na=False)].copy()
        if rows.empty:
            return []
        latest_dt = rows['날짜_dt'].max()
        latest = rows[rows['날짜_dt'] == latest_dt].copy()
        latest['원장행번호_num'] = pd.to_numeric(latest.get('원장행번호', 0), errors='coerce').fillna(0)
        latest = latest.sort_values('원장행번호_num', ascending=False).head(max_items)
        return [_v5286_trade_cash_item(r) for _, r in latest.iterrows()]
    except Exception as e:
        try:
            logging.warning('v5286 latest individual cash candidates failed: %s', e, exc_info=True)
        except Exception:
            pass
        return []
...
try:
    _자산변화통합최신이동후보_v5286_base = _자산변화통합최신이동후보_v5285_base if '_자산변화통합최신이동후보_v5285_base' in globals() else 자산변화통합최신이동후보_v52212
except Exception:
    _자산변화통합최신이동후보_v5286_base = None


def 자산변화통합최신이동후보_v52212(거래df=None, 비주식자산df=None, 최근일수=90):
    """v5.28.6: 최근 현금성 자산 이동 해석은 합산하지 않고 거래원장 행 단위로 표시합니다."""
    items = _v5286_latest_individual_cash_candidates(거래df, max_items=5)
    if items:
        first = dict(items[0])
        first['거래목록'] = items
        return first
    try:
        if _자산변화통합최신이동후보_v5286_base:
            return _자산변화통합최신이동후보_v5286_base(거래df, 비주식자산df, 최근일수=최근일수)
    except Exception:
        pass
    return {}
...
def _v5286_clean_display_rows(df):
    ...
    if '실현손익' not in out.columns:
        out['실현손익'] = out['수익손실부분'] if '수익손실부분' in out.columns else 0
    for c in ['날짜', '행유형', '구분', '종목명', '수량', '단가', '금액', '실현손익', '계좌', '자동분석']:
        if c not in out.columns:
            out[c] = 0 if c in ['수량', '단가', '금액', '실현손익'] else ''

    표시열 = ['날짜', '행유형', '구분', '종목명', '수량', '단가', '금액', '실현손익', '계좌', '자동분석']
    out = out[표시열].copy().rename(columns={'행유형': '유형'})

    for c in ['날짜', '유형', '구분', '종목명', '계좌', '자동분석']:
        out[c] = out[c].apply(_v5286_text)
    out['수량'] = out['수량'].apply(_v5286_qty_fmt)
    out['단가'] = out['단가'].apply(_v5286_price_fmt)
    out['금액'] = out['금액'].apply(_v5286_money_fmt)
    out['실현손익'] = out['실현손익'].apply(lambda v: _v5286_money_fmt(v, dash_zero=False))
    return out


def 최근자산변화_표시_v5286(이동df, 최대표시=80):
    try:
        if '최근자산변화_정리_v5284' in globals():
            df = 최근자산변화_정리_v5284(이동df)
        else:
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric('자산변화', f'{총건수:,}건')
        c2.metric('이동금액', _v5286_money_fmt(총금액))
        c3.metric('원장 기준 실현손익', _v5286_money_fmt(원장실현손익))
        표시 = _v5286_clean_display_rows(df.head(max(최대표시, 80)).copy())
        try:
            if '표데이터프레임' in globals():
                표데이터프레임(표시, width='stretch', hide_index=True)
            else:
        ...
        try:
            st.caption(f'최근자산변화 v5.28.6 표시 오류: {type(e).__name__}: {e}')
        except Exception:
            pass
        try:
            return pd.DataFrame(이동df)
...
def _v5288_num(value, default=0.0):
    try:
        if value is None:
            return default
        if isinstance(value, str):
...
def _v5288_date(value):
    try:
        ...
        pass
    return _v5288_text(value)


def _v5288_pick_col(df, candidates):
    try:
        cols = list(df.columns)
        norm = {str(c).strip().replace(" ", ""): c for c in cols}
        for c in candidates:
            key = str(c).strip().replace(" ", "")
            if key in norm:
                return norm[key]
        for col in cols:
            sc = str(col).strip().replace(" ", "")
            if any(str(k).replace(" ", "") in sc for k in candidates):
                return col
    except Exception:
        pass
...
def _v5288_col_after(df, base_col, offset):
    try:
        cols = list(df.columns)
        if base_col in cols:
            idx = cols.index(base_col) + offset
...
def _v5288_value(row, col, default=""):
    try:
        if col is not None and col in row:
            return row.get(col, default)
    except Exception:
...
def _v5288_money_fmt(x, signed=False, dash_zero=False):
    try:
        n = int(round(_v5288_num(x, 0)))
        if dash_zero and n == 0:
            return "-"
        if signed and n > 0:
            return f"+{n:,}원"
        ...
        return _v5288_text(x)


def _v5288_qty_fmt(x):
    try:
        n = _v5288_num(x, 0)
        if abs(n) < 1e-9:
            return "-"
...
def _v5288_signed_money_style(value):
    try:
        n = _v5288_num(value, 0)
        color = "#ef4444" if n > 0 else "#3b82f6" if n < 0 else "#cbd5e1"
        return f"color:{color};font-weight:800;"
    except Exception:
        return ""


def _v5288_get_realized_detail(거래df):
    try:
        if "v5260_거래원장실현손익계산" in globals():
            detail, _summary, total = v5260_거래원장실현손익계산(거래df, include_manual_tdf=True)
            d = pd.DataFrame(detail).copy()
            total_int = 0
            try:
                if total is not None and not pd.DataFrame(total).empty and "실현손익" in pd.DataFrame(total).columns:
                    total_int = int(round(_v5288_num(pd.DataFrame(total)["실현손익"].iloc[0], 0)))
                elif not d.empty and "실현손익" in d.columns:
                    total_int = int(round(pd.to_numeric(d["실현손익"], errors="coerce").fillna(0).sum()))
            except Exception:
                total_int = 0
            try:
                st.session_state["v5288_ledger_realized_total"] = total_int
                st.session_state["v5269_ledger_realized_total"] = total_int
                st.session_state["v5284_ledger_realized_total"] = total_int
            except Exception:
                pass
            return d, total_int
    except Exception as e:
        try:
            logging.warning("v5288 realized detail failed: %s", e, exc_info=True)
        except Exception:
            pass
    return pd.DataFrame(), 0


def _v5288_norm_code(code, name=""):
    try:
        if "normalize_asset_code_v518" in globals():
            return _v5288_text(normalize_asset_code_v518(code, name))
    except Exception:
        pass
    return _v5288_text(code)


def _v5288_build_ledger_rows(거래df):
    try:
        src = pd.DataFrame(거래df).copy()
    except Exception:
    ...
    date_col = _v5288_pick_col(src, ["거래일자", "거래일", "날짜", "일자", "매매일자"])
    code_col = _v5288_pick_col(src, ["종목코드", "코드", "ticker", "symbol"])
    name_col = _v5288_pick_col(src, ["종목명", "상품명", "자산명", "보유종목"])
    type_col = _v5288_pick_col(src, ["거래구분", "구분", "매매구분", "유형"])
    qty_col = _v5288_pick_col(src, ["수량", "매수수량", "매도수량", "거래수량", "체결수량", "주수"])
    price_col = _v5288_pick_col(src, ["단가", "매수단가", "매도단가", "거래단가", "체결단가", "평균매입단가", "가격"])
    amount_col = _v5288_pick_col(src, ["금액", "거래금액", "매수금액", "매도금액", "매입금액", "체결금액", "매매금액", "거래대금", "매수원금"])
    account_col = _v5288_pick_col(src, ["계좌", "운용사", "증권사"])
    memo_col = _v5288_pick_col(src, ["비고", "메모", "투자메모", "자동분석"])

    if qty_col is None and type_col is not None:
        qty_col = _v5288_col_after(src, type_col, 1)
    if price_col is None and type_col is not None:
        price_col = _v5288_col_after(src, type_col, 2)
    if amount_col is None and type_col is not None:
        amount_col = _v5288_col_after(src, type_col, 3)
    if account_col is None and amount_col is not None:
        account_col = _v5288_col_after(src, amount_col, 1)
    if memo_col is None and account_col is not None:
        memo_col = _v5288_col_after(src, account_col, 1)

    detail, ledger_total = _v5288_get_realized_detail(거래df)
    detail_records = []
    try:
        if not detail.empty:
            for didx, dr in detail.iterrows():
                ddate = _v5288_date(dr.get("거래일자", dr.get("날짜", "")))
                dcode = _v5288_norm_code(dr.get("종목코드", ""), dr.get("종목명", ""))
                dname = _v5288_text(dr.get("종목명", ""))
                dqty = abs(_v5288_num(dr.get("매도수량", 0), 0))
                dprice = abs(_v5288_num(dr.get("매도단가", 0), 0))
                damt = int(round(abs(_v5288_num(dr.get("매도금액", 0), 0))))
                detail_records.append({
                    ...
                    "단가": dprice,
                    "매도금액": damt,
                    "매수원금": int(round(abs(_v5288_num(dr.get("매수원금", 0), 0)))),
                    "실현손익": int(round(_v5288_num(dr.get("실현손익", 0), 0))),
                    "used": False,
                })
    except Exception:
        detail_records = []
    ...
    rows = []
    for idx, r in src.iterrows():
        구분 = _v5288_text(_v5288_value(r, type_col, ""))
        if 구분 and not any(k in 구분 for k in ["매수", "매도", "배당", "입금", "출금"]):
            continue
        날짜 = _v5288_date(_v5288_value(r, date_col, ""))
        이름 = _v5288_text(_v5288_value(r, name_col, ""))
        코드 = _v5288_norm_code(_v5288_value(r, code_col, ""), 이름)
        계좌 = _v5288_text(_v5288_value(r, account_col, ""))
        수량 = abs(_v5288_num(_v5288_value(r, qty_col, 0), 0))
        단가 = abs(_v5288_num(_v5288_value(r, price_col, 0), 0))
        금액 = abs(_v5288_num(_v5288_value(r, amount_col, 0), 0))
        if 금액 <= 0 and 수량 > 0 and 단가 > 0:
            금액 = 수량 * 단가
        메모 = _v5288_text(_v5288_value(r, memo_col, ""))
        if not 날짜 or (not 이름 and not 코드 and 금액 <= 0):
            continue
        ...
        if "매도" in 구분:
            ...
            best_score = -1
            amt_i = int(round(금액))
            for j, rec in enumerate(detail_records):
                if rec.get("used"):
                    continue
                score = 0
                if rec["날짜"] == 날짜:
                    score += 30
                if rec["종목코드"] and 코드 and rec["종목코드"] == 코드:
                    score += 30
                if rec["종목명"] and 이름 and rec["종목명"] == 이름:
                    score += 20
                if rec["매도금액"] == amt_i:
                    score += 30
                if rec["수량"] and abs(rec["수량"] - 수량) < 1e-6:
                    score += 10
                if rec["단가"] and abs(rec["단가"] - 단가) < 1e-6:
                    score += 10
                # 최소한 날짜+금액 또는 종목+금액이 맞아야 같은 매도 거래로 인정합니다.
                if score > best_score and score >= 60:
                    best_score = score
                    best_i = j
            if best_i is not None:
                rec = detail_records[best_i]
                detail_records[best_i]["used"] = True
                원금부분 = rec["매수원금"]
                손익부분 = rec["실현손익"]
                매칭상태 = "실현손익 연결"
            else:
        ...
        상세 = f"{이름 or 코드} {구분 or '거래'}".strip()
        if "매수" in 구분 and 계좌:
            자동 = f"{계좌} 예수금에서 {이름 or 코드} 매수로 이동"
        elif "매도" in 구분:
            자동 = f"거래원장 기준 매도 거래 반영"
            if 매칭상태:
                자동 += f" · {매칭상태}"
        else:
            자동 = "거래원장 기준 실제 거래행 반영"
    ...
    try:
        st.session_state["v5288_unmatched_realized_rows"] = int(sum(1 for r in detail_records if not r.get("used") and abs(_v5288_num(r.get("실현손익", 0), 0)) > 0))
    except Exception:
        pass
    return df, detail, ledger_total


def _v5288_explain_rows_from_nonstock(비주식자산df):
    try:
        if "_v5284_explain_rows_from_nonstock" in globals():
            ex = _v5284_explain_rows_from_nonstock(비주식자산df)
        else:
            ex = pd.DataFrame()
        ex = pd.DataFrame(ex).copy()
        if not ex.empty:
            ex["행유형"] = "설명행"
            ex["실현손익"] = 0
            ex["원금"] = ex.get("원금부분", 0)
...
def 최근자산변화_생성_v5288(거래df=None, 비주식자산df=None, 최근일수=3650):
    ledger_rows, detail, ledger_total = _v5288_build_ledger_rows(거래df)
    explain_rows = _v5288_explain_rows_from_nonstock(비주식자산df)
    try:
        out = pd.concat([ledger_rows, explain_rows], ignore_index=True, sort=False)
    except Exception:
        out = ledger_rows.copy()
    if out.empty:
        return out
    for c in ["금액", "원금부분", "수익손실부분", "원금", "실현손익", "수량", "단가", "원장행번호"]:
        if c not in out.columns:
            out[c] = 0
        out[c] = pd.to_numeric(out[c], errors="coerce").fillna(0)
    for c in ["날짜", "행유형", "구분", "종목명", "종목코드", "계좌", "상세설명", "자동분석", "출처"]:
        if c not in out.columns:
            out[c] = ""
        out[c] = out[c].apply(_v5288_text)
    out["날짜"] = out["날짜"].apply(_v5288_date)
    try:
        out["_dt_v5288"] = pd.to_datetime(out["날짜"], errors="coerce")
        out = out.sort_values(["_dt_v5288", "원장행번호"], ascending=[False, False], kind="mergesort")
        out = out.drop(columns=["_dt_v5288"], errors="ignore").reset_index(drop=True)
    except Exception:
        pass
    try:
        trade_count = int((out["행유형"] == "실거래").sum())
        explain_count = int((out["행유형"] == "설명행").sum())
        recent_realized = int(round(pd.to_numeric(out.loc[out["행유형"] == "실거래", "실현손익"], errors="coerce").fillna(0).sum()))
        ledger_total_i = int(round(_v5288_num(ledger_total, 0)))
        st.session_state["v5288_ledger_trade_rows"] = int(len(ledger_rows))
        st.session_state["v5288_recent_total_rows"] = int(len(out))
        st.session_state["v5288_recent_trade_rows"] = trade_count
        st.session_state["v5288_recent_explain_rows"] = explain_count
        st.session_state["v5288_recent_realized_sum"] = recent_realized
        st.session_state["v5288_ledger_realized_total"] = ledger_total_i
        st.session_state["v5269_ledger_realized_total"] = ledger_total_i
    except Exception:
        pass
    return out


def _v5288_display_df(df):
    d = pd.DataFrame(df).copy()
    cols = ["날짜", "행유형", "구분", "종목명", "수량", "단가", "금액", "실현손익", "계좌", "자동분석"]
    for c in cols:
        if c not in d.columns:
            d[c] = 0 if c in ["수량", "단가", "금액", "실현손익"] else ""
    d = d[cols].copy().rename(columns={"행유형": "유형"})
    d["수량"] = d["수량"].apply(_v5288_qty_fmt)
    d["단가"] = d["단가"].apply(_v5288_price_fmt)
    d["금액"] = d["금액"].apply(_v5288_money_fmt)
    d["실현손익"] = d["실현손익"].apply(lambda x: _v5288_money_fmt(x, signed=True, dash_zero=False))
    for c in ["날짜", "유형", "구분", "종목명", "계좌", "자동분석"]:
        d[c] = d[c].apply(_v5288_text)
    return d


def 최근자산변화_진단패널_v5288(df):
    try:
        d = pd.DataFrame(df).copy()
        total = int(len(d))
        trade = int((d.get("행유형", "") == "실거래").sum()) if not d.empty else 0
        explain = int((d.get("행유형", "") == "설명행").sum()) if not d.empty else 0
        recent_realized = int(round(pd.to_numeric(d.loc[d.get("행유형", "") == "실거래", "실현손익"], errors="coerce").fillna(0).sum())) if not d.empty and "실현손익" in d.columns else 0
        ledger_realized = int(st.session_state.get("v5288_ledger_realized_total", st.session_state.get("v5269_ledger_realized_total", 0)))
        with st.expander("최근자산변화 진단 v5.28.8", expanded=False):
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("총 표시행", f"{total:,}건")
            c2.metric("실거래", f"{trade:,}건")
            c3.metric("설명행", f"{explain:,}건")
            c4.metric("미연결 손익", f"{int(st.session_state.get('v5288_unmatched_realized_rows', 0)):,}건")
            d1, d2, d3 = st.columns(3)
            d1.metric("원장 거래행", f"{int(st.session_state.get('v5288_ledger_trade_rows', 0)):,}건")
            d2.metric("최근자산변화 실현손익", _v5288_money_fmt(recent_realized, signed=True))
            d3.metric("원장 기준 실현손익", _v5288_money_fmt(ledger_realized, signed=True))
            if abs(recent_realized - ledger_realized) > 1:
                st.error(f"실현손익 불일치: 최근자산변화 {recent_realized:,}원 / 원장 {ledger_realized:,}원")
            else:
                st.success("최근자산변화 실현손익이 원장 기준 실현손익과 일치합니다.")
    except Exception:
        pass


def 최근자산변화_표시_v5288(이동df, 최대표시=80):
    try:
        df = pd.DataFrame(이동df).copy()
        if df.empty:
            st.markdown("### 🔎 최근 자산변화")
            ...
            return df
        st.markdown("### 🔎 최근 자산변화")
        최근자산변화_진단패널_v5288(df)
        trade_df = df[df.get("행유형", "") == "실거래"].copy() if "행유형" in df.columns else df.copy()
        total_rows = int(len(df))
        trade_rows = int(len(trade_df))
        total_amount = int(round(pd.to_numeric(trade_df.get("금액", 0), errors="coerce").fillna(0).abs().sum())) if not trade_df.empty else 0
        realized_sum = int(round(pd.to_numeric(trade_df.get("실현손익", 0), errors="coerce").fillna(0).sum())) if not trade_df.empty else 0
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("실거래", f"{trade_rows:,}건")
        c2.metric("총 표시행", f"{total_rows:,}건")
        c3.metric("이동금액", _v5288_money_fmt(total_amount))
        c4.metric("실현손익", _v5288_money_fmt(realized_sum, signed=True))
        표시 = _v5288_display_df(df.head(max(최대표시, 80)))
        try:
            sty = 표시.style.applymap(lambda v: _v5288_signed_money_style(v), subset=["실현손익"])
            if "표데이터프레임" in globals():
        ...
        try:
            st.caption(f"최근자산변화 v5.28.8 표시 오류: {type(e).__name__}: {e}")
        except Exception:
            pass
        try:
            return pd.DataFrame(이동df)
...
def _v5298_css():
    ...
    try:
        ...
        .v5298-ledger-oneline b{color:#f8fafc;font-weight:760;}
        .v5298-mini-chip{
            ...
            padding:.12rem .43rem;
            margin:.10rem .24rem .10rem 0;
            color:#dbeafe;
            font-size:.74rem;
            font-weight:720;
            white-space:nowrap;
        }
        .v5298-mini-chip.good{background:rgba(22,163,74,.14);border-color:rgba(74,222,128,.23);color:#bbf7d0;}
        .v5298-mini-chip.blue{background:rgba(37,99,235,.14);border-color:rgba(96,165,250,.23);color:#bfdbfe;}
        .v5298-ledger-rule{
            ...
            border-radius:11px;
            padding:.46rem .64rem;
            margin:.42rem 0 .65rem 0;
            font-size:.78rem;
            line-height:1.45;
        }

        /* 현금성 자산 이동 해석 */
        .v5298-cash-section{margin:1.0rem 0 .55rem 0;}
        .v5298-cash-title{
            font-size:1.18rem;
            font-weight:800;
            color:#f8fafc;
        ...
        .v5298-summary-box{
            ...
            background:linear-gradient(180deg,rgba(30,64,175,.18),rgba(15,23,42,.38));
            border-radius:13px;
            padding:.62rem .78rem;
            margin:.62rem 0 .78rem 0;
        }
        .v5298-summary-label{
            display:inline-flex;
            border-radius:999px;
        ...
        .v5298-system-note b{color:#93c5fd;margin-right:.28rem;font-weight:800;}
        </style>
        """, unsafe_allow_html=True)
    except Exception:
        pass
...
def _v5298_ledger_summary_html(df):
    """중복 KPI 대신 한 줄 요약만 생성합니다."""
    try:
        d = pd.DataFrame(df).copy()
        total = int(len(d))
        if "행유형" in d.columns:
            ...
            explain = 0
        recent_pnl = 0
        if "실현손익" in d.columns:
            target = d[d["행유형"] == "실거래"] if "행유형" in d.columns else d
            recent_pnl = int(round(pd.to_numeric(target["실현손익"], errors="coerce").fillna(0).sum()))
        ledger_pnl = int(st.session_state.get("v5288_ledger_realized_total", st.session_state.get("v5269_ledger_realized_total", recent_pnl)))
        diff = ledger_pnl - recent_pnl
        normal = (total == trade + explain)
        diff_note = ""
        if abs(diff) == 3690927:
            diff_note = " · TDF2035 실현손익 포함 차이"
        elif abs(diff) > 1:
            diff_note = f" · 실현손익 차이 {_v5296_money(diff, signed=True)}"
        return (
            '<div class="v5298-ledger-oneline">'
            '<b>최근자산변화 요약</b><br>'
            f'<span class="v5298-mini-chip {"good" if normal else ""}">전체 {total:,}건 = 실거래 {trade:,}건 + 잔액설명 {explain:,}건</span>'
            f'<span class="v5298-mini-chip blue">거래 실현손익 {_v5296_money(recent_pnl, signed=True)}</span>'
            f'<span class="v5298-mini-chip blue">원장 전체 실현손익 {_v5296_money(ledger_pnl, signed=True)}</span>'
            f'<span style="color:#94a3b8;font-size:.76rem;">{_v5296_html(diff_note)}</span>'
            '</div>'
        )
    except Exception:
        return ""
...
# 과거 진단 패널은 중복 노출 원인이므로 화면 출력하지 않습니다.
...
def 최근자산변화_표시_v5298(이동df, 최대표시=100):
    """v5.29.8: 최근자산변화 중복 KPI 제거 최종 표시부."""
    try:
        _v5298_css()
        df = pd.DataFrame(이동df).copy()
        st.markdown(
            '<div class="v5296-page-title">'
            '<div><div class="v5296-title-main">🔎 최근 자산변화</div>'
            '<div class="v5296-title-sub">실거래와 잔액 설명행을 구분하되, 중복 진단 지표는 한 줄 요약으로 정리합니다.</div></div>'
            '<div class="v5296-version-pill">v5.29.8 UI cleanup</div>'
            '</div>',
            unsafe_allow_html=True,
        )
        if df.empty:
        ...
        st.markdown(_v5298_ledger_summary_html(df), unsafe_allow_html=True)
        st.markdown(
            '<div class="v5298-ledger-rule"><b>표시 원칙</b> · 최근자산변화는 거래원장입니다. '
            '같은 날짜·같은 종목이라도 수량·단가·금액이 다르면 각각 별도 행으로 표시합니다. '
            '잔액 설명행은 매수/매도 실행이 아니라 거래 후 남은 현금성 잔액 반영입니다.</div>',
            unsafe_allow_html=True,
        )

        summary_html = _v5296_split_summary_html(df) if '_v5296_split_summary_html' in globals() else ''
        if summary_html:
            st.markdown(summary_html, unsafe_allow_html=True)
        ...
        try:
            st.caption(f'최근자산변화 v5.29.8 표시 오류: {type(e).__name__}: {e}')
        except Exception:
            pass
        try:
            return 최근자산변화_표시_v5297(이동df, 최대표시=최대표시)
        except Exception:
            return pd.DataFrame(이동df)


def _v5298_cash_summary_html(items):
    try:
        if not items:
            return ""
        first = items[0]
        ...
        trade_txt = []
        if buy_count:
            trade_txt.append(f'매수 {buy_count}건')
        if sell_count:
            trade_txt.append(f'매도 {sell_count}건')
        trade_txt = ' · '.join(trade_txt) if trade_txt else f'{count}건'
        return (
            '<div class="v5298-summary-box">'
            '<div class="v5298-summary-label">거래 요약</div>'
            f'<div class="v5298-summary-main">{date} · 거래 {count:,}건 · {trade_txt} · 총 거래금액 {_v5296_money(total)}</div>'
            '<div class="v5298-summary-note">이 영역은 묶음 요약입니다. 아래 건별 이동 해석에서는 수량·단가·금액이 다른 거래를 합산하지 않고 각각 따로 표시합니다.</div>'
            '</div>'
        )
    except Exception:
        return ""
...
최근자산변화_표시_v5284 = 최근자산변화_표시_v5298
최근자산변화_표시_v5286 = 최근자산변화_표시_v5298
최근자산변화_표시_v5288 = 최근자산변화_표시_v5298
최근자산변화_표시_v52810 = 최근자산변화_표시_v5298
최근자산변화_표시_v52811 = 최근자산변화_표시_v5298
최근자산변화_표시_v5293 = 최근자산변화_표시_v5298
최근자산변화_표시_v5294 = 최근자산변화_표시_v5298
최근자산변화_표시_v5295 = 최근자산변화_표시_v5298
최근자산변화_표시_v5296 = 최근자산변화_표시_v5298
