...
# [2026-09-22] v2.1.7 → v2.1.8: 한국거래소(KRX)가 2026-09-14부터 자체 애프터마켓(16:00~20:00,
# 정규장과 동일한 실시간 접속매매)을 신설하고 기존 시간외단일가(15:30~18:00) 제도를
# 폐지함에 따라, 장 마감 후 안내 배너 문구를 갱신함(is_after_krx_close 주변 주석, 아래
# render_dashboard의 배너 텍스트 참고). 시간 창(15:30~20:00) 자체는 이미 이 범위를 포괄하고
# 있어 로직(시각 판정) 변경은 불필요했고, 문구만 "NXT 전용" 프레이밍에서 "KRX 자체
# 애프터마켓 + NXT 애프터마켓 동시 운영"으로 수정함. ETF·ETN은 이 신설 애프터마켓 대상에서도
...
# [2026-08-19 추가, 2026-09-22 배경설명 갱신] 장 마감 후(15:30~20:00) 안내 배너 판단용.
# Jone 실측 확인(16:52, ETF 3종목은 정확히 일치·개별주식만 크게 벌어짐)으로 원인이
# 확정됨: 이 앱의 가격 소스는 KRX 정규장 15:30 종가에서 더 이상 안 바뀌는데, 이 시간대에
# 거래 가능한 종목은 계속 실시간으로 움직이기 때문. 아침의 프리마켓(is_before_krx_open)과
# 근본 원인이 같음 — 이 앱이 정규장 종가 이후의 실시간 시세 자체를 못 가져옴.
# [2026-09-22 추가] 애초엔 이 시간대 실시간 거래가 대체거래소 NXT(2025-03~)의 애프터마켓
# (15:40~20:00, 이후 NXT 자체 표기 기준 반영)에만 국한됐었으나, 2026-09-14부터 KRX 자체
# 애프터마켓(16:00~20:00, 정규장과 동일한 실시간 접속매매)이 신설되고 기존 시간외단일가
# (15:30~18:00) 제도는 폐지됨 — 이제 ETF·ETN을 제외한 대부분의 개별 종목이 NXT 상장 여부와
# 무관하게 이 시간대에 실시간으로 움직인다. 시간 창(15:30~20:00) 자체는 이미 이 범위를
# 포괄하므로 아래 판정 로직은 변경할 필요가 없었음 — 배너 문구만 갱신함(render_dashboard 참고).
def is_after_krx_close() -> bool:
    """지금이 KRX 정규장 마감(15:30 KST) 이후이면서 애프터마켓 종료(20:00, KRX 자체·NXT
    공통) 이전인지 여부. 20:00 이후는 양쪽 다 종료되어 다시 다음날 프리마켓까지 가격이
    고정되므로, 굳이 안내가 필요하지 않다."""
    now = datetime.now(KST)
...
def render_dashboard(holdings_df, nonstock_df, monthly_df, prices, trade_df=None, transfer_df=None):
    ...
    # [2026-08-19 추가, 2026-09-22 문구 갱신] 장 시작 전(09:00 KST 이전)/장 마감 후
    # (15:30~20:00) 안내 배너. Jone 제보 + 실측(16:52, ETF 3종목은 정규장 종가와 정확히
    # 일치·개별주식만 크게 벌어짐)으로 원인이 확정됨: 이 앱의 시세 소스(pykrx→네이버, KRX
    # 정규장 데이터)는 정규장(09:00~15:30) 밖의 시간에는 "정규장 종가"에서 더 이상 안
    # 바뀌는데, 그 시간대에 거래 가능한 종목은 실시간으로 계속 움직인다.
    # [2026-09-22] 2026-09-14부터 KRX 자체 애프터마켓(16:00~20:00, 정규장과 동일한 실시간
    # 접속매매)이 신설되고 기존 시간외단일가(15:30~18:00) 제도는 폐지됨 — 장 마감 후
    # 배너가 안내하던 "실시간으로 계속 움직이는 시간대"가 이제 NXT 상장 종목만이 아니라
    # ETF·ETN을 제외한 대부분의 개별 종목으로 넓어져 문구를 갱신함. 장 시작 전 배너는
    # 대상(NXT 프리마켓, 08:00~08:50)에 변화가 없어 문구 그대로 둠 — KRX 자체 프리마켓
    # (07:00~07:50) 도입은 2026-06-19 발표로 2027년 말로 재연기되어 아직 시행되지 않음.
    ...
    if is_before_krx_open():
        ...
        st.warning(
            ...
            "16:00~20:00 애프터마켓(2026-09-14 신설된 KRX 자체 애프터마켓 + 기존 NXT "
            "애프터마켓)의 실시간 시세는 반영되지 않습니다. "
            "(참고: ETF는 애프터마켓 거래 대상이 아니라 이 시간대에도 실제가와 대체로 일치합니다.) "
