...
def 자산변화로그추가저장(계산포트폴리오, 보유계산포트폴리오, 비주식자산df=None, 메모="", 원금변화사유="", 원금변화확인금액=0, 원금변화설명=""):
    ...
    if 새행.get("원금변화사유") in ["자산 이동", "현금→ETF/주식 매수", "ETF/주식 매도→현금"]:
        새행["변화유형"] = "자산이동"
        설명 = 새행.get("원금변화설명", "") or "현금성자산과 주식/ETF 사이의 내부 이동"
...
def 자산변화최근거래기반이동후보(로그df=None, 기준일수=14):
    ...
    try:
        ...
        if "매수" in 거래구분:
            ...
            자동분석 = f"거래이력의 {날짜문자} {종목명} {자산종류} 매수 {원화정수포맷(금액)}이 확인되었습니다. 현금성자산이 주식/ETF로 이동한 것이므로 통합원금 변화가 아니라 자산이동으로 기록하는 것이 맞습니다."
            방향 = "매수"
        elif "매도" in 거래구분:
            설명 = f"{종목명} {자산종류} 매도 → 예수금"
            자동분석 = f"거래이력의 {날짜문자} {종목명} {자산종류} 매도 {원화정수포맷(금액)}이 확인되었습니다. 주식/ETF가 현금성자산으로 이동한 것이므로 통합원금 변화가 아니라 자산이동으로 기록하는 것이 맞습니다."
            방향 = "매도"
        else:
            return {}
...
def 자산변화사유입력UI(미리보기행, 이동후보=None):
    ...
    if 이동후보 and 이동후보.get("설명"):
        ...
        기본설명 = "예수금 → 주식/ETF 매수 또는 주식/ETF 매도 → 예수금"
