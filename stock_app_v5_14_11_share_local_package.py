...
APP_VERSION = "v5.14.11 share-local"  # 개별 PC 실행용 Share 패키지 / 로컬 저장·복원 / 개인 데이터 제거
...
if 모바일여부():
    ...
    st.caption("Share 로컬 패키지 · 공개 샘플 제공 · 사용자 PC에만 저장")
else:
    st.title(f"📈 투자 분석 시스템 {APP_VERSION}")
    st.caption("Share 로컬 패키지 · 공개 샘플 제공 · 사용자 PC에만 저장")
...
def Share공개샘플거래이력표():
    """개별 실행용 Share 패키지의 공개 샘플 거래이력입니다.
    Jone님의 개인 거래이력이나 기존 자동저장 파일을 참조하지 않습니다.
    사용자는 이 형식을 참고해 본인 거래이력 엑셀을 업로드하면 됩니다.
    """
    return pd.DataFrame([
        {"종목코드": "069500", "종목명": "KODEX 200", "거래일자": "2026-01-05", "거래구분": "매수", "거래수량": 10, "거래단가": 36000, "운용사": "샘플계좌", "비고": "공개 샘플"},
        {"종목코드": "005930", "종목명": "삼성전자", "거래일자": "2026-01-08", "거래구분": "매수", "거래수량": 5, "거래단가": 72000, "운용사": "샘플계좌", "비고": "공개 샘플"},
        {"종목코드": "000660", "종목명": "SK하이닉스", "거래일자": "2026-01-10", "거래구분": "매수", "거래수량": 2, "거래단가": 185000, "운용사": "샘플계좌", "비고": "공개 샘플"},
        {"종목코드": "069500", "종목명": "KODEX 200", "거래일자": "2026-02-03", "거래구분": "매도", "거래수량": 2, "거래단가": 38500, "운용사": "샘플계좌", "비고": "공개 샘플 부분매도"},
    ], columns=거래이력표준열)


기본포트폴리오 = Share공개샘플거래이력표()
...
# -----------------------------------
# Share-local 저장 정책
# - 배포받은 각 사용자의 PC 폴더에만 저장합니다.
# - Streamlit Cloud 다중 사용자 sid 구조와 개인용 legacy 자동복원은 사용하지 않습니다.
# - 처음 실행은 공개 샘플, 업로드 후 저장하면 다음 실행부터 로컬 저장본을 복원합니다.
# -----------------------------------
SHARE_LOCAL_DATA_DIR = "share_user_data"
os.makedirs(SHARE_LOCAL_DATA_DIR, exist_ok=True)
SHARE_SESSION_ID = "local_pc"

목표비중저장파일 = os.path.join(SHARE_LOCAL_DATA_DIR, "share_target_weights.json")
거래이력자동저장파일 = os.path.join(SHARE_LOCAL_DATA_DIR, "share_trade_history_autosave.json")
최근업로드거래이력파일 = os.path.join(SHARE_LOCAL_DATA_DIR, "share_trade_history_latest_uploaded.json")
최근업로드메타파일 = os.path.join(SHARE_LOCAL_DATA_DIR, "share_trade_history_latest_uploaded_meta.json")
거래이력복원메타파일 = os.path.join(SHARE_LOCAL_DATA_DIR, "share_trade_history_restore_meta.json")

# 개별 실행용 Share 패키지는 과거 개인용/공용 JSON을 읽지 않습니다.
거래이력레거시자동저장파일목록 = []
모니터관심종목저장파일 = os.path.join(SHARE_LOCAL_DATA_DIR, "share_monitor_custom_assets.json")

IRP비주식자산저장파일 = os.path.join(SHARE_LOCAL_DATA_DIR, "share_integrated_non_stock_assets.json")
IRP비주식자산레거시저장파일목록 = []


def 기본IRP비주식자산표():
    """Share 공개용 비주식·현금성 자산 샘플입니다.
    개인 IRP·예수금 정보는 포함하지 않습니다.
    """
    return pd.DataFrame([
        {"계좌": "샘플 IRP", "자산군": "TDF", "상품명": "TDF 샘플", "원금": 10000000, "평가금액": 10300000, "예상연수익률": 3.0, "만기일": "", "반영일자": "2026-01-31", "비고": "공개 샘플"},
        {"계좌": "샘플 증권계좌", "자산군": "현금성자산", "상품명": "예수금 샘플", "원금": 3000000, "평가금액": 3000000, "예상연수익률": 0.0, "만기일": "", "반영일자": "2026-01-31", "비고": "공개 샘플"},
    ])
...
def IRP비주식자산불러오기():
    ...
    후보파일목록 = [IRP비주식자산저장파일] + list(IRP비주식자산레거시저장파일목록)
    for 후보파일 in 후보파일목록:
        if os.path.exists(후보파일):
            try:
                with open(후보파일, "r", encoding="utf-8") as f:
                    data = json.load(f)
                df = IRP비주식자산표준열맞추기(pd.DataFrame(data))
                st.session_state["irp_non_stock_assets_df_v512"] = df
                if 후보파일 != IRP비주식자산저장파일:
                    try:
                        IRP비주식자산저장(df)
                    except Exception:
                        pass
...
def 거래이력자동저장실행(df):
    저장데이터 = 거래이력JSON변환(df)
    저장성공, 저장메시지 = 안전JSON저장(저장데이터, 거래이력자동저장파일)
    if 저장성공:
        meta = 거래이력저장메타생성(df, source="autosave")
        거래이력복원메타저장(meta)
        try:
            # 중요한 실제 사용자 데이터는 자동백업 폴더에도 1회 스냅샷을 남깁니다.
            자동백업파일저장(df, backup_type="edit", reason="autosave_snapshot", source="autosave")
        except Exception:
            pass
    return 저장성공, 저장메시지
...
def 최근업로드거래이력저장(df, 파일명=""):
    ...
    메타정보 = {
        ...
        "source": "latest_uploaded",
        "app_version": APP_VERSION,
    }
    if 저장성공:
        메타성공, 메타메시지 = 안전JSON저장(메타정보, 최근업로드메타파일)
        거래이력복원메타저장(거래이력저장메타생성(df, source="latest_uploaded", file_name=파일명 or ""))
        try:
            자동백업파일저장(df, backup_type="edit", reason="upload_snapshot", source="latest_uploaded")
        except Exception:
            pass
...
def 거래이력복원후보목록생성():
    """Share-local 복원 후보 목록입니다.
    이 버전은 share_user_data 폴더의 로컬 저장 파일만 읽고,
    개인용 v5145/v5146 legacy 파일은 절대 읽지 않습니다.
    """
    후보파일목록 = [
...
def 거래이력자동복원상태문구():
    ...
    출처 = st.session_state.get("trade_history_source_v1", meta.get("source", "public_sample"))
    건수 = len(st.session_state.get("trade_history_df_v22", pd.DataFrame()))
    저장시각 = meta.get("saved_at", "")
    if 저장시각:
        return f"Share-local 복원 출처: {출처} · 현재 {건수}건 · 마지막 저장 {서울조회문자열(저장시각)}"
    return f"Share-local 복원 출처: {출처} · 현재 {건수}건"


def 현재거래이력가져오기():
    """개별 실행용 Share 패키지의 거래이력 시작 로직입니다.

    동작 원칙:
    1. 현재 Streamlit 세션에 거래이력이 있으면 그대로 사용합니다.
    2. 없으면 share_user_data 폴더의 최근 업로드/자동저장 파일을 복원합니다.
    3. 로컬 저장 파일이 없으면 코드 내부 공개 샘플로 시작합니다.
    4. 개인용 legacy JSON과 Jone님의 개인 데이터는 읽지 않습니다.
    """
    if "trade_history_df_v22" not in st.session_state:
        후보목록 = 거래이력복원후보목록생성()
        if 후보목록:
            선택후보 = 후보목록[0]
            초기df = 선택후보.get("df", pd.DataFrame()).copy()
            st.session_state["trade_history_source_v1"] = 선택후보.get("source", "local_restore")
            st.session_state["trade_history_restore_path_v1"] = 선택후보.get("path", "")
        else:
            초기df = Share공개샘플거래이력표()
        ...
        try:
            거래이력복원메타저장(거래이력저장메타생성(편집df, source=st.session_state.get("trade_history_source_v1", "public_sample")))
        except Exception:
            pass
...
with st.sidebar.expander("거래이력 관리", expanded=False):
    ...
    st.caption("처음에는 공개 샘플로 시작합니다. 본인 거래이력 파일을 업로드하고 저장하면 다음 실행부터 이 PC에서 자동 복원됩니다.")
    ...
    st.caption("Share-local 모드: 업로드한 데이터는 이 PC의 share_user_data 폴더에만 저장됩니다.")
    if st.button("공개 샘플로 초기화", key="reset_share_public_sample_v51411", use_container_width=True):
        try:
            for _파일 in [거래이력자동저장파일, 최근업로드거래이력파일, 최근업로드메타파일, 거래이력복원메타파일]:
                if os.path.exists(_파일):
                    os.remove(_파일)
        except Exception:
            pass
        거래이력세션반영(Share공개샘플거래이력표(), 저장강제=True, 자동저장허용=False)
        st.session_state["trade_history_source_v1"] = "public_sample_reset"
        st.success("공개 샘플 거래이력으로 초기화했습니다.")
        st.rerun()
