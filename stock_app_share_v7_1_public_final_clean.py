...
st.set_page_config(page_title="투자 분석 시스템 Share v7.1", layout="wide")
...
st.markdown(
    f"""
    <div style="margin-top:-0.2rem; margin-bottom:1.4rem; font-size:1.05rem; line-height:2.1; color:#d1d5db; max-width:1200px; word-break:keep-all;">
        {SHARE_안내문}
        """,
...
기본포트폴리오 = pd.DataFrame([
    {"종목코드": "069500", "종목명": "KODEX 200", "거래일자": "2026-03-05", "거래구분": "매수", "거래수량": 10, "거래단가": 33500, "운용사": "Demo Account", "비고": "샘플 데이터"},
    {"종목코드": "005930", "종목명": "삼성전자", "거래일자": "2026-03-07", "거래구분": "매수", "거래수량": 15, "거래단가": 61200, "운용사": "Demo Account", "비고": "샘플 데이터"},
    {"종목코드": "229200", "종목명": "KODEX 코스닥150", "거래일자": "2026-03-10", "거래구분": "매수", "거래수량": 8, "거래단가": 14980, "운용사": "Demo Account", "비고": "샘플 데이터"},
    {"종목코드": "069500", "종목명": "KODEX 200", "거래일자": "2026-03-18", "거래구분": "매수", "거래수량": 5, "거래단가": 34120, "운용사": "Demo Account", "비고": "추가매수 예시"},
    {"종목코드": "005930", "종목명": "삼성전자", "거래일자": "2026-03-21", "거래구분": "매도", "거래수량": 3, "거래단가": 62800, "운용사": "Demo Account", "비고": "부분매도 예시"},
])
...
목표비중저장파일 = "target_weights_share_v71.json"
거래이력자동저장파일 = "trade_history_autosave_share_v71.json"
...
def 자동판정배지HTML(판정, 실행, 강도):
    ...
    <div style="background:{배경}; border-radius:20px; padding:16px 18px; color:white; margin:8px 0 14px 0; box-shadow:0 10px 24px rgba(15,23,42,0.18);">
        <div style="font-size:0.95rem; opacity:0.9;">자동 매수·매도 판단        <div style="font-size:1.6rem; font-weight:800; margin-top:4px;">{판정}        <div style="font-size:1rem; margin-top:6px;">실행 방향: {실행}        <div style="font-size:0.92rem; margin-top:6px; opacity:0.95;">신호 강도: {강도}/100        """
...
def 대시보드스타일적용():
    ...
    .simple-market-label {{font-size: 0.77rem; font-weight: 700; color: {라벨색}; margin-bottom: 1px;}}
    .simple-market-title {{font-size: 0.92rem; font-weight: 800; color: {제목색}; margin-bottom: 6px; line-height: 1.2;}}
    .simple-market-price {{font-size: 1.10rem; font-weight: 800; color: {제목색}; line-height: 1.12;}}
    .simple-market-delta {{font-size: 0.80rem; font-weight: 700; margin-top: 6px;}}
    .simple-market-delta.up {{color: #dc2626;}}
    .simple-market-delta.down {{color: #2563eb;}}
    .simple-market-delta.flat {{color: {메타색};}}
    .simple-market-meta {{font-size: 0.74rem; color: {메타색}; margin-top: 5px;}}
...
def 지표변화HTML(지표명, 현재값, 전일대비):
    ...
    <div style="background:#020817; border:1px solid #1f2937; border-radius:18px; padding:18px 18px 14px 18px; min-height:140px;">
        <div style="font-size:0.95rem; color:#ffffff; font-weight:700; margin-bottom:8px;">{지표명}        <div style="font-size:2.1rem; color:#f8fafc; font-weight:800; line-height:1.2; margin-bottom:12px;">{현재값문자}        <div style="display:inline-block; background:rgba(15,23,42,0.65); border:1px solid {델타색}; color:{델타색}; padding:6px 12px; border-radius:999px; font-size:1rem; font-weight:700;">{화살표}{델타문자}        """
...
def 캔들유형HTML(캔들유형):
    ...
    <div style="display:flex; align-items:center; gap:10px; padding:10px 12px; border:1px solid #334155; border-radius:14px; background:#0f172a; width:fit-content;">
        <div style="font-size:1.15rem;">{아이콘}        <div style="position:relative; width:20px; height:44px;">
            <div style="position:absolute; left:9px; top:{wick_top}; width:2px; height:{wick_height}; background:{색상};">            <div style="position:absolute; left:4px; top:{top_pos}; width:12px; height:{height}; background:{몸통배경}; border:2px solid {색상}; border-radius:2px;">                <div style="font-size:1.05rem; font-weight:800; color:{색상};">{라벨}        """
...
대시보드기준거래 = st.session_state.get("trade_history_df_v22", 기본포트폴리오.copy())
대시보드기준거래 = 거래이력정규화(대시보드기준거래)
...
if 보유코드목록:
    보유종목명문자 = ", ".join([코드명매핑.get(코드, 코드) for 코드 in 보유코드목록])
...
st.markdown("---")
st.subheader("주요 지표")
st.caption("환율·금·원유 지표를 같은 방식의 간단한 카드형으로 표시합니다.")
...
st.caption("거래일자·거래구분·거래수량·거래단가·운용사를 입력하면 아래 포트폴리오 현황이 자동 집계됩니다. 종목코드 또는 종목명 중 하나를 입력하면 다른 값은 가능한 범위에서 자동 보정됩니다. CSV와 엑셀(xlsx, xls) 업로드를 지원합니다.")
...
st.markdown("<div style=\"font-size:1.05rem; line-height:1.9; margin-top:-0.35rem; margin-bottom:0.8rem; color:#d1d5db;\">" + SHARE_안내문 + "</div>", unsafe_allow_html=True)
with st.expander("Share 버전 사용 안내", expanded=False):
    st.write("1. 현재 화면은 공개용 샘플 거래이력으로 시작합니다.")
    st.write("2. 거래이력 입력창에서 행을 수정·추가하면 아래 보유종목과 포트폴리오 분석이 함께 갱신됩니다.")
    st.write("3. 업로드 파일로 교체하면 기존 샘플 대신 업로드한 거래이력을 기준으로 전체 화면이 다시 계산됩니다.")
    st.write("4. 초기 상태로 되돌리고 싶으면 아래 초기화 버튼을 사용하세요.")

초기화_col1, 초기화_col2 = st.columns([1, 1])
with 초기화_col1:
    if st.button("샘플 거래이력으로 초기화", use_container_width=True, key="reset_sample_share_v71"):
        st.session_state["trade_history_df_v22"] = 거래이력정규화(기본포트폴리오.copy())
        try:
            if os.path.exists(거래이력자동저장파일):
                os.remove(거래이력자동저장파일)
        except Exception:
            pass
        st.success("샘플 거래이력으로 초기화했습니다.")
with 초기화_col2:
    if st.button("빈 거래이력으로 시작", use_container_width=True, key="reset_empty_share_v71"):
        빈_df = pd.DataFrame(columns=["종목코드", "종목명", "거래일자", "거래구분", "거래수량", "거래단가", "운용사", "비고"])
        st.session_state["trade_history_df_v22"] = 거래이력정규화(빈_df)
        try:
            if os.path.exists(거래이력자동저장파일):
                os.remove(거래이력자동저장파일)
        except Exception:
            pass
        st.success("빈 거래이력으로 시작하도록 초기화했습니다.")
...
수정포트폴리오 = st.data_editor(
    ...
    column_config={
        ...
        "운용사": st.column_config.TextColumn("운용사", help="예: 일반계좌, 연금계좌, ISA"),
...
if 계산포트폴리오.empty:
    ...
    st.markdown("### 포트폴리오 전체 매수·매도 기록")
    전체거래표 = 종목거래이력표생성(수정포트폴리오)
    if 전체거래표.empty:
        st.info("표시할 거래기록이 없습니다.")
    else:
        st.dataframe(거래기록표시용서식(index_1부터(전체거래표)), use_container_width=True)
                ...
                with 기술차트탭1:
                    ...
                    for 기간라벨, 기간탭 in zip(["일", "주", "월", "년"], 보유기간탭들):
                        with 기간탭:
                            ...
                            st.plotly_chart(캔들차트그래프(표시기간데이터, f"{선택종목명} {기간라벨} 단위 기술적 분석"), use_container_width=True, config={"displaylogo": False, "responsive": True})
                with 기술차트탭2:
                    기술체크 = pd.DataFrame([
...
st.markdown(
    "<div style='text-align:center; color:#9ca3af;'>공개용 Share 버전 · 개인 정보 제거 · 샘플 거래이력 포함</div>",
    unsafe_allow_html=True
)
...
st.markdown(
    ...
    ">
        개발자 :
        <a href="mailto:hwcho@me.com" style="color:#93c5fd; text-decoration:none;">
            조현웅 hwcho@me.com
        </a>
    </div>
    """,
