...
APP_VERSION = "v5.14.28-cloud-ready"  # v5.14.6 UI/기능 유지 + Google Sheets 표시 형식 안정화
...
def 구글시트설정가능여부():
    ...
    try:
        ...
        if "gcp_service_account" in st.secrets:
            계정정보 = dict(st.secrets["gcp_service_account"])
...
def 구글시트문서연결():
    ...
    try:
        ...
        # Streamlit Cloud: Secrets에 서비스 계정 전체 JSON을 저장한 경우
        if "gcp_service_account" in st.secrets:
            계정정보 = dict(st.secrets["gcp_service_account"])
            if 계정정보.get("client_email") and 계정정보.get("private_key"):
                ...
                raise ValueError("[gcp_service_account]에 client_email/private_key가 없습니다.")
...
def 구글시트연결상태표시():
    ...
    if spreadsheet is not None:
        ...
        st.warning(f"Google Sheets 미연결 · 로컬 저장 fallback 사용 · {info.get('메시지', '')}")
...
def 구글시트사이드바간단표시():
    ...
    try:
        ...
        if spreadsheet is not None:
            ...
            st.caption(f"Google Sheets 미연결 · 로컬 저장 사용")
    except Exception as e:
...
def IRP비주식자산불러오기():
    """비주식자산을 Google Sheets에서 우선 불러옵니다.
    연결 실패 시 기존 로컬 JSON 구조를 fallback으로 사용해 기존 UI 호환성을 유지합니다.
    """
...
def IRP비주식자산저장(df):
    """비주식자산을 Google Sheets에 우선 저장합니다.
    연결 실패 시 기존 로컬 JSON 저장으로 fallback합니다.
    """
    ...
    로컬성공, 로컬메시지 = 안전JSON저장(저장용, IRP비주식자산저장파일)
    if 로컬성공:
        return True, f"Google Sheets 저장 실패로 로컬 저장 사용: {저장메시지}"
    return False, f"{저장메시지} / 로컬 저장도 실패: {로컬메시지}"
...
def 거래이력자동저장실행(df):
    """거래이력을 Google Sheets에 우선 저장합니다.
    연결 실패 시 기존 로컬 JSON 자동저장으로 fallback합니다.
    """
    편집df = 거래이력편집용자동보정(df)
    ...
    # fallback: 기존 로컬 JSON
    저장데이터 = 거래이력JSON변환(편집df)
    로컬성공, 로컬메시지 = 안전JSON저장(저장데이터, 거래이력자동저장파일)
    if 로컬성공:
        meta = 거래이력저장메타생성(편집df, source="local_fallback")
        거래이력복원메타저장(meta)
        try:
            자동백업파일저장(편집df, backup_type="edit", reason="autosave_snapshot", source="local_fallback")
        except Exception:
            pass
        return True, f"Google Sheets 저장 실패로 로컬 저장 사용: {저장메시지}"
    return False, f"{저장메시지} / 로컬 저장도 실패: {로컬메시지}"
...
def 최근업로드거래이력저장(df, 파일명=""):
    """업로드 거래이력도 Google Sheets에 우선 저장합니다."""
    편집df = 거래이력편집용자동보정(df)
    저장성공, 저장메시지 = 구글시트데이터프레임저장(GOOGLE_SHEETS_TRADE_SHEET, 편집df)
    메타정보 = {
        "파일명": 파일명 or "",
        ...
        "source": "google_sheets_latest_uploaded" if 저장성공 else "local_latest_uploaded",
        "app_version": APP_VERSION,
    }
    if 저장성공:
    ...
    # fallback: 기존 로컬 저장
    저장데이터 = 거래이력JSON변환(편집df)
    로컬성공, 로컬메시지 = 안전JSON저장(저장데이터, 최근업로드거래이력파일)
    if 로컬성공:
        안전JSON저장(메타정보, 최근업로드메타파일)
        거래이력복원메타저장(거래이력저장메타생성(편집df, source="local_latest_uploaded", file_name=파일명 or ""))
        try:
            자동백업파일저장(편집df, backup_type="edit", reason="upload_snapshot", source="local_latest_uploaded")
        except Exception:
            pass
        return True, f"Google Sheets 저장 실패로 로컬 저장 사용: {저장메시지}"
    return False, f"{저장메시지} / 로컬 저장도 실패: {로컬메시지}"
...
def 현재거래이력가져오기():
    """세션 상태가 없으면 Google Sheets의 거래이력 시트를 최우선으로 불러옵니다.
    기존 v5.14.6 UI와 분석 로직은 유지하고, 저장소만 Google Sheets 중심으로 전환합니다.
    Google Sheets 연결 실패 또는 시트 공란이면 기존 로컬 자동복원 후보를 fallback으로 사용합니다.
    """
    if "trade_history_df_v22" not in st.session_state:
        ...
        # fallback: 기존 로컬 자동복원 구조
        if 초기df is None or 초기df.empty:
            후보목록 = 거래이력복원후보목록생성()
            if 후보목록:
                선택후보 = 후보목록[0]
                출처 = 선택후보.get("source", "autosave")
...
with st.sidebar.expander("거래이력 관리", expanded=False):
    ...
    구글시트사이드바간단표시()
    st.caption("Google Sheets가 기본 저장소입니다. 엑셀 업로드는 초기 이관·백업 복구가 필요할 때만 사용합니다.")
