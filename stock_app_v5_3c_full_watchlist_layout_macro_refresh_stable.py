...
APP_VERSION = "v5.3"  # 전체 코드본 / 관심종목 추가 / 카드 슬림화 / 주요지표 재구성
...
모니터관심종목저장파일 = "monitor_custom_assets_v53.json"
...
def 세션모니터관심종목가져오기():
    if "monitor_custom_codes_v53" not in st.session_state:
        st.session_state["monitor_custom_codes_v53"] = 모니터관심종목불러오기()
    return list(st.session_state.get("monitor_custom_codes_v53", []))
...
def 모니터추가카드버튼(key="monitor_add_card_btn_v53", 키=None):
    실사용키 = 키 or key
    st.markdown("<div class='monitor-add-card'>", unsafe_allow_html=True)
    clicked = st.button("＋\n관심종목 추가", key=실사용키, use_container_width=True)
...
def 현재거래이력가져오기():
    """세션 상태가 아직 없더라도 최근 업로드본/자동저장/기본 거래이력을 우선순위대로 즉시 반영합니다."""
    if "trade_history_df_v22" not in st.session_state:
        ...
        elif 자동저장df is not None and not 자동저장df.empty:
            초기df = 자동저장df.copy()
            st.session_state["trade_history_source_v1"] = "autosave"
        else:
            초기df = 기본포트폴리오.copy()
...
if st.session_state.get("show_monitor_add_form_v53", False):
    st.markdown("##### 관심종목 추가")
    옵션목록 = []
    ...
    add_col1, add_col2, add_col3 = st.columns([3.1, 1.4, 1.0])
    with add_col1:
        선택표시 = st.selectbox("종목 선택", options=[""] + 옵션목록, index=0, key="monitor_add_select_v53", label_visibility="collapsed", placeholder="종목명으로 선택")
    with add_col2:
        코드입력값 = st.text_input("종목코드", key="monitor_add_code_v53", label_visibility="collapsed", placeholder="6자리 코드")
    with add_col3:
        추가클릭 = st.button("추가", key="monitor_add_submit_v53", use_container_width=True)

    close_col1, close_col2 = st.columns([4.5, 1.0])
    with close_col2:
        닫기클릭 = st.button("닫기", key="monitor_add_close_v53", use_container_width=True)

    if 추가클릭:
        선택코드 = ""
        if 선택표시:
            m = re.search(r"\((\d{6})\)$", 선택표시)
            if m:
                선택코드 = m.group(1)
        if not 선택코드 and 코드입력값:
            선택코드 = re.sub(r"[^0-9]", "", 코드입력값).zfill(6)

        현재목록 = 세션모니터관심종목가져오기()
        보유목록 = 현재보유종목코드목록(대시보드기준거래)
        if not 선택코드:
            st.warning("추가할 종목을 선택하거나 6자리 종목코드를 입력해 주세요.")
        elif 선택코드 in 보유목록:
            st.warning("이미 보유 종목 카드에 자동 반영되는 종목입니다.")
        elif 선택코드 in 현재목록:
            st.info("이미 추가된 관심종목입니다.")
        elif len(현재목록) >= 4:
            st.warning("관심종목은 최대 4개까지 추가할 수 있습니다.")
        else:
            세션모니터관심종목저장(현재목록 + [선택코드])
            시세관련캐시초기화()
            st.session_state["price_refresh_token_v51"] = st.session_state.get("price_refresh_token_v51", 0) + 1
            st.session_state["show_monitor_add_form_v53"] = False
            st.rerun()
...
관심목록 = 세션모니터관심종목가져오기()
if 관심목록:
    삭제옵션 = [f"{종목코드기준종목명(코드) or 코드명매핑.get(코드) or 코드} ({코드})" for 코드 in 관심목록]
    del_col1, del_col2 = st.columns([4, 1])
    with del_col1:
        삭제표시 = st.selectbox("관심종목 삭제", options=[""] + 삭제옵션, key="monitor_delete_select_v53", label_visibility="collapsed", placeholder="추가된 관심종목 제거")
    with del_col2:
        if st.button("삭제", key="monitor_delete_btn_v53", use_container_width=True):
            m = re.search(r"\((\d{6})\)$", 삭제표시 or "")
            if m:
                삭제코드 = m.group(1)
                세션모니터관심종목저장([x for x in 관심목록 if x != 삭제코드])
                시세관련캐시초기화()
                st.session_state["price_refresh_token_v51"] = st.session_state.get("price_refresh_token_v51", 0) + 1
                st.rerun()
...
if 거래이력변경됨:
    st.caption("입력 내용은 화면에 즉시 반영됩니다. 자동저장은 중단했고, 필요하면 상단의 저장 버튼으로 백업해 주세요.")
