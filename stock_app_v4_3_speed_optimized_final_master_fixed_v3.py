...
APP_VERSION = "v4.3"  # 다음 업데이트 시 APP_VERSION 숫자만 올리면 됩니다.

st.set_page_config(page_title=f"투자 분석 시스템 {APP_VERSION}", layout="wide")

if "price_refresh_token_v43" not in st.session_state:
    st.session_state["price_refresh_token_v43"] = 0
...
def 야후1분봉요약가져오기(심볼):
    ...
    try:
        ...
        return {
            ...
            "전일대비": 전일대비,
            "등락률": 등락률,
            "기준일": pd.to_datetime(최신행["날짜"]).date(),
...
def 실시간포함시세요약가져오기(구분, 코드, lookback_days=15, refresh_token=0):
    요약 = 최근시세요약가져오기(구분, 코드, lookback_days=lookback_days, refresh_token=refresh_token).copy()

    분봉요약 = 준실시간시세요약가져오기(구분, 코드)
    if 분봉요약 is not None and 분봉요약.get("현재가") not in [None, 0]:
        요약["현재가"] = float(분봉요약.get("현재가"))
        요약["전일가"] = 분봉요약.get("전일가")
        요약["전일대비"] = 분봉요약.get("전일대비")
        요약["등락률"] = 분봉요약.get("등락률")
        요약["상태"] = 분봉요약.get("상태", "준실시간 1분봉 반영")
        요약["기준일"] = 분봉요약.get("기준일")
        요약["기준시각"] = 분봉요약.get("기준시각")
        return 요약
...
def 대시보드보유정보사전(거래df):
    try:
        계산결과 = 포트폴리오계산캐시(거래이력서명생성(거래df), refresh_token=st.session_state.get("price_refresh_token_v43", 0))
...
with 모니터헤더칸1:
    새로고침클릭 = st.button("시세 새로고침", key="refresh_monitor_btn_v839g", use_container_width=True)
if 새로고침클릭:
    try:
        야후1분봉요약가져오기.clear()
        최근OHLCV가져오기.clear()
        ...
        st.cache_data.clear()
    st.session_state["manual_price_refresh_ts_v1"] = 서울현재시각ISO()
    st.session_state["price_refresh_token_v43"] = st.session_state.get("price_refresh_token_v43", 0) + 1
    st.rerun()
...
for row_start in range(0, len(모니터자산목록), 카드열수):
    ...
    for col, (자산명, 자산정보, 구분라벨) in zip(cols, 현재행):
        정보 = 자산현재가정보(자산명, 자산정보, refresh_token=st.session_state.get("price_refresh_token_v43", 0))
...
if 모니터실패건수 > 0:
    ...
    st.caption("상단 모니터는 실시간 현재가를 우선 조회하고, 실패 시 최근 종가로 대체합니다.")
...
수정포트폴리오 = 거래이력편집용자동보정(수정포트폴리오.reset_index(drop=True))
수정포트폴리오, 거래이력변경됨, 자동저장성공, 자동저장메시지 = 거래이력세션반영(
    수정포트폴리오,
    저장강제=False,
    자동저장허용=False,
)
...
계산용거래이력 = st.session_state.get("trade_history_calc_df_v1", 거래이력계산대상추출(수정포트폴리오))
통합점검표 = 거래이력통합점검표캐시(거래이력서명생성(계산용거래이력))
...
# 포트폴리오 계산 결과
# -----------------------------------
계산포트폴리오 = 포트폴리오계산캐시(거래이력서명생성(계산용거래이력), refresh_token=st.session_state.get("price_refresh_token_v43", 0))
