...
APP_VERSION = "v5.13.3-share"  # 공개용 Share 버전 · 심플 세로라인 카드 UI + 샘플 통합자산 데이터 포함

st.set_page_config(page_title=f"투자 분석 시스템 (공개용) {APP_VERSION}", layout="wide")
...
st.caption("공개용 Share 버전 · 개인 정보 제거 · 샘플 거래이력 및 비주식자산 포함")
...
기본포트폴리오 = pd.DataFrame([{'종목코드': '069500', '종목명': 'KODEX 200', '거래일자': '2026-03-10', '거래구분': '매수', '거래수량': 59, '거래단가': 82900, '운용사': '신한IRP', '비고': ''}, {'종목코드': '005930', '종목명': '삼성전자', '거래일자': '2026-03-23', '거래구분': '매수', '거래수량': 5, '거래단가': 186300, '운용사': '미래에셋', '비고': ''}])
...
거래이력자동저장파일 = "share_trade_history_autosave.json"
최근업로드거래이력파일 = "share_trade_history_latest_uploaded.json"
최근업로드메타파일 = "share_trade_history_latest_uploaded_meta.json"
모니터관심종목저장파일 = "share_monitor_custom_assets_v54.json"

IRP비주식자산저장파일 = "share_integrated_non_stock_assets_v513.json"
공개용공유모드 = True


def 기본IRP비주식자산표():
    return pd.DataFrame([{'계좌': '신한은행 IRP', '자산군': 'TDF', '상품명': 'TDF2035', '원금': 50000000, '평가금액': 51873538, '예상연수익률': 3.74, '만기일': '', '반영일자': '2026-04-30', '비고': ''}, {'계좌': '신한은행 IRP', '자산군': 'TDF', '상품명': 'TDF2045', '원금': 30000000, '평가금액': 31443846, '예상연수익률': 4.81, '만기일': '', '반영일자': '2026-04-30', '비고': ''}, {'계좌': '신한은행 IRP', '자산군': '정기예금', '상품명': '푸본현대생명 정기예금', '원금': 27499444, '평가금액': 27499444, '예상연수익률': 3.1, '만기일': '2027-02-11', '반영일자': '2026-04-30', '비고': ''}, {'계좌': '신한은행 IRP', '자산군': '현금성자산', '상품명': '현금성 대기자산', '원금': 5030813, '평가금액': 5030813, '예상연수익률': 2.3, '만기일': '', '반영일자': '2026-04-30', '비고': ''}, {'계좌': '미래에셋증권', '자산군': '현금성자산', '상품명': '예수금', '원금': 17188280, '평가금액': 17188280, '예상연수익률': 0.0, '만기일': '', '반영일자': '2026-04-30', '비고': ''}])
...
def IRP비주식자산불러오기():
    if globals().get("공개용공유모드", False) and "irp_non_stock_assets_df_v512" not in st.session_state:
        df = 기본IRP비주식자산표()
...
def 현재거래이력가져오기():
    ...
    if "trade_history_df_v22" not in st.session_state:
        ...
        # 공개용 Share 버전은 개인정보 잔존 위험을 막기 위해 내 로컬 자동저장/최근업로드 파일보다
        # 코드에 포함된 샘플 데이터를 기본값으로 우선 사용합니다.
        if globals().get("공개용공유모드", False):
            초기df = 기본포트폴리오.copy()
            st.session_state["trade_history_source_v1"] = "share_default"
...
st.markdown("---")
st.markdown('공개용 Share 버전 · 개발자 조현웅 <a href="mailto:hwcho@me.com">hwcho@me.com</a>', unsafe_allow_html=True)
